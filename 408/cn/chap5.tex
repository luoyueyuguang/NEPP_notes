...
\item \textbf{（2009年·选择题·第39题）} 一个TCP连接总是以1 KB的最大段长发送TCP段，发送方有足够多的数据要发送。当拥塞窗口为16 KB时发生了超时，如果接下来的4个RTT时间内的TCP段的传输都是成功的，那么当第4个RTT时间内发送的所有TCP段都得到确认后，拥塞窗口大小是
\begin{center}
\begin{tabular}{ll}
(A) 7 KB & (B) 8 KB \\[6pt]
(C) 9 KB & (D) 16 KB
...
\ifshowsolutions\par\noindent\textbf{解析：} 超时→阈值$=16/2=8$ KB，cwnd$=1$ KB，进入慢开始。RTT1: cwnd$=2$；RTT2: cwnd$=4$；RTT3: cwnd$=8$（到达阈值，进入拥塞避免）；RTT4: cwnd$=9$ KB。\boxed{\textbf{答案：}(C)}\fi

\vspace{8pt}

\item \textbf{（2010年·选择题·第39题）} 主机甲向主机乙发送一个TCP报文段，序号为100，数据部分长度为200 B。主机乙收到后发回的确认号是
\begin{center}
\begin{tabular}{ll}
(A) 100 & (B) 200 \\[6pt]
(C) 300 & (D) 301
...
\item \textbf{（2011年·选择题·第39题）} TCP使用慢开始和拥塞避免算法。设TCP的慢开始门限ssthresh的初始值为8（单位为报文段）。当拥塞窗口上升到12时，网络发生了超时，TCP开始使用慢开始和拥塞避免。第1次到第5次成功传输时拥塞窗口大小依次为
\begin{center}
\begin{tabular}{ll}
...
\ifshowsolutions\par\noindent\textbf{解析：} 超时时cwnd$=1$，ssthresh$=12/2=6$。慢开始：1→2→4→6（达阈值）→拥塞避免：7。\boxed{\textbf{答案：}(A)}\fi

\vspace{8pt}

\item \textbf{（2012年·选择题·第39题）} TCP连接管理中，连接释放的过程称为"四次挥手"。以下关于四次挥手的说法，正确的是
\begin{center}
\begin{tabular}{ll}
...
\item \textbf{（2014年·选择题·第38题）} 主机甲和主机乙已建立TCP连接，甲始终以MSS=1 KB大小的段向乙发送数据，并一直有数据发送；乙每收到一个数据段都会发出一个接收窗口为10 KB的确认段。若甲在$t$时刻发生超时，拥塞窗口为8 KB，则从$t$时刻起，不再发生超时的情况下，经过10个RTT后，甲的发送窗口是
\begin{center}
\begin{tabular}{ll}
(A) 10 KB & (B) 12 KB \\[6pt]
(C) 14 KB & (D) 15 KB
...
\ifshowsolutions\par\noindent\textbf{解析：} 发送窗口$=\min(\text{cwnd},\text{rwnd})$。rwnd始终$=10$ KB。超时后ssthresh$=4$，cwnd$=1$→慢开始。RTT:1→2→4→(拥塞避免)5→6→7→8→9→10→11。但rwnd=10限制窗口为10。第9个RTT后cwnd=10已达rwnd，后续不再增长。\boxed{\textbf{答案：}(A)}\fi
...
\item \textbf{（2015年·选择题·第39题）} 主机甲和乙新建TCP连接，甲向乙发送一个起始序号为200的TCP段后，乙发回的确认段中确认号为800，则甲发送的TCP段携带的数据字节数是
\begin{center}
\begin{tabular}{ll}
(A) 600 & (B) 599 \\[6pt]
(C) 800 & (D) 200
...
\item \textbf{（真题风格·综合题）} TCP的拥塞窗口cwnd和门限ssthresh的初始值分别为1和16（单位为MSS）。按时间顺序发生以下事件：连续成功传输使cwnd分别变为2,4,8,16,17,18,19，随后发生3次重复ACK（快重传），之后又经历2次成功传输。
\begin{enumerate}
    \item[(1)] 画出cwnd随传输轮次变化的折线图；
    \item[(2)] 标注每次状态转换（慢开始→拥塞避免→快恢复→拥塞避免）；
    \item[(3)] 第2次成功传输后cwnd的值是多少？
\end{enumerate}

\ifshowsolutions\par\noindent\textbf{解析：}
(1) 慢开始：1→2→4→8→16（达ssthresh）→拥塞避免：17→18→19。
3次重复ACK触发快重传：ssthresh$=19/2=9.5\approx 9$（向下取整为9或10，TCP实现通常向下），cwnd$=9+3=12$（快恢复）。
之后拥塞避免：13→14。

(2) 轮次1-4:慢开始；轮次5-7:拥塞避免；轮次8:快恢复；轮次9后:拥塞避免。
(3) 第2次成功传输后cwnd$=14$。\fi
