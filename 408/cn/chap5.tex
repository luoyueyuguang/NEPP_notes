...
\ifshowsolutions\par\noindent\textbf{解析：} 超时时cwnd$=1$，ssthresh$=12/2=6$。慢开始：1$\to$2$\to$4$\to$6（达阈值）$\to$拥塞避免：7。\boxed{\textbf{答案：}(A)}\fi
...
\ifshowsolutions\par\noindent\textbf{解析：} 发送窗口$=\min(\text{cwnd},\text{rwnd})$。rwnd始终$=10$ KB。超时后ssthresh$=4$，cwnd$=1$$\to$慢开始。RTT:1$\to$2$\to$4$\to$(拥塞避免)5$\to$6$\to$7$\to$8$\to$9$\to$10$\to$11。但rwnd=10限制窗口为10。第9个RTT后cwnd=10已达rwnd，后续不再增长。\boxed{\textbf{答案：}(A)}\fi
...
\begin{enumerate}
    ...
    \item[(2)] 标注每次状态转换（慢开始$\to$拥塞避免$\to$快恢复$\to$拥塞避免）；
    \item[(3)] 第2次成功传输后cwnd的值是多少？
\end{enumerate}

\ifshowsolutions\par\noindent\textbf{解析：}
(1) 慢开始：1$\to$2$\to$4$\to$8$\to$16（达ssthresh）$\to$拥塞避免：17$\to$18$\to$19。
