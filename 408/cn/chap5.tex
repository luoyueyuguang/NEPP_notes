...
\item \textbf{（真题风格·选择题）} TCP使用慢开始和拥塞避免算法。设TCP的慢开始门限ssthresh的初始值为8（单位为报文段）。当拥塞窗口上升到12时，网络发生了超时，TCP开始使用慢开始和拥塞避免。第1次到第5次成功传输时拥塞窗口大小依次为
\begin{center}
\begin{tabular}{ll}
...
(2) cwnd 从 1 增至 16 的阶段为慢开始；从 16 增至 20 的阶段为拥塞避免；3 个重复 ACK 触发快重传并把窗口降到 10；此后仍按拥塞避免线性增长。
