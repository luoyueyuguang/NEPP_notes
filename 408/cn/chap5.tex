...
\begin{tikzpicture}[font=\scriptsize]
  \filldraw[fill=SoftTeal,draw=black!40] (0,1.6) rectangle (4.5,2.3) node[midway,align=center]{源端口号\\16 bit};
  \filldraw[fill=SoftTeal,draw=black!40] (4.5,1.6) rectangle (9.0,2.3) node[midway,align=center]{目的端口号\\16 bit};
  \filldraw[fill=SoftGray,draw=black!40] (0,0.7) rectangle (4.5,1.4) node[midway,align=center]{UDP 长度\\16 bit};
  \filldraw[fill=SoftGray,draw=black!40] (4.5,0.7) rectangle (9.0,1.4) node[midway,align=center]{UDP 校验和\\16 bit};
  \node[align=center,font=\scriptsize,black!70] at (4.5,-0.15){UDP 首部（8 字节），数据部分接在校验和之后};
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}[>=stealth, font=\scriptsize, scale=1.0,
  hrow/.style={draw,rounded corners=1.2pt,minimum width=3.5cm,minimum height=0.62cm,align=center}]
  % 表头
  \node[hrow,fill=SoftBlue,font=\small\bfseries] (th) at (-1.85,4.0){TCP};
  \node[hrow,fill=SoftTeal,font=\small\bfseries] (uh) at (1.85,4.0){UDP};
  % 对比行
  \node[hrow,fill=SoftBlue!18] (t1) at (-1.85,3.2){面向连接};
  \node[hrow,fill=SoftTeal!18] (u1) at (1.85,3.2){无连接};
  \node[hrow,fill=SoftBlue!18] (t2) at (-1.85,2.4){可靠（确认/重传）};
  \node[hrow,fill=SoftTeal!18] (u2) at (1.85,2.4){不可靠（尽最大努力）};
  \node[hrow,fill=SoftBlue!18] (t3) at (-1.85,1.6){面向字节流};
  \node[hrow,fill=SoftTeal!18] (u3) at (1.85,1.6){面向报文（保留边界）};
  \node[hrow,fill=SoftBlue!18] (t4) at (-1.85,0.8){首部 $\geqslant 20$ B};
  \node[hrow,fill=SoftTeal!18] (u4) at (1.85,0.8){仅首部 8 B};
  \node[hrow,fill=SoftBlue!18] (t5) at (-1.85,0.0){有流量/拥塞控制};
  \node[hrow,fill=SoftTeal!18] (u5) at (1.85,0.0){无流量/拥塞控制};
  % 底部说明
  \node[align=center,font=\scriptsize,black!70] at (0,-0.7){TCP 可靠但开销大（文件传输、网页浏览）；UDP 简单快捷（音视频、DNS、实时应用）。};
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}[>=stealth, font=\scriptsize]
  \draw[->,thick] (0,0) -- (12.2,0) node[right]{序号};
  \foreach \x in {0,2,4,6,8,10,12} \draw (\x,0.05) -- (\x,-0.05) node[below]{\x};
  % 四段：已确认 | 已发送未确认 | 可发送 | 不可发送
  \fill[blue!12] (0,0.15) rectangle (3,0.55);
  \fill[red!15] (3,0.15) rectangle (6,0.55);
  \fill[green!15] (6,0.15) rectangle (9,0.55);
  \fill[gray!12] (9,0.15) rectangle (12,0.55);
  % 窗口下括号
  \draw[<->] (3,-0.5) -- (9,-0.5) node[midway,below,font=\scriptsize]{发送窗口};
  % 名称
  \node[font=\scriptsize,blue!70] at (1.5,0.85){已发送已确认};
  \node[font=\scriptsize,red!70] at (4.5,0.85){已发送未确认};
  \node[font=\scriptsize,green!60!black] at (7.5,0.85){可发送};
  \node[font=\scriptsize,gray!65] at (10.5,0.85){不可发送};
\end{tikzpicture}
\end{center}
...
\begin{conclusion}{cwnd 变化图（图文对照）}{cwnd-graph-visual}
上表逐轮列出 cwnd，形象化后是一条典型的\textbf{锯齿形}曲线：
\begin{center}
\begin{tikzpicture}
\begin{axis}[
  width=0.86\linewidth, height=6.2cm,
  xlabel={传输轮次（RTT）}, ylabel={cwnd（MSS）},
  xmin=0, xmax=25, ymin=0, ymax=26,
  xtick={0,5,10,15,20,25}, ytick={0,4,8,12,16,20,24},
  grid=both, legend pos=north west, font=\footnotesize,
]
\addplot[Teal,thick,mark=*] coordinates {(1,1)(2,2)(3,4)(4,8)(5,16)(6,17)(7,18)(8,19)(9,20)(10,21)(11,22)(12,23)(13,24)(14,1)(15,2)(16,4)(17,8)(18,12)(19,13)(20,14)(21,15)(22,16)(23,8)};
\addlegendentry{cwnd}
\addplot[Navy,dashed,thick,const plot] coordinates {(0,16)(13,16)(13,12)(22,12)(22,8)(25,8)};
\addlegendentry{ssthresh（16、12、8）}
\draw[dashed,red] (axis cs:13,0) -- (axis cs:13,24);
\node[red,font=\scriptsize,anchor=north] at (axis cs:13,1){超时};
\draw[dashed,red] (axis cs:22,0) -- (axis cs:22,16);
\node[red,font=\scriptsize,anchor=north] at (axis cs:22,1){3个重复ACK};
\end{axis}
\end{tikzpicture}
\end{center}
...
\textbf{读图要点（与上表逐轮对应）：}
\begin{enumerate}
    \item \textbf{慢开始段（第 1--5 轮）}：cwnd 每轮翻倍 $1\to2\to4\to8\to16$，曲线陡峭（指数上升）；
    \item \textbf{拥塞避免段（第 6--13 轮）}：cwnd 达到 ssthresh=16 后每轮 $+1$（$17\to\cdots\to24$），坡度明显放缓（线性上升）——这就是\textbf{加法增大}；
    \item \textbf{超时（第 13 轮后）}：cwnd=24 时发生超时，ssthresh 减半为 12、cwnd 跌回 1，重新慢开始（曲线垂直下落）；
    \item \textbf{二次慢开始段}：再次按 $1\to2\to4\to8\to12$ 指数增长，到达新的 ssthresh=12 后转入拥塞避免；
    \item \textbf{3 个重复 ACK（第 22 轮）}：cwnd=16 时收到 3 个重复 ACK 触发快重传，ssthresh 再减半为 8、cwnd 置为 8（教材化简化规则），随后进入拥塞避免。
\end{enumerate}
\textbf{一句话记忆：} 慢开始指数飙、拥塞避免线性走；超时暴跌到 1、三个 ACK 砍一半；每次丢包 ssthresh 都取当场 cwnd 的一半。
\end{conclusion}
...
\begin{tikzpicture}[>=stealth, font=\scriptsize]
  ...
  \draw[thick] (7,0) -- (7,4.4);
  \node[above,font=\small] at (0,4.4){客户端 A};
  \node[above,font=\small] at (7,4.4){服务器 B};
  % 1: SYN
  \draw[->,thick] (0,3.9) -- (7,3.9) node[midway,above]{$\text{SYN}=1,\ \text{seq}=x$};
  % 2: SYN + ACK
  \draw[->,thick] (7,3.2) -- (0,3.2) node[midway,below]{$\text{SYN}=1,\ \text{ACK}=1,\ \text{seq}=y,\ \text{ack}=x+1$};
  % 3: ACK
  \draw[->,thick] (0,2.5) -- (7,2.5) node[midway,above]{$\text{ACK}=1,\ \text{ack}=y+1$};
  % 状态标注
  \node[left] at (0,4.15){\texttt{SYN-SENT}};
  \node[right] at (7,3.9){\texttt{SYN-RCVD}};
  \node[left] at (0,2.15){\texttt{ESTABLISHED}};
  \node[right] at (7,2.75){\texttt{ESTABLISHED}};
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}[>=stealth, font=\scriptsize]
  ...
  \node[above,font=\small] at (0,5.0){客户端 A};
  \node[above,font=\small] at (7,5.0){服务器 B};
  % 1 FIN
  \draw[->,thick] (0,4.5) -- (7,4.5) node[midway,above]{$\text{FIN}=1,\ \text{seq}=u$};
  % 2 ACK
  \draw[->,thick] (7,3.85) -- (0,3.85) node[midway,below]{$\text{ACK}=1,\ \text{ack}=u+1$};
  % 3 FIN+ACK
  \draw[->,thick] (7,3.2) -- (0,3.2) node[midway,below]{$\text{FIN}=1,\ \text{ACK}=1,\ \text{seq}=w,\ \text{ack}=u+1$};
  % 4 ACK
  \draw[->,thick] (0,2.55) -- (7,2.55) node[midway,above]{$\text{ACK}=1,\ \text{ack}=w+1$};
  % 2MSL
  \draw[<->,gray] (0.5,1.15) -- (0.5,2.35) node[midway,right,font=\scriptsize,gray]{等待 2MSL};
  % 状态
  \node[left] at (0,4.75){\texttt{FIN-WAIT-1}};
  \node[right] at (7,4.5){\texttt{CLOSE-WAIT}};
  \node[right] at (7,3.2){\texttt{LAST-ACK}};
  \node[left] at (0,2.35){\texttt{TIME-WAIT}};
  \node[left] at (0,0.85){\texttt{CLOSED}};
  \node[right] at (7,0.85){\texttt{CLOSED}};
\end{tikzpicture}
\end{center}
...
\ifshowsolutions\par\noindent\textbf{解析：} 超时$\to$阈值$=16/2=8$ KB，cwnd$=1$ KB，进入慢开始。RTT1: cwnd$=2$；RTT2: cwnd$=4$；RTT3: cwnd$=8$（到达阈值，进入拥塞避免）；RTT4: cwnd$=9$ KB。\boxed{\textbf{答案：}(C)}\else \answerarea \fi
...
\ifshowsolutions\par\noindent\textbf{解析：} 确认号$=$期望收到的下一字节序号$=100+200=300$。\boxed{\textbf{答案：}(C)}\else \answerarea \fi
...
\ifshowsolutions\par\noindent\textbf{解析：} 超时后ssthresh$=12/2=6$，cwnd$=1$。记录值从1开始：$1\to2\to4$；当某轮开始时cwnd$=4<6$，该轮仍按慢开始近似翻倍为8，并不会人为截断为6。此后cwnd已不小于门限，按拥塞避免增至9。因此序列为1,2,4,8,9。\boxed{\textbf{答案：}(A)}\else \answerarea \fi
...
\ifshowsolutions\par\noindent\textbf{解析：} 主动关闭方发送FIN后进入FIN-WAIT-1，收到ACK后进入FIN-WAIT-2，收到FIN后发送ACK进入TIME-WAIT（2MSL）。\boxed{\textbf{答案：}(D)}\else \answerarea \fi
...
\ifshowsolutions\par\noindent\textbf{解析：} 发送窗口$=\min(\text{cwnd},\text{rwnd})$，且 rwnd 始终为10 KB。超时后ssthresh$=4$ KB、cwnd$=1$ KB。按本章教材化的逐 RTT 规则，10个成功 RTT 后 cwnd 依次增长为 $2,4,5,6,7,8,9,10,11,12$ KB；即使拥塞窗口继续增长，接收窗口仍把实际发送窗口限制为 $\min(12,10)=10$ KB。\boxed{\textbf{答案：}(A)}\else \answerarea \fi
...
\ifshowsolutions\par\noindent\textbf{解析：} 确认号$=$期望的下一字节。确认号800，起始序号200，数据长度$=800-200=600$ B。\boxed{\textbf{答案：}(A)}\else \answerarea \fi
...
(3) 事件发生后的第 2 个成功 RTT 结束时，cwnd$=12$。\else \answerarea \fi
