...
\begin{tikzpicture}[>=stealth, font=\scriptsize]
  % 上排：IP 地址三段（网络号 / 子网号 / 主机号）
  \node[draw,rectangle,minimum width=3.0cm,minimum height=0.7cm] (net) at (1.5,0.4) {网络号};
  \node[draw,rectangle,minimum width=2.2cm,minimum height=0.7cm] (sub) at (4.1,0.4) {子网号};
  \node[draw,rectangle,minimum width=3.6cm,minimum height=0.7cm] (host) at (7.0,0.4) {主机号};
  \node[font=\scriptsize] at (3.4,0.95) {IP 地址};
  % 下排：子网掩码（1 覆盖网络+子网，0 覆盖主机）
  \node[draw,rectangle,minimum width=5.2cm,minimum height=0.7cm,fill=SoftGray] (m1) at (2.6,-0.9) {$1\;1\;\cdots\;1\;1$};
  \node[draw,rectangle,minimum width=3.6cm,minimum height=0.7cm,fill=SoftBlue] (m0) at (7.0,-0.9) {$0\;0\;\cdots\;0\;0$};
  \node[font=\scriptsize] at (3.4,-1.5) {子网掩码};
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}[font=\scriptsize]
  % row 0：版本4/首部长度4/区分服务8/总长度16（10cm/32bit=0.3125cm per bit）
  \filldraw[fill=SoftGray,draw=black!40] (0,3.5) rectangle (1.25,4.2) node[midway,align=center]{版本\\4 bit};
  \filldraw[fill=SoftGray,draw=black!40] (1.25,3.5) rectangle (2.5,4.2) node[midway,align=center]{首部长度\\4 bit};
  \filldraw[fill=SoftTeal,draw=black!40] (2.5,3.5) rectangle (5.0,4.2) node[midway,align=center]{区分服务\\8 bit};
  \filldraw[fill=SoftBlue,draw=black!40] (5.0,3.5) rectangle (10.0,4.2) node[midway,align=center]{总长度\\16 bit};
  % row 1：标识16/标志3/片偏移13（16:3:13）
  \filldraw[fill=SoftTeal,draw=black!40] (0,2.6) rectangle (5.0,3.3) node[midway,align=center]{标识\\16 bit};
  \filldraw[fill=SoftAmber,draw=black!40] (5.0,2.6) rectangle (5.9375,3.3) node[midway,align=center]{标志\\3 bit};
  \filldraw[fill=SoftGray,draw=black!40] (5.9375,2.6) rectangle (10.0,3.3) node[midway,align=center]{片偏移\\13 bit};
  % row 2：TTL8/协议8/首部校验和16（8:8:16）
  \filldraw[fill=SoftGray,draw=black!40] (0,1.7) rectangle (2.5,2.4) node[midway,align=center]{TTL\\8 bit};
  \filldraw[fill=SoftGray,draw=black!40] (2.5,1.7) rectangle (5.0,2.4) node[midway,align=center]{协议\\8 bit};
  \filldraw[fill=SoftGray,draw=black!40] (5.0,1.7) rectangle (10.0,2.4) node[midway,align=center]{首部校验和\\16 bit};
  % row 3：源 IP
  \filldraw[fill=SoftTeal,draw=black!40] (0,0.8) rectangle (10.0,1.5) node[midway,align=center]{源 IP 地址\\32 bit};
  % row 4：目的 IP
  \filldraw[fill=SoftTeal,draw=black!40] (0,-0.1) rectangle (10.0,0.6) node[midway,align=center]{目的 IP 地址\\32 bit};
  \node[align=center,font=\scriptsize,black!70] at (5.0,4.55){IPv4 首部（固定部分 20 字节，每行 32 bit = 4 字节）};
  \node[align=center,font=\scriptsize,black!70] at (5.0,-0.75){固定部分共 5 行；其下为「选项」（可选）与「数据」};
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}[>=stealth, font=\scriptsize]
  % 原数据报
  \filldraw[fill=SoftBlue,draw=black!40] (0,3.2) rectangle (8.4,3.85) node[midway]{原数据报数据部分 3800 B（MTU=1420，IP 首部 20 B）};
  % 分片1
  \filldraw[fill=SoftGray,draw=black!40] (0,2.25) rectangle (1.0,2.9) node[midway,align=center]{IP\\首部};
  \filldraw[fill=SoftTeal,draw=black!40] (1.0,2.25) rectangle (4.1,2.9) node[midway,align=center]{分片1 数据 1400B\\偏移 0，MF=1};
  % 分片2
  \filldraw[fill=SoftGray,draw=black!40] (0,1.25) rectangle (1.0,1.9) node[midway,align=center]{IP\\首部};
  \filldraw[fill=SoftTeal,draw=black!40] (1.0,1.25) rectangle (4.1,1.9) node[midway,align=center]{分片2 数据 1400B\\偏移 175，MF=1};
  % 分片3
  \filldraw[fill=SoftGray,draw=black!40] (0,0.25) rectangle (1.0,0.9) node[midway,align=center]{IP\\首部};
  \filldraw[fill=SoftTeal,draw=black!40] (1.0,0.25) rectangle (3.1,0.9) node[midway,align=center]{分片3 数据 1000B\\偏移 350，MF=0};
  \node[align=center,font=\scriptsize,black!70] at (2.0,-0.3){每个分片都携带完整 IP 首部（20 B）；片偏移值是数据字节偏移量 $\div$ 8；仅目的主机重组};
\end{tikzpicture}
\end{center}
...
\textbf{自治系统（AS，Autonomous System）：} 由同一机构管理、对外呈现统一路由策略的一组网络。参与公网 BGP 路由的 AS 使用分配的公有 ASN；此外也存在仅供组织内部使用、不能直接在公网传播的私有 ASN，不能笼统地说所有 ASN 都全球唯一。
...
\begin{enumerate}
    ...
    \item \textbf{构建 LSDB：} 每个路由器收集本区域内洪泛的 LSA，构建链路状态数据库（LSDB）——本区域的完整拓扑图；跨区域信息由 ABR 按区域间路由传播。
...
\textbf{收敛速度} & 慢（计数到无穷问题） & 快（区域内 LSDB 一致） \\
...
\begin{itemize}
    ...
    \item OSPF 的区域内路由器拥有本区域的完整拓扑（LSDB），独立计算最短路径树；多区域网络中不能简单说每台路由器都拥有整个 AS 的详细拓扑。
...
\begin{itemize}
    ...
    \item Peering 关系仍会交换和通告路由；“不提供 transit”指通常不为对等方转发其通往第三方 AS 的流量，而不是不传递路由。
\end{itemize}
\end{conclusion}
...
\begin{itemize}
    ...
    \item \textbf{组播 MAC 地址：} 以太网中，组播 IP 到组播 MAC 有固定映射：MAC 地址前 24 位固定为 01-00-5E，第 25 位固定为 0（即第 4 字节最高位为 0，该字节范围为 0x00--0x7F），后 23 位为组播 IP 地址的低 23 位（IP 的高 9 位不参与映射）。
\end{itemize}
...
\ifshowsolutions\par\noindent\textbf{解析：} 数据部分$=2000-20=1980$ B。每片最大数据$=820-20=800$ B（需8字节对齐）。$1980/800=2.475$，需3片。片1:800 B(偏移0)，片2:800 B(偏移100)，片3:380 B(偏移200)。\boxed{\textbf{答案：}(B)}\else \answerarea \fi
...
\ifshowsolutions\par\noindent\textbf{解析：} 掩码 255.255.255.248 为 /29。从原 /24 地址块中借用 $29-24=5$ 位作为子网位，因此可划分 $2^5=32$ 个等长子网；每个子网保留 $32-29=3$ 位主机位，传统子网中最多可分配 $2^3-2=6$ 个主机地址。\boxed{\textbf{答案：}(B)}\else \answerarea \fi
...
\ifshowsolutions\par\noindent\textbf{解析：} RIP收到邻居R2到达Net1距离为2，则R1经R2到Net1的距离$=2+1=3$。与原有路由比较取最小。\boxed{\textbf{答案：}(B)}\else \answerarea \fi
...
\ifshowsolutions\par\noindent\textbf{解析：} NAPT/PAT 通过包含协议、地址和端口号的映射项区分并发连接；仅靠共享的公网 IP 或目的 IP 无法区分两台内部主机的连接。\boxed{\textbf{答案：}(C)}\else \answerarea \fi
...
\ifshowsolutions\par\noindent\textbf{解析：} IPv4 地址属于 IP 首部，因此影响 IPv4 首部校验和；地址参与 TCP 伪首部，端口号属于 TCP 首部，因此 TCP 校验和也必须更新。链路层 FCS 会由每一跳重新生成，但不是本题所问的 NAT 字段一致性校验。\boxed{\textbf{答案：}(B)}\else \answerarea \fi
...
\ifshowsolutions\par\noindent\textbf{解析：} ARP报文直接封装在以太网帧中（数据链路层），不使用IP。ARP介于网络层和数据链路层之间。\boxed{\textbf{答案：}(B)}\else \answerarea \fi
...
\ifshowsolutions\par\noindent\textbf{解析：} IP首部校验和错误，路由器直接丢弃。IP不提供纠错和重传机制。\boxed{\textbf{答案：}(C)}\else \answerarea \fi
...
\ifshowsolutions\par\noindent\textbf{解析：}
(1) /26再分4子网需2位$\to$/28。
(2) 每个子网共有 $2^{4}=16$ 个地址，其中可分配给普通主机的地址为 $16-2=14$ 个（扣除网络地址和广播地址）。
(3) 136.23.12.64/28(64-79), 136.23.12.80/28(80-95), 136.23.12.96/28(96-111), 136.23.12.112/28(112-127)。\else \answerarea \fi

\vspace{8pt}

\item \textbf{（真题风格·综合题）} 收到一个IP数据报，其首部十六进制表示为：\texttt{45 00 00 54 00 03 20 00 80 01 99 52 C0 A8 00 01 C0 A8 00 02}。回答：
\begin{enumerate}
...
\ifshowsolutions\par\noindent\textbf{解析：}
总长度$=0x0054=84$ B。首部长度$=(0x45\&0x0F)\times 4=5\times 4=20$ B。数据部分$=84-20=64$ B。标志+片偏移$=0x2000$：DF=0，MF=1，片偏移字段为 0，表示这是第一片且后面还有分片。协议$=0x01$(ICMP)。TTL$=0x80=128$。\else \answerarea \fi
