...
\item \textbf{（2009年·选择题·第38题）} 一个IP数据报总长度为2000 B（含固定首部20 B），要通过MTU=820 B的链路传输。设IP数据报首部无选项字段，则原始数据报被分成的片数是
\begin{center}
\begin{tabular}{ll}
(A) 2 & (B) 3 \\[6pt]
(C) 4 & (D) 5
...
\item \textbf{（2010年·选择题·第37题）} 某网络的IP地址空间为192.168.5.0/24，采用定长子网划分，子网掩码为255.255.255.248，则该网络的最大子网个数和每个子网内的最大可分配地址个数分别是
\begin{center}
\begin{tabular}{ll}
(A) 32, 8 & (B) 32, 6 \\[6pt]
(C) 8, 32 & (D) 8, 30
...
\ifshowsolutions\par\noindent\textbf{解析：} /24分出3位子网（掩码248=11111000），子网数$=2^3=8$。每子网$2^5-2=30$个可用地址。等等——/24$\to$子网掩码/29（248），子网位数$=29-24=5$，子网数$=2^5=32$。每子网主机位$=32-29=3$，地址数$=2^3-2=6$。\boxed{\textbf{答案：}(B)}\fi

\vspace{8pt}

\item \textbf{（2011年·选择题·第37题）} 某网络拓扑如下图所示，路由器R1和R2均运行RIP协议。R1的路由表如下，当R1收到R2发来的距离向量报文（目的网络Net1, 距离2）后，R1更新后的路由表中，到达Net1的跳数是
（图略）
\begin{center}
\begin{tabular}{ll}
(A) 2 & (B) 3 \\[6pt]
...
\item \textbf{（2012年·选择题·第38题）} 在TCP/IP体系结构中，ARP协议数据单元封装的载体是
\begin{center}
\begin{tabular}{ll}
(A) IP数据报 & (B) 以太网帧 \\[6pt]
(C) TCP报文段 & (D) UDP数据报
...
\item \textbf{（2015年·选择题·第37题）} 某路由器收到了一个IP数据报，对其首部进行校验后发现该数据报存在错误。路由器最有可能采取的动作是
\begin{center}
\begin{tabular}{ll}
