...
\item \textbf{（2009年·选择题·第40题）} FTP客户和服务器间传递FTP命令时，使用的连接是
\begin{center}
\begin{tabular}{ll}
...
\item \textbf{（2010年·选择题·第40题）} 如果本地域名服务器无缓存，当采用递归方法解析另一网络中的某主机域名时，用户主机和本地域名服务器分别发送的域名请求消息条数分别为
\begin{center}
\begin{tabular}{ll}
(A) 1条,1条 & (B) 1条,多条 \\[6pt]
(C) 多条,1条 & (D) 多条,多条
...
\item \textbf{（2012年·选择题·第40题）} 若用户1与用户2之间发送和接收电子邮件的过程如下图所示，则图中①,②,③阶段分别使用的协议最可能是
（图略：用户1→邮件服务器1→邮件服务器2→用户2）
\begin{center}
\begin{tabular}{ll}
...
\ifshowsolutions\par\noindent\textbf{解析：} 用户1→服务器1:SMTP；服务器1→服务器2:SMTP（邮件服务器之间也使用SMTP）；服务器2→用户2:POP3（用户从服务器取邮件）。\boxed{\textbf{答案：}(A)}\fi

\vspace{8pt}

\item \textbf{（2014年·选择题·第40题）} 使用浏览器访问某大学Web网站主页时，不可能使用到的协议是
\begin{center}
\begin{tabular}{ll}
(A) PPP & (B) ARP \\[6pt]
(C) SMTP & (D) HTTP
...
\item \textbf{（2018年·选择题·第40题）} 在TCP/IP体系结构中，以下协议中属于应用层的是
\begin{center}
\begin{tabular}{ll}
(A) ARP & (B) ICMP \\[6pt]
(C) DHCP & (D) TCP
...
\ifshowsolutions\par\noindent\textbf{解析：}
(1) 递归：本地DNS$\to$根$\to$本地DNS$\to$com$\to$本地DNS$\to$example.com$\to$本地DNS$\to$主机（每个上级替下级查）。迭代：本地DNS$\to$根（返回com地址）$\to$本地DNS$\to$com（返回example.com地址）$\to$本地DNS$\to$example.com（返回IP）$\to$主机。
(2) 递归加重根服务器负担（需替查询者完成全部查询）；迭代加重本地DNS负担（需自己逐级查询）。\fi

\vspace{8pt}

\item \textbf{（真题风格·综合题）} 简述HTTP持久连接和非持久连接的区别。某网页包含1个基本HTML文件和5个JPEG图像，所有文件均在同一个服务器上。分别计算使用非持久连接和持久连接（流水线方式）所需的RTT数（忽略传输时间）。

\ifshowsolutions\par\noindent\textbf{解析：}
非持久连接（HTTP/1.0）：每个对象需单独TCP连接。HTML:2RTT(建立+请求)；5个图像:各2RTT。总计$6\times 2=12$ RTT。

持久连接流水线（HTTP/1.1）：1个TCP连接，连续发送6个请求。建立连接:1RTT；HTML请求+响应:1RTT；5个图像连续请求+5个响应（可重叠）。总计$1+1+1=3$ RTT（TCP建立+第一个对象确认+最后一个对象确认）。实际上HTTP/1.1持久连接流水线约2-3 RTT。\fi
