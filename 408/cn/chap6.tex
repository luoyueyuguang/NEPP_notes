...
\begin{center}
\begin{tikzpicture}[>=stealth, font=\scriptsize, level distance=1.7cm, level 1/.style={sibling distance=3.0cm}, level 2/.style={sibling distance=2.6cm}]
\node[draw,rectangle,fill=Navy!12] (root) {$\cdot$（根域）}
  child { node[draw,rectangle] {com}
    child { node[draw,rectangle] {example.com} } }
  child { node[draw,rectangle,fill=Teal!12] {org}
    child { node[draw,rectangle] {example.org} } }
  child { node[draw,rectangle] {cn}
    child { node[draw,rectangle] {edu.cn} } };
\node[align=center,font=\scriptsize,black!70] at (7.2,0.2){域名层次树状结构\\（右向层次递增，解析从根向下）};
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}[>=stealth, font=\scriptsize,
  nbox/.style={draw, rounded corners=2pt, minimum width=1.9cm, minimum height=0.6cm, font=\scriptsize}]
  \node[nbox,fill=SoftTeal] (h) at (0,0){主机};
  \node[nbox,fill=SoftBlue] (l) at (2.6,0){本地DNS};
  \node[nbox,fill=SoftGray] (r) at (6.6,1.6){根DNS};
  \node[nbox,fill=SoftGray] (t) at (6.6,0){顶级DNS};
  \node[nbox,fill=SoftGray] (a) at (6.6,-1.6){权威DNS};
  \draw[->,thick,Teal] (h) -- (l) node[midway,above,font=\scriptsize]{递归};
  \draw[->,thick] (l) -- (r) node[midway,above,font=\scriptsize]{(1)};
  \draw[->,thick] (l) -- (t) node[midway,above,font=\scriptsize]{(2)};
  \draw[->,thick] (l) -- (a) node[midway,above,font=\scriptsize]{(3)};
  \draw[->,dashed,Burgundy] (r) -- (l) node[midway,below,font=\scriptsize]{顶级DNS地址};
  \draw[->,dashed,Burgundy] (t) -- (l) node[midway,below,font=\scriptsize]{权威DNS地址};
  \draw[->,dashed,Burgundy] (a) -- (l) node[midway,below,font=\scriptsize]{最终IP};
  \draw[->,dashed,Burgundy] (l) -- (h) node[midway,below,font=\scriptsize]{最终IP};
  \node[align=center,font=\scriptsize,black!70] at (3.3,-2.6){主机$\to$本地DNS 为递归查询；本地DNS$\to$根/顶级/权威为\textbf{迭代查询}\\
  ——每次由一个服务器返回下一站地址，由本地DNS 自己继续查\\
  （实线：请求，虚线：返回）};
\end{tikzpicture}
\end{center}
...
\begin{center}
\begin{tikzpicture}[>=stealth, font=\scriptsize]
  \filldraw[fill=SoftTeal,draw=black!40] (0,0.7) rectangle (1.8,1.4) node[midway]{http};
  \node[font=\scriptsize] at (2.2,1.05){://};
  \filldraw[fill=SoftBlue,draw=black!40] (2.6,0.7) rectangle (6.4,1.4) node[midway]{www.example.com};
  \node[font=\scriptsize] at (6.7,1.05){:};
  \filldraw[fill=SoftAmber,draw=black!40] (7.0,0.7) rectangle (8.2,1.4) node[midway]{80};
  \node[font=\scriptsize] at (8.55,1.05){/};
  \filldraw[fill=SoftTeal,draw=black!40] (8.9,0.7) rectangle (11.3,1.4) node[midway]{index.html};
  \node[font=\scriptsize] at (0.9,1.65){协议};
  \node[font=\scriptsize] at (4.5,1.65){主机（域名/IP）};
  \node[font=\scriptsize] at (7.6,1.65){端口};
  \node[font=\scriptsize] at (10.1,1.65){路径};
  \node[align=center,font=\scriptsize,black!70] at (5.6,-0.2){$\texttt{http://www.example.com:80/index.html}$ 的分解};
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}[>=stealth, font=\scriptsize, scale=1.0,
  hdrow/.style={draw,rounded corners=1.2pt,minimum width=6.3cm,minimum height=0.66cm,align=center},
  row/.style={draw,rounded corners=1.2pt,minimum width=6.3cm,minimum height=0.62cm,align=center}]
  % 左列：请求报文
  \node[hdrow,fill=SoftTeal!32,font=\small\bfseries] (rt) at (-3.3,4.5){请求报文 (Request)};
  \node[row,fill=SoftTeal!22] (r1) at (-3.3,3.65){请求行：\texttt{GET /index.html HTTP/1.1}};
  \node[row,fill=SoftGray!22] (r2) at (-3.3,2.62){首部行：\texttt{Host: www.example.com}\\ \texttt{Cookie: 1678}};
  \node[row,fill=white!95] (r3) at (-3.3,1.75){空行};
  \node[row,fill=SoftAmber!22] (r4) at (-3.3,0.88){实体主体（通常为空）};
  % 右列：响应报文
  \node[hdrow,fill=SoftTeal!32,font=\small\bfseries] (st) at (3.3,4.5){响应报文 (Response)};
  \node[row,fill=SoftTeal!22] (s1) at (3.3,3.65){状态行：\texttt{HTTP/1.1 200 OK}};
  \node[row,fill=SoftGray!22] (s2) at (3.3,2.62){首部行：\texttt{Set-Cookie: 1678}\\ \texttt{Content-Type: text/html}};
  \node[row,fill=white!95] (s3) at (3.3,1.75){空行};
  \node[row,fill=SoftAmber!22] (s4) at (3.3,0.88){实体主体：HTML/JSON 网页内容};
  % 结构对齐箭头（对应关系）
  \draw[<->,gray,thick] (r1.east) -- (s1.west);
  \draw[<->,gray,thick] (r2.east) -- (s2.west);
  \draw[<->,gray,thick] (r4.east) -- (s4.west);
  % 底部说明
  \node[align=center,font=\scriptsize,black!70] at (0,-0.4){请求与响应均形如「起始行 + 首部行 + 空行 + 实体主体」；\\客户端在请求中用 \texttt{Cookie} 首部携带会话标识，服务器在响应中用 \texttt{Set-Cookie} 首部下发会话标识。};
\end{tikzpicture}
\end{center}
...
\textbf{详细过程：}
\begin{enumerate}
    \item \textbf{DHCPDISCOVER（发现阶段）}：客户端在物理子网内广播 UDP 报文（服务器端口 67），源 IP = 0.0.0.0，目的 IP = 255.255.255.255（受限广播）。网络中可能存在多个 DHCP 服务器；收到报文且愿意、能够提供配置的服务器可以响应。

    \item \textbf{DHCPOFFER（提供阶段）}：愿意提供地址的服务器可以回应 DHCPOFFER 报文（包含：可分配的 IP 地址 yiaddr、子网掩码、租用期、DHCP 服务器标识等）。因此客户端可能收到零个、一个或多个 OFFER，并非每台服务器都必然回应。

    \item \textbf{DHCPREQUEST（请求阶段）}：客户端选择一个 OFFER，再次广播 DHCPREQUEST 报文，明确声明选择哪个 DHCP 服务器（报文中包含该服务器的标识符）。\textbf{广播的目的：} 让其他提供过 OFFER 的服务器知道自己未被选中，从而撤销本次要约，并在实现已预留地址时释放该预留。
...
\ifshowsolutions\par\noindent\textbf{解析：} FTP使用两个TCP连接：控制连接（端口21）传递命令，数据连接（端口20）传递文件数据。\boxed{\textbf{答案：}(A)}\else \answerarea \fi
...
\ifshowsolutions\par\noindent\textbf{解析：} 递归解析：用户主机向本地域名服务器发1条请求，本地域名服务器代替用户完成全部查询（可能发多条请求），最终返回结果。用户发1条，本地服务器发多条。\boxed{\textbf{答案：}(B)}\else \answerarea \fi
...
\ifshowsolutions\par\noindent\textbf{解析：} 用户1$\to$服务器1:SMTP；服务器1$\to$服务器2:SMTP（邮件服务器之间也使用SMTP）；服务器2$\to$用户2:POP3（用户从服务器取邮件）。\boxed{\textbf{答案：}(A)}\else \answerarea \fi
...
\ifshowsolutions\par\noindent\textbf{解析：} 访问Web使用HTTP协议，需要ARP解析MAC地址，可能使用PPP（若是拨号连接）。SMTP是电子邮件发送协议，与Web访问无关。\boxed{\textbf{答案：}(C)}\else \answerarea \fi
...
\ifshowsolutions\par\noindent\textbf{解析：} ARP属网络接口层，ICMP属网际层，TCP属传输层，DHCP属应用层（基于UDP）。\boxed{\textbf{答案：}(C)}\else \answerarea \fi
...
(2) 完全递归要求上级服务器代请求者继续查询，增加上级（包括根服务器）的状态与处理负担；迭代把逐级查询工作交给本地 DNS，因此本地 DNS 的查询负担更大。实际 Internet 中通常是「主机到本地 DNS 递归、本地 DNS 向层次服务器迭代」。\else \answerarea \fi
...
持久连接流水线：建立唯一的 TCP 连接用 1 RTT；请求并收到基本 HTML 用 1 RTT；得知图像 URL 后一次流水线发出 5 个请求并收到响应，用 1 RTT。总计 $1+1+1=3$ RTT。\else \answerarea \fi
