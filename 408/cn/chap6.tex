...
\begin{tikzpicture}[>=stealth, font=\scriptsize,
  ...
  \node[nbox,fill=SoftGray] (r) at (5.2,0){根DNS};
  \node[nbox,fill=SoftGray] (t) at (7.8,0){顶级DNS};
  \node[nbox,fill=SoftGray] (a) at (10.4,0){权威DNS};
  \draw[->,thick,Teal] (h) -- (l) node[midway,above,font=\scriptsize]{递归};
  \draw[->,thick] (l) -- (r) node[midway,above,font=\scriptsize]{(1)};
  \draw[->,thick] (r) -- (t) node[midway,above,font=\scriptsize]{(2)};
  \draw[->,thick] (t) -- (a) node[midway,above,font=\scriptsize]{(3)};
  \draw[->,dashed,Burgundy] (a) -- (t) node[midway,below,font=\scriptsize]{记录};
  \draw[->,dashed,Burgundy] (t) -- (r);
  \draw[->,dashed,Burgundy] (r) -- (l);
  \draw[->,dashed,Burgundy] (l) -- (h) node[midway,below,font=\scriptsize]{最终IP};
  \node[align=center,font=\scriptsize,black!70] at (5.2,-0.75){主机$\to$本地DNS 为递归查询；本地DNS$\to$根/顶级/权威为迭代查询（实线：请求，虚线：返回）};
\end{tikzpicture}
\end{center}
