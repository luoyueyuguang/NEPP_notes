...
\textbf{递归查询（Recursive Query）：} 主机向本地域名服务器发起查询后，\textbf{本地域名服务器以客户的身份逐级代理查询}，最终将\emph{完整结果}返回给主机。此时若本地域名服务器不知道结果，它自己向根服务器查询，而不是返回给主机让主机再去查。

\[
\text{主机} \xrightarrow[]{(1)递归} \text{本地DNS} \xrightarrow[]{(2)递归} \text{根DNS} \xrightarrow[]{(3)递归} \text{顶级DNS} \xrightarrow[]{(4)递归} \text{权限DNS}
\]
\[
\text{权限DNS} \xrightarrow[]{(5)} \text{顶级DNS} \xrightarrow[]{(6)} \text{根DNS} \xrightarrow[]{(7)} \text{本地DNS} \xrightarrow[]{(8)} \text{主机}
\]

\textbf{迭代查询（Iterative Query）：} 当被查询的服务器没有所需记录时，\textbf{它返回下一步应查询的服务器的 IP 地址}，由请求者自己继续查询。本地域名服务器向根、顶级、权限服务器的查询通常为迭代。

\[
\text{主机} \xleftrightarrow[]{\text{递归}} \text{本地DNS} \xrightleftharpoons[\text{返回根IP}]{\text{迭代}} \text{根DNS} \xrightleftharpoons[\text{返回顶级IP}]{\text{迭代}} \text{顶级DNS} \xrightleftharpoons[\text{返回结果}]{\text{迭代}} \text{权限DNS}
\]
\end{conclusion}
...
\begin{itemize}
    \item \textbf{必需部分：} 主机 $\to$ 本地域名服务器，永远是\underline{递归查询}（主机不承担迭代负担）。
...
\begin{itemize}
    ...
    \item 主机本身也缓存 DNS 记录（浏览器缓存 + OS 缓存），如 \texttt{hosts} 文件。
...
\end{enumerate}
邮件的整个生命周期：\textbf{发送方 UA} $\xrightarrow{\text{SMTP}}$ \textbf{发送方邮件服务器} $\xrightarrow{\text{SMTP}}$ \textbf{接收方邮件服务器} $\xrightarrow{\text{POP3/IMAP}}$ \textbf{接收方 UA}。
\end{mydef}

\subsubsection{SMTP — 简单邮件传送协议}

\begin{conclusion}{SMTP 协议}{smtp}
SMTP（Simple Mail Transfer Protocol）是\textbf{发送}邮件的协议，使用 TCP 端口 25（或 587 用于加密提交）。
...
\begin{itemize}
    ...
    \item \textbf{SMTP 仅支持 7 位 ASCII}，不能直接传输二进制文件或非英文字符。需要 MIME 扩展解决。
...
存储模型 & 下载到本地，默认删除服务器副本 & 服务器端管理，多设备同步 \\
文件夹 & 不支持（仅收件箱） & 支持多文件夹、搜索 \\
工作模式 & 下载—删除（或保留） & 联机操作 \\
离线能力 & 强（邮件已下到本地） & 弱（需联机访问） \\
适用场景 & 单设备收邮件 & 多设备同步邮件 \\
\hline
\end{tabular}
\end{tablebox}
...
\begin{itemize}
    ...
    \item \texttt{Connection: keep-alive} — 指定持久连接。
...
传送时间：
\[
T_{\text{持久}} = 1\text{RTT} + \sum \text{各对象传输时间}
\]

\textbf{流水线方式（Pipelining）：} 持久连接的进一步优化——不需等前一个对象的响应，可以连续发送请求（类似滑动窗口）。传送时间进一步缩小为：
\[
T_{\text{流水线}} \approx 2\text{RTT} + \sum \text{各对象传输时间}
\]
（第一个 RTT 建连接，第二个 RTT 完成所有请求—响应。）
\end{conclusion}
...
\begin{enumerate}
    \item \textbf{DNS 递归 vs 迭代查询}：主机到本地一定是递归；本地到其他通常是迭代。
...
\item \textbf{（单项选择题）} DNS 协议在传输层使用的协议和端口号是
\begin{center}
\begin{tabular}{ll}
(A) TCP 53 & (B) UDP 53 \\[6pt]
(C) TCP 67 & (D) UDP 67
\end{tabular}
\end{center}

\item \textbf{（单项选择题）} 以下关于 FTP 的说法，\textbf{错误的是}
\begin{center}
\begin{tabular}{p{14cm}}
(A) FTP 使用两条 TCP 连接：控制连接和数据连接 \\[4pt]
(B) FTP 的控制连接使用端口 21，数据连接使用端口 20 \\[4pt]
(C) FTP 被动模式下，服务器主动向客户端发起数据连接 \\[4pt]
(D) FTP 的控制连接在整个会话期间始终保持
\end{tabular}
\end{center}

\item \textbf{（单项选择题）} 在浏览器中输入 \texttt{www.example.com} 后，主机首先向本地 DNS 服务器发出的是
\begin{center}
\begin{tabular}{ll}
(A) 迭代查询 & (B) 递归查询 \\[6pt]
(C) 直接查询根域名服务器 & (D) 区域传送
...
\item \textbf{（填空题）} DHCP 协议的四步过程依次是 \underline{\qquad}（发现）、\underline{\qquad}（提供）、\underline{\qquad}（请求）和 \underline{\qquad}（确认），简称 D-O-R-A。

\item \textbf{（填空题）} HTTP 状态码中，200 表示 \underline{\qquad}，301 表示 \underline{\qquad}，404 表示 \underline{\qquad}，500 表示 \underline{\qquad}。

\item \textbf{（填空题）} 在电子邮件系统中，发送邮件使用 \underline{\qquad} 协议（端口 \underline{\qquad}），接收邮件可使用 \underline{\qquad} 协议（端口 110）或 \underline{\qquad} 协议（端口 143）。（填协议缩写）

\item \textbf{（填空题）} FTP 的\textbf{主动模式}中，数据连接由 \underline{\qquad}（填「服务器」或「客户端」）从端口 \underline{\qquad} 向 \underline{\qquad}（填「服务器」或「客户端」）的端口发起。

\item \textbf{（简答题）} 简要说明 SMTP 和 POP3 的区别，以及 MIME 扩展的作用。
...
\item \textbf{（真题风格，单项选择题）} 某主机通过本地域名服务器解析域名 \texttt{www.abc.com}，本地域名服务器依次向根域名服务器、.com 顶级域名服务器和 abc.com 权限域名服务器发出 DNS 查询。设每次查询的 RTT 均为 $R$，忽略其他时延，则解析过程的总时延至少为
\begin{center}
\begin{tabular}{ll}
(A) 1R & (B) 2R \\[6pt]
(C) 3R & (D) 4R
...
\item \textbf{（真题风格，单项选择题）} 关于 DHCP 协议，以下说法\textbf{正确的是}
\begin{center}
\begin{tabular}{p{14cm}}
(A) DHCP 服务器使用 TCP 协议与客户端通信 \\[4pt]
(B) DHCPREQUEST 报文由客户端单播发送给选定的 DHCP 服务器 \\[4pt]
(C) DHCPDISCOVER 报文的源 IP 地址为 0.0.0.0 \\[4pt]
(D) DHCP 分配的 IP 地址是永久的
\end{tabular}
\end{center}

\item \textbf{（真题风格，综合题）} 一个 HTML 网页（大小为 $H$）中包含 5 个内嵌图片（大小均为 $P$）。客户端到服务器的 RTT 均为 $R$，忽略传输时间（$H,P \approx 0$）。回答以下问题：
\begin{enumerate}
    \item[(1)] 若使用非持久 HTTP/1.0（串行），从客户端请求网页到收到网页中所有内容，总共需要多长时间？
    \item[(2)] 若使用持久 HTTP/1.1（流水线），总共需要多长时间？
    \item[(3)] 若使用持久 HTTP/1.1，但非流水线方式，总共需要多长时间？
\end{enumerate}

\item \textbf{（真题风格，综合题）} 假设主机 A 要通过 FTP 从服务器 S 下载一个文件。回答以下问题：
\begin{enumerate}
    \item[(1)] 控制连接和数据连接各使用哪个端口号？
    \item[(2)] 若 A 位于 NAT 后面且未配置端口映射，FTP 应选择主动模式还是被动模式？请说明理由。
    \item[(3)] 简要说明 FTP 使用双连接的原因（即控制连接和数据连接分离的好处）。
\end{enumerate}

\item \textbf{（真题风格，综合题）} 描述从主机开机（无 IP 地址）到通过 DHCP 获得 IP 地址并可访问互联网的完整过程，包括 DHCP 四个阶段及后续 DNS 解析过程。（要求：说明各阶段报文的发送方式为广播还是单播。）
...
\noindent\textbf{拓展阅读：}
\begin{itemize}
    \item Kurose \& Ross《计算机网络：自顶向下方法》第 2 章（应用层）——内容最为丰富，从 DNS 到 HTTP 到 P2P 覆盖所有应用层核心协议，例题和习题质量极高，适合深入理解 HTTP 持久连接的计算。
    \item 谢希仁《计算机网络》第 6 章（应用层）——DNS、FTP、电子邮件、万维网等基础协议的标准国内表述，用于对齐 408 术语和考点。
    \item RFC 1034/1035（DNS）、RFC 959（FTP）、RFC 5321（SMTP）、RFC 1939（POP3）、RFC 3501（IMAP）、RFC 2616/7230-7235（HTTP/1.1）、RFC 2131（DHCP）——协议设计的原始文档，想深入理解协议细节时可查阅。
    \item 陈海波《现代操作系统》——可以作为操作系统和网络交叉（如 Socket 编程、网络 I/O 模型）的补充参考。
\end{itemize}
