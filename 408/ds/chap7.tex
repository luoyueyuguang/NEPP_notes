...
\textbf{复杂度分析：}
\begin{itemize}
    \item 比较次数：$\sum_{i=1}^{n-1}\lceil\log_2 i\rceil \approx n\log_2 n = O(n\log n)$。
...
\textbf{复杂度分析：}
\begin{itemize}
    \item 时间复杂度：与增量序列有关。最坏 $O(n^2)$（希尔原始增量），Hibbard 增量最坏 $O(n^{3/2})$。平均约为 $O(n^{1.3})$。
    \item 空间复杂度：$O(1)$。
...
\boxed{\textbf{例}} 对序列 $\{49,38,65,97,76,13,27,49',55,4\}$ 进行希尔排序（增量 $5,3,1$）：\\
第一趟（$d=5$）：子序列 $\{49,13\}$、$\{38,27\}$、$\{65,49'\}$、$\{97,55\}$、$\{76,4\}$ 排序后得 $\{13,27,49',55,4,49,38,65,97,76\}$。注意 $49$ 和 $49'$ 的相对次序已变化——\textbf{不稳定}。
...
\begin{itemize}
    \item \textbf{给定序列，写出每趟排序后的结果。} 注意：408 的快排一趟通常指一次 Partition 执行完毕（pivot 落位，左右分好），而不是一次递归调用。
    \item \textbf{最坏情况的触发条件}：初始序列基本有序或基本逆序，且 pivot 选法固定（如始终选第一个/最后一个）时退化为 $O(n^2)$。优化方法：随机选 pivot、三数取中。
    \item \textbf{递归次数 ≠ 比较次数}：递归深度 = 栈深度，每趟比较次数 $\approx$ 子序列长度。
    \item \textbf{快排的趟数与初始序列有关}：有序时需 $n$ 趟（每趟定一个位置），随机时约 $\log_2 n$ 趟。
\end{itemize}
\end{attention}
...
堆排序分为两步：① 建初堆（$O(n)$）；② 反复输出堆顶，将堆尾元素移至堆顶后向下调整（$n-1$ 次，每次 $O(\log n)$）。
...
void HeapSort(int A[], int n) {
    ...
    for (int i = n - 1; i > 0; i--) {
        ...
        HeapAdjust(A, 0, i);             // ② 调整剩余 i 个元素为新堆
    }
}
\end{lstlisting}
...
希尔排序 & $O(n^{1.3})$ & $O(n^{1.3})$ & $O(n^2)$ & $O(1)$ & 不稳定 \\
\hline
...
\begin{itemize}
    ...
    \item 快速排序趟数与初始序列有关，取决于递归树深度（最好 $\approx \log_2 n$，最坏 $n$）。
...
\begin{itemize}
    ...
    \item \textbf{重建}：输出胜者后，从对应叶结点到根路径上重赛，每次只需比较兄弟结点（或当前值与父结点值）。每选一个最小元的代价为 $O(\log k)$。
    \item 与 \textbf{胜者树（Winner Tree）} 的区别：败者树中兄弟结点比较后「失败者」留存在父结点，重建时只需与路径上的父结点（保存的失败者）比较即可，比胜者树少访问兄弟结点，效率略高。
\end{itemize}

\textbf{败者树的比较次数：} 每趟归并共比较 $(n-1)\lceil\log_2 k\rceil$ 次（$n$ 为记录总数），与 $k$ 无关的调整只需 $(n-1)\times 1$ 次关键比较（败者树的优势）。
\end{conclusion}
...
\item \textbf{（真题风格，单项选择题）} 对序列 $\{45,38,66,90,88,10,25,45'\}$ 以第一个元素 45 为枢轴进行一趟快速排序（Partition），得到的序列是
\begin{center}
\begin{tabular}{ll}
