...
% 下标
\foreach \nn in {0,...,9}
  \node[font=\scriptsize,black!60] at (\nn*1.0-4.5,0.7) {\nn};
% 数值（按 d=5 子序列分组着色）
\node[draw,rectangle,minimum width=0.9cm,minimum height=0.6cm,fill=Teal!18] (g0) at (0*1.0-4.5,0) {49};
\node[draw,rectangle,minimum width=0.9cm,minimum height=0.6cm,fill=Amber!18] (g1) at (1*1.0-4.5,0) {38};
\node[draw,rectangle,minimum width=0.9cm,minimum height=0.6cm,fill=Burgundy!18] (g2) at (2*1.0-4.5,0) {65};
\node[draw,rectangle,minimum width=0.9cm,minimum height=0.6cm,fill=Navy!18] (g3) at (3*1.0-4.5,0) {97};
\node[draw,rectangle,minimum width=0.9cm,minimum height=0.6cm,fill=CodeGreen!18] (g4) at (4*1.0-4.5,0) {76};
\node[draw,rectangle,minimum width=0.9cm,minimum height=0.6cm,fill=Teal!18] (g5) at (5*1.0-4.5,0) {13};
\node[draw,rectangle,minimum width=0.9cm,minimum height=0.6cm,fill=Amber!18] (g6) at (6*1.0-4.5,0) {27};
\node[draw,rectangle,minimum width=0.9cm,minimum height=0.6cm,fill=Burgundy!18] (g7) at (7*1.0-4.5,0) {$49'$};
\node[draw,rectangle,minimum width=0.9cm,minimum height=0.6cm,fill=Navy!18] (g8) at (8*1.0-4.5,0) {55};
\node[draw,rectangle,minimum width=0.9cm,minimum height=0.6cm,fill=CodeGreen!18] (g9) at (9*1.0-4.5,0) {4};
% 组内对应（下方弧线，同色为一组）
\draw[Teal] (g0.south) .. controls +(0,-0.9) and +(0,-0.9) .. (g5.south);
\draw[Amber] (g1.south) .. controls +(0,-0.9) and +(0,-0.9) .. (g6.south);
\draw[Burgundy] (g2.south) .. controls +(0,-0.9) and +(0,-0.9) .. (g7.south);
\draw[Navy] (g3.south) .. controls +(0,-0.9) and +(0,-0.9) .. (g8.south);
\draw[CodeGreen] (g4.south) .. controls +(0,-0.9) and +(0,-0.9) .. (g9.south);
\node[align=center,font=\scriptsize,black!70] at (0,-2.2){希尔排序 $d=5$：下标相差 $5$ 的元素（同色）构成同一子序列，各子序列分别做直接插入排序\\第一趟结果为 $\{13,27,49',55,4,49,38,65,97,76\}$——$49$ 与 $49'$ 次序改变（\textbf{不稳定}）};
\end{tikzpicture}
\end{center}
...
\textbf{划分过程演示（挖坑法，pivot = A[low] = 49）：}
\begin{center}
\begin{tikzpicture}[>=stealth, font=\scriptsize]
  % 挖坑法一趟划分：6 行数组状态（pivot=49）。
  % 行0（初始）8 格
  \foreach \i/\v in {0/49,1/38,2/65,3/97,4/76,5/13,6/27,7/49'}{
    \node[draw,rectangle,minimum width=0.66cm,minimum height=0.48cm,inner sep=0pt,fill=SoftGray] at (\i*0.7-2.5,2.75) {$\v$};
  }
  % 行1：0=27(移入), 1=38,2=65,3=97,4=76,5=13,6=27(坑),7=49'
  \foreach \i/\v in {0/27,1/38,2/65,3/97,4/76,5/13,6/27,7/49'}{
    \ifnum\i=0\relax
      \node[draw,rectangle,minimum width=0.66cm,minimum height=0.48cm,inner sep=0pt,fill=Amber!55] at (\i*0.7-2.5,1.6) {$\v$};
    \else
      \node[draw,rectangle,minimum width=0.66cm,minimum height=0.48cm,inner sep=0pt,fill=SoftGray] at (\i*0.7-2.5,1.6) {$\v$};
    \fi
  }
  % 行2：0=27,1=38,2=65,3=97,4=76,5=13,6=65(移入),7=49'
  \foreach \i/\v in {0/27,1/38,2/65,3/97,4/76,5/13,6/65,7/49'}{
    \ifnum\i=6\relax
      \node[draw,rectangle,minimum width=0.66cm,minimum height=0.48cm,inner sep=0pt,fill=Amber!55] at (\i*0.7-2.5,0.45) {$\v$};
    \else
      \node[draw,rectangle,minimum width=0.66cm,minimum height=0.48cm,inner sep=0pt,fill=SoftGray] at (\i*0.7-2.5,0.45) {$\v$};
    \fi
  }
  % 行3：0=27,1=38,2=13(移入),3=97,4=76,5=13,6=65,7=49'
  \foreach \i/\v in {0/27,1/38,2/13,3/97,4/76,5/13,6/65,7/49'}{
    \ifnum\i=2\relax
      \node[draw,rectangle,minimum width=0.66cm,minimum height=0.48cm,inner sep=0pt,fill=Amber!55] at (\i*0.7-2.5,-0.7) {$\v$};
    \else
      \node[draw,rectangle,minimum width=0.66cm,minimum height=0.48cm,inner sep=0pt,fill=SoftGray] at (\i*0.7-2.5,-0.7) {$\v$};
    \fi
  }
  % 行4：0=27,1=38,2=13,3=97,4=76,5=97(移入),6=65,7=49'
  \foreach \i/\v in {0/27,1/38,2/13,3/97,4/76,5/97,6/65,7/49'}{
    \ifnum\i=5\relax
      \node[draw,rectangle,minimum width=0.66cm,minimum height=0.48cm,inner sep=0pt,fill=Amber!55] at (\i*0.7-2.5,-1.85) {$\v$};
    \else
      \node[draw,rectangle,minimum width=0.66cm,minimum height=0.48cm,inner sep=0pt,fill=SoftGray] at (\i*0.7-2.5,-1.85) {$\v$};
    \fi
  }
  % 行5（落位）：0=27,1=38,2=13,3=49(落位),4=76,5=97,6=65,7=49'
  \foreach \i/\v in {0/27,1/38,2/13,3/49,4/76,5/97,6/65,7/49'}{
    \ifnum\i=3\relax
      \node[draw,rectangle,minimum width=0.66cm,minimum height=0.48cm,inner sep=0pt,fill=Amber!80] at (\i*0.7-2.5,-3.0) {$\v$};
    \else
      \node[draw,rectangle,minimum width=0.66cm,minimum height=0.48cm,inner sep=0pt,fill=SoftGray] at (\i*0.7-2.5,-3.0) {$\v$};
    \fi
  }
  % 下标
  \foreach \i in {0,1,2,3,4,5,6,7}{
    \node[font=\scriptsize,black!60] at (\i*0.7-2.5,3.15) {\i};
  }
  \node[font=\scriptsize,black!60] at (-2.9,3.15) {\texttt{idx}};
  % 指针：行0 low0 high7
  \draw[Teal,->] (0*0.7-2.5,2.45) -- (0*0.7-2.5,2.65); \node[Teal,font=\scriptsize] at (0*0.7-2.5,2.2) {low};
  \draw[red,->] (7*0.7-2.5,2.45) -- (7*0.7-2.5,2.65); \node[red,font=\scriptsize] at (7*0.7-2.5,2.2) {high};
  % 行1：low0 high6
  \draw[Teal,->] (0*0.7-2.5,1.3) -- (0*0.7-2.5,1.5); \node[Teal,font=\scriptsize] at (0*0.7-2.5,1.05) {low};
  \draw[red,->] (6*0.7-2.5,1.3) -- (6*0.7-2.5,1.5); \node[red,font=\scriptsize] at (6*0.7-2.5,1.05) {high};
  % 行2：low2 high6
  \draw[Teal,->] (2*0.7-2.5,0.15) -- (2*0.7-2.5,0.35); \node[Teal,font=\scriptsize] at (2*0.7-2.5,-0.1) {low};
  \draw[red,->] (6*0.7-2.5,0.15) -- (6*0.7-2.5,0.35); \node[red,font=\scriptsize] at (6*0.7-2.5,-0.1) {high};
  % 行3：low2 high5
  \draw[Teal,->] (2*0.7-2.5,-1.0) -- (2*0.7-2.5,-0.8); \node[Teal,font=\scriptsize] at (2*0.7-2.5,-1.25) {low};
  \draw[red,->] (5*0.7-2.5,-1.0) -- (5*0.7-2.5,-0.8); \node[red,font=\scriptsize] at (5*0.7-2.5,-1.25) {high};
  % 行4：low3 high5
  \draw[Teal,->] (3*0.7-2.5,-2.15) -- (3*0.7-2.5,-1.95); \node[Teal,font=\scriptsize] at (3*0.7-2.5,-2.4) {low};
  \draw[red,->] (5*0.7-2.5,-2.15) -- (5*0.7-2.5,-1.95); \node[red,font=\scriptsize] at (5*0.7-2.5,-2.4) {high};
  % 行5：low=high=3（相遇）
  \draw[Teal,->] (3*0.7-2.5,-3.3) -- (3*0.7-2.5,-3.1); \node[Teal,font=\scriptsize] at (3*0.7-2.5,-3.55) {low=high};
  % 右侧步骤说明（与各行 y 对齐）
  \node[align=left,font=\scriptsize,black!70,anchor=west] at (3.3,2.75) {\textbf{初始}：pivot$=$49，low$=$0，high$=$7};
  \node[align=left,font=\scriptsize,black!70,anchor=west] at (3.3,1.6) {\textbf{右找小}：A[high]$=27<49$，填入左坑};
  \node[align=left,font=\scriptsize,black!70,anchor=west] at (3.3,0.45) {\textbf{左找大}：A[low]$=65>49$，填入右坑};
  \node[align=left,font=\scriptsize,black!70,anchor=west] at (3.3,-0.7) {\textbf{右找小}：A[high]$=13<49$，填入左坑};
  \node[align=left,font=\scriptsize,black!70,anchor=west] at (3.3,-1.85) {\textbf{左找大}：A[low]$=97>49$，填入右坑};
  \node[align=left,font=\scriptsize,black!70,anchor=west] at (3.3,-3.0) {\textbf{相遇}：low$=$high$=$3，枢轴 49 落位};
\end{tikzpicture}
\end{center}
\end{formula}
...
\begin{tikzpicture}[>=stealth, font=\scriptsize]
% 完全二叉树
\node[draw,circle,minimum size=0.7cm,fill=Teal!15] (n0) at (0,2.2) {4};
\node[draw,circle,minimum size=0.7cm,fill=Teal!15] (n1) at (-1.9,0.8) {10};
\node[draw,circle,minimum size=0.7cm,fill=Teal!15] (n2) at (1.9,0.8) {3};
\node[draw,circle,minimum size=0.7cm,fill=Teal!15] (n3) at (-2.9,-0.6) {5};
\node[draw,circle,minimum size=0.7cm,fill=Teal!15] (n4) at (-0.9,-0.6) {1};
\node[draw,circle,minimum size=0.7cm,fill=Teal!15] (n5) at (1.0,-0.6) {2};
\draw (n0) -- (n1);
\draw (n0) -- (n2);
\draw (n1) -- (n3);
\draw (n1) -- (n4);
\draw (n2) -- (n5);
% 数组下标
\node[font=\scriptsize,black!60] at (0,-1.5){下标};
\foreach \nn in {0,...,5} {
  \node[font=\scriptsize,black!60] (idx\nn) at (\nn*1.0-2.5,-1.9) {\nn};
}
% 数组值
\node[draw,rectangle,minimum width=0.85cm,minimum height=0.6cm,fill=SoftGray] (v0) at (0*1.0-2.5,-2.6) {4};
\node[draw,rectangle,minimum width=0.85cm,minimum height=0.6cm,fill=SoftGray] (v1) at (1*1.0-2.5,-2.6) {10};
\node[draw,rectangle,minimum width=0.85cm,minimum height=0.6cm,fill=SoftGray] (v2) at (2*1.0-2.5,-2.6) {3};
\node[draw,rectangle,minimum width=0.85cm,minimum height=0.6cm,fill=SoftGray] (v3) at (3*1.0-2.5,-2.6) {5};
\node[draw,rectangle,minimum width=0.85cm,minimum height=0.6cm,fill=SoftGray] (v4) at (4*1.0-2.5,-2.6) {1};
\node[draw,rectangle,minimum width=0.85cm,minimum height=0.6cm,fill=SoftGray] (v5) at (5*1.0-2.5,-2.6) {2};
\node[align=center,font=\scriptsize,black!70] at (0,-3.5){完全二叉树的顺序存储：数组 $A[0..5]$ 与树一一对应\\下标 $i$ 的左孩子为 $2i+1$、右孩子为 $2i+2$（如 $A[1]=10$ 的孩子为 $A[3]=5$、$A[4]=1$）};
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}[>=stealth, font=\scriptsize]
% 第 0 层：单元素段
\foreach \nn/\val in {0/49,1/38,2/65,3/97,4/76,5/13,6/27}
  \node[draw,rectangle,fill=Amber!25,minimum height=0.5cm] (s\nn) at (\nn*1.0-3.0,0) {\val};
% 第 1 趟
\node[draw,rectangle,fill=Teal!25,minimum height=0.5cm] (t10) at (-2.5,-1.6) {38,49};
\node[draw,rectangle,fill=Teal!25,minimum height=0.5cm] (t11) at (-0.5,-1.6) {65,97};
\node[draw,rectangle,fill=Teal!25,minimum height=0.5cm] (t12) at (1.5,-1.6) {13,76};
\node[draw,rectangle,fill=Teal!25,minimum height=0.5cm] (t13) at (3.0,-1.6) {27};
\draw[->] (s0) -- (t10);
\draw[->] (s1) -- (t10);
\draw[->] (s2) -- (t11);
\draw[->] (s3) -- (t11);
...
\draw[->] (s6) -- (t13);
% 第 2 趟
\node[draw,rectangle,fill=Navy!20,minimum height=0.5cm] (t20) at (-1.5,-3.2) {38,49,65,97};
\node[draw,rectangle,fill=Navy!20,minimum height=0.5cm] (t21) at (2.25,-3.2) {13,27,76};
\draw[->] (t10) -- (t20);
\draw[->] (t11) -- (t20);
\draw[->] (t12) -- (t21);
\draw[->] (t13) -- (t21);
% 第 3 趟
\node[draw,rectangle,fill=Burgundy!22,minimum height=0.5cm] (t30) at (0.4,-4.8) {13,27,38,49,65,76,97};
\draw[->] (t20) -- (t30);
\draw[->] (t21) -- (t30);
\node[align=center,font=\scriptsize,black!70] at (0.4,-5.6){二路归并排序（段长 $1\to2\to4\to8$）：共 $\lceil\log_2 7\rceil=3$ 趟得到有序序列};
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}[>=stealth, font=\scriptsize]
  % 初始归并段
  \node[draw,rectangle,fill=Amber!25,minimum width=1.1cm,minimum height=0.6cm] (r1) at (0,0){$R_1$};
  \node[draw,rectangle,fill=Amber!25,minimum width=1.1cm,minimum height=0.6cm] (r2) at (2,0){$R_2$};
  \node[draw,rectangle,fill=Amber!25,minimum width=1.1cm,minimum height=0.6cm] (r3) at (4,0){$R_3$};
  \node[draw,rectangle,fill=Amber!25,minimum width=1.1cm,minimum height=0.6cm] (r4) at (6,0){$R_4$};
  % 第一趟
  \node[draw,rectangle,fill=Teal!25,minimum width=1.4cm,minimum height=0.6cm] (m1) at (1,-2){$R_{12}$};
  \node[draw,rectangle,fill=Teal!25,minimum width=1.4cm,minimum height=0.6cm] (m2) at (5,-2){$R_{34}$};
  % 第二趟
  \node[draw,rectangle,fill=Navy!18,minimum width=1.8cm,minimum height=0.6cm] (fin) at (3,-4){$R_{1234}$};
  % 箭头
  \draw[->] (r1.south) -- (m1.north);
  \draw[->] (r2.south) -- (m1.north);
  \draw[->] (r3.south) -- (m2.north);
  \draw[->] (r4.south) -- (m2.north);
  \draw[->] (m1.south) -- (fin.north);
  \draw[->] (m2.south) -- (fin.north);
  % 阶段标注
  \node[font=\scriptsize,black!70] at (3,1.2){初始归并段（内部排序生成）};
  \node[font=\scriptsize,black!70] at (3,-1.1){第 1 趟：二路归并};
  \node[font=\scriptsize,black!70] at (3,-3.0){第 2 趟：二路归并};
  \node[font=\scriptsize,black!70] at (3,-4.6){有序文件（趟数 $\lceil\log_2 4\rceil=2$）};
\end{tikzpicture}
\end{center}
...
\ifshowsolutions\par\noindent\textbf{解析：} 直接插入、冒泡和采用“相等时先取左段”的归并排序均可稳定；快速排序的远距离交换可能改变相等关键字的相对次序。\boxed{\textbf{答案：}(D)}\else \answerarea \fi
...
\ifshowsolutions\par\noindent\textbf{解析：} 在枢轴位置近似均匀的平均模型下，递推为 $T(n)=T(k)+T(n-k-1)+O(n)$，期望为 $O(n\log n)$。\boxed{\textbf{答案：}(B)}\else \answerarea \fi
...
\ifshowsolutions\par\noindent\textbf{解析：} 自底向上建堆时，大量靠近叶子的结点只需很少调整，所有下沉距离之和为 $O(n)$。\boxed{\textbf{答案：}(B)}\else \answerarea \fi
...
\ifshowsolutions\par\noindent\textbf{解析：} 直接插入排序的移动和比较次数与逆序程度相关，序列基本有序时接近 $O(n)$；选择排序和堆排序不能利用这一性质，固定首元素枢轴的快排还可能退化。\boxed{\textbf{答案：}(B)}\else \answerarea \fi

\item \textbf{（填空题）} 基于比较的排序算法在最坏情况下的时间下界为 \underline{\qquad\qquad}（用大 $O$ 或大 $\Omega$ 记法）。

\ifshowsolutions\par\noindent\textbf{答案：} $\Omega(n\log n)$。比较决策树至少有 $n!$ 个叶结点，其高度不小于 $\lceil\log_2(n!)\rceil=\Omega(n\log n)$。\else \answerarea \fi

\item \textbf{（填空题）} 希尔排序的最后一趟增量必须为 \underline{\qquad\qquad}。

\ifshowsolutions\par\noindent\textbf{答案：} 1。最后必须对整个序列执行一次普通插入排序式的相邻间隔整理，才能保证全局有序。\else \answerarea \fi

\item \textbf{（填空题）} 对序列 $\{6,5,3,1,8,7,2,4\}$ 进行二路归并排序，第一趟归并后的序列为 \underline{\qquad\qquad}。

\ifshowsolutions\par\noindent\textbf{答案：} $\{5,6,1,3,7,8,2,4\}$。第一趟把相邻的单元素段两两归并。\else \answerarea \fi
...
\]
因此自底向上建堆是 $O(n)$，而逐个插入建堆才通常按 $O(n\log n)$ 上界估计。\else \answerarea \fi
...
\end{lstlisting}
最坏时间复杂度 $O(n^2)$，最好 $O(n)$，辅助空间 $O(1)$；使用“$\leqslant$”越过已有相等结点可保持稳定性。\else \answerarea \fi
...
\end{lstlisting}
每个元素只复制一次，时间复杂度 $O(m+n)$；除输出数组外辅助空间为 $O(1)$。相等时先取 A 可保持跨段稳定性。\else \answerarea \fi
...
\ifshowsolutions\par\noindent\textbf{解析：} 按指定填坑法，右侧先找到25填入首位置，左侧再找到66移到右侧空位；随后10移到左侧，最终两指针在下标3相遇并放回枢轴，得到 $\{25,38,10,45,88,90,66,45'\}$。\boxed{\textbf{答案：}(B)}\else \answerarea \fi
...
\ifshowsolutions\par\noindent\textbf{解析：} 自最后一个非叶结点向根调整：先把15与6交换，再把10与8交换，最后把根12与较小孩子6交换，得到 $\{6,8,12,10,20,15,25,18\}$。\boxed{\textbf{答案：}(A)}\else \answerarea \fi
...
\ifshowsolutions\par\noindent\textbf{解析：} 堆排序不稳定；建堆为 $O(n)$；使用大根堆升序排序时，每趟把当前最大元素放到末尾。其最好、平均和最坏时间均为 $O(n\log n)$。\boxed{\textbf{答案：}(B)}\else \answerarea \fi
...
(4) 在这组特定数据和这次执行中，两种算法都恰好保持了 $49$ 在 $49'$ 之前；但“某一次没有改变”不能证明算法稳定。归并时相等关键字总是先取左段，因而可保证稳定；快速排序的跨区移动在其他输入上可能改变相等元素次序，所以快速排序仍是不稳定算法。\else \answerarea \fi
...
\]
若以“记录被读一次或写一次”为一次 I/O 单位，每层归并对参与记录各读、写一次，总记录 I/O 量为 $2\times\mathrm{WPL}=426$；若磁盘按块计数，还需再结合每块记录数换算。败者树建立后，每输出一个最小记录只需沿树调整 $O(\log k)$ 次比较，比逐次线性扫描 $k$ 个段的 $O(k)$ 比较更适合较大的 $k$。\else \answerarea \fi
...
(3) 选堆排序：额外空间 $O(1)$，最坏时间保证 $O(n\log n)$；它不稳定，但题设没有要求稳定性。\else \answerarea \fi
