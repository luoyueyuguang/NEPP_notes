...
\begin{tikzpicture}[>=stealth, font=\scriptsize, every node/.style={draw, circle, minimum size=5mm, inner sep=1pt}]
  ...
  \draw[->] (B) -- (D); \draw[->] (B) -- (E);
  \draw[->] (C) -- (F); \draw[->] (C) -- (G);
  \node[font=\scriptsize, Navy] at (0,-2.85){满二叉树：每一层结点数都达到最大（$n=2^3-1=7$）};
\end{tikzpicture}
\hspace{1.1cm}
\begin{tikzpicture}[>=stealth, font=\scriptsize, every node/.style={draw, circle, minimum size=5mm, inner sep=1pt}]
  \node[fill=SoftTeal] (A) at (0,0){A};
  ...
  \draw[->] (B) -- (D); \draw[->] (B) -- (E);
  \draw[->] (C) -- (F);
  \node[font=\scriptsize, Navy] at (0,-2.85){完全二叉树：最后一层从左到右连续（$n=6$）};
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}[>=stealth, font=\scriptsize, every node/.style={draw, circle, minimum size=5mm, inner sep=1pt}]
  ...
  \node[fill=SoftBlue] (C) at (1.2,-1){C};
  \draw[->] (A) -- (B); \draw[->] (A) -- (C);
  \node[font=\scriptsize, black!70] at (0,-1.95){先序（根左右）：A B C\quad 中序（左根右）：B A C\quad 后序（左右根）：B C A};
\end{tikzpicture}
\end{center}
\end{formula}
