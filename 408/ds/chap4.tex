...
后序遍历（左→右→根）：\texttt{DEBFGCA}。\boxed{\textbf{答案：}(A)}\fi
...
\ifshowsolutions\par\noindent\textbf{解析：} 先序 = 根→左→右，后序 = 左→右→根。二者相反，意味着对每个结点，先访问根再访问子树 vs 先访问子树再访问根。
...
\ifshowsolutions\par\noindent\textbf{解析：} 森林→二叉树的转换规则：第一棵树的根 = 二叉树根 $p$，第一棵树中除根以外的结点 → $p$ 的左子树；其余森林中的树 → $p$ 的右子树。
...
右子树先序 \texttt{CF}，中序 \texttt{CF}：C 根，F 右（中序 C 前为空→无左子树，后为 F→右子树）。
...
(2) 后序遍历（左→右→根）：\texttt{DGEBFCA}。层次遍历（从上到下，从左到右）：\texttt{ABCDEFG}。
(3) 二叉树→森林：抹去右子树的父子关系（二叉树中根结点沿右链走到底，每遇到一个右孩子即断开），得到多棵树。规则：二叉树中，根和沿左链走到的结点为第一棵树中的父子关系，沿右链走到的结点为下一棵树的根。

断开：A 右 C，断开得到两棵树。第一棵树（A 为根，左子树 B）：先恢复为一般树——每个结点保留左孩子，左孩子的右链变为该结点的其他孩子。B 的左 D→B 的第一个孩子 D；D 的右 E→B 的第二个孩子 E；E 的左 G→E 的第一个孩子 G。森林第一棵树：A-B(D,E(G))。第二棵树：C(F)。
