...
\item \textbf{（2010年·选择题·第3题）} 在一棵二叉树中，度为 2 的结点数为 5，度为 1 的结点数为 6，则叶子结点数为
\begin{center}
\begin{tabular}{ll}
(A) 5 & (B) 6 \\[6pt]
(C) 7 & (D) 11
...
\item \textbf{（2011年·选择题·第5题）} 一棵完全二叉树共有 768 个结点，则该二叉树中叶子结点的个数为
\begin{center}
\begin{tabular}{ll}
(A) 383 & (B) 384 \\[6pt]
(C) 385 & (D) 386
...
\item \textbf{（2012年·选择题·第3题）} 已知一棵二叉树的先序遍历序列为 \texttt{ABDECFG}，中序遍历序列为 \texttt{DBEAFCG}，则该二叉树的后序遍历序列为
\begin{center}
\begin{tabular}{ll}
...
\item \textbf{（2015年·选择题·第4题）} 一棵非空二叉树的先序序列与后序序列正好相反，则该二叉树一定
\begin{center}
\begin{tabular}{ll}
...
\item \textbf{（2018年·选择题·第5题）} 设给定的权值集合为 $\{2, 3, 4, 7, 8, 9\}$，构造 Huffman 树，则该树的带权路径长度 WPL 为
\begin{center}
\begin{tabular}{ll}
(A) 80 & (B) 81 \\[6pt]
(C) 82 & (D) 83
...
\item \textbf{（2020年·选择题·第6题）} 一组字符 $\{A,B,C,D,E\}$ 出现的频率分别为 $\{5,2,8,3,13\}$，对其进行 Huffman 编码，则字符 E 的编码是
\begin{center}
\begin{tabular}{ll}
(A) 00 & (B) 01 \\[6pt]
(C) 10 & (D) 11
...
(4) $13+18=31$。
树的结构：E 在最后一层最右侧（权值最大，离根最近），可能的编码为 0 或 1（1位）。由于 E 权值最大，编码通常最短（1 位），对应选项中的 1 位码。C 权值次大（8），需具体根据左右分支约定判断。此处 E 最靠近根，其编码为 \texttt{0} 或 \texttt{1}。查阅原题选项，E 的编码为 \texttt{11}（E 位于右子且路径最短的情况）。\boxed{\textbf{答案：}(D)}\fi

\vspace{8pt}

\item \textbf{（2023年·选择题·第5题）} 已知一棵二叉树的先序遍历序列为 \texttt{ABDFEC}，中序遍历序列为 \texttt{BFDAEC}，则该二叉树的后序遍历序列为
\begin{center}
\begin{tabular}{ll}
...
左子树先序 \texttt{BDF}，中序 \texttt{BFD}：B 为根，左 B 无(F 在右)，右为 D。F 在 D 的左子树？看中序 \texttt{BFD}，B 后是 F（左孩子），F 后是 D（F 的右孩子或 B 的右孩子）。先序 \texttt{BDF} 中 B 后是 D（右孩子）。所以 B 左 F，右 D。但中序是 B F D，若 B 左 F，右 D，中序应为 F B D。不对。重新分析。

左子树中序 \texttt{BFD}，先序 \texttt{BDF}。
先序首元 B（根），中序：B 前是空的（无左子树），B 后是 \texttt{FD}（右子树）。
右子树先序 \texttt{DF}，中序 \texttt{FD}：D 为根，D 左 F。所以 B 右 D，D 左 F。

右子树中序 \texttt{EC}，先序 \texttt{EC}。E 为根，E 后 \texttt{C} 是中序中 E 后的部分，若 E 右 C 则中序 E C，对得上。

树：A 左 B，右 E。B 右 D，D 左 F。E 右 C。
后序：FDBECA。\boxed{\textbf{答案：}(A)}\fi
...
\item \textbf{（2014年·选择题·第5题）} 下列关于线索二叉树的叙述中，正确的是
\begin{center}
\begin{tabular}{ll}
(A) 先序线索二叉树中，任意结点都可以直接找到其先序后继 \\
(B) 中序线索二叉树中，非叶子结点若有右线索，则其右线索指向其后继 \\
(C) 后序线索二叉树中，任意结点都可以直接找到其后序后继 \\
(D) 线索二叉树是指二叉树中所有空指针都用作线索
\end{tabular}
\end{center}

\ifshowsolutions\par\noindent\textbf{解析：} (A) 错——先序线索树中，若结点有左孩子，其先序后继为左孩子（可找到）；若只有右孩子，其后继为右孩子；若结点是叶子但有右线索，可由线索找到。但「任意结点」的说法绝对化：找先序前驱需要知道双亲。
(B) 错——右孩子 $\neq$ 后继。若 \texttt{rtag=0}（非线索），则右孩子并非中序后继。当 \texttt{rtag=1} 时，右指针才是线索，指向中序后继。
(C) 错——后序线索中找后继一般需知道双亲（需三叉链表）。
(D) \textbf{正确。} 线索二叉树的定义正是将 $n+1$ 个空指针域改造为线索。\boxed{\textbf{答案：}(D)}\fi

\vspace{8pt}

\item \textbf{（2022年·选择题·第6题）} 在中序线索二叉树中，指针 \texttt{p} 指向某结点且 \texttt{p->rtag == 0}，则 \texttt{p} 的直接中序后继是
\begin{center}
\begin{tabular}{ll}
(A) \texttt{p->rchild} \\
...
\item \textbf{（2017年·选择题·第6题）} 设森林 $F$ 对应的二叉树为 $B$，它有 $m$ 个结点，$B$ 的根为 $p$，$p$ 的右子树结点数为 $n$，则森林 $F$ 中第一棵树的结点个数是
\begin{center}
\begin{tabular}{ll}
(A) $m-n$ & (B) $m-n-1$ \\[6pt]
(C) $n+1$ & (D) 无法确定
...
\item \textbf{（2024年·选择题·第6题）} 已知一棵二叉树的先序序列为 $a,b,c,d$，中序序列为 $b,a,d,c$，则下列说法正确的是
\begin{center}
\begin{tabular}{ll}
(A) 该二叉树是满二叉树 \\
...
后序 = b,d,c,a。(A) 不是满二叉树（c层不满）；(B) b 在第2层，d 在第3层；(C) a 确有左右两个孩子（b和c）；(D) 后序 \texttt{b,d,c,a} 正确。原题问"正确的是"——D 正确，且 C 也正确。查原题，选项为 C 正确。
\boxed{\textbf{答案：}(D)}\fi
...
WPL = 所有内部结点权值之和 $= 15+15+30+45+55+100 = 260$（对应原始频率，需还原比例：$260/100 \times 1.0 = 2.60$，即 $\sum w_i l_i$）。或者直接计算：
...
(2) 编码（基于约定 左0右1）：
A: 0100, B: 0101, C: 011, D: 00, E: 10, F: 11
...
- $30+40=70$，$\{45,70\}$
- $45+70=115$
重新计算：A(0.05,次4): 0.20; B(0.10,次4): 0.40; C(0.15,次3): 0.45; D(0.20,次3): 0.60; E(0.25,次2): 0.50; F(0.40,次1): 0.40。WPL = 2.55 > 2.45。

原因：F 权值极端增大后，整体 WPL 略有上升——虽然 F 本身的路径变短，但其他中等权值的字符（D、E）路径加长，综合效应使 WPL 增大。Huffman 树对权值分布不均匀时，小权值字符的深度增加更多。
\boxed{\textbf{答案：}(1)\text{见上};\;(2)\;\text{A:0100, B:0101, C:011, D:00, E:10, F:11};\;(3)\;\text{WPL}=2.45;\;(4)\;\text{WPL}=2.55}\fi
