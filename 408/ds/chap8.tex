...
\textbf{双栈法（最简单的实现思路）：}先用「根→右→左」的顺序入栈，再反转输出即得「左→右→根」的后序。
或者用单栈 + 标记法：
...
bool DFSCycle(ALGraph *G, int v, int parent, bool visited[]) {
    ...
    for (ArcNode *p = G->adjlist[v].first; p; p = p->next) {
        ...
        } else if (w != parent) {    // 已访问且不是父结点 → 回边 → 有环
            return true;
        }
    }
    return false;
...
bool DFSCycleD(ALGraph *G, int v) {
    ...
    for (ArcNode *p = G->adjlist[v].first; p; p = p->next) {
        int w = p->adjvex;
        if (color[w] == GRAY) return true;   // 回到当前路径上 → 环
