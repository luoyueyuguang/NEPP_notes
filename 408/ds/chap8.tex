...
\ifshowsolutions\par\noindent\textbf{解析：} 长度为 $2k$ 时，快指针走完 $2k$ 个结点后变为 \texttt{NULL}，慢指针恰好前进 $k$ 步，指向第 $k+1$ 个结点，即后半段的第一个结点。\boxed{\textbf{答案：}(B)}\fi
...
\ifshowsolutions\par\noindent\textbf{解析：} 经典 Floyd 算法的步长为慢指针1、快指针2，环内相对步长为1，故必能相遇。即使快指针改为每次3步，环内相对步长为2；对任意有限环长 $\lambda$，总能取到满足 $2t\equiv0\pmod\lambda$ 的时刻，因此检测阶段仍会相遇。不过，经典入口定位依赖相遇点满足 $a=k\lambda-b$ 的 1:2 关系；改成1:3后，直接重置并各走一步不一定落在入口。因此错误的是 (C)。\boxed{\textbf{答案：}(C)}\fi
...
\ifshowsolutions\par\noindent\textbf{解析：} 三色 DFS 中遇到指向 GRAY 结点的边说明存在回边；拓扑排序无法处理完全部顶点也说明有环。普通并查集适合无向图判环，不能直接判断一般有向图中的环。\boxed{\textbf{答案：}(C)}\fi
...
\ifshowsolutions\par\noindent\textbf{答案：} 逆序对；$O(n\log n)$。左右两段已经有序，右段当前元素一旦小于左段当前元素，就同时小于左段全部剩余元素。\fi
...
\ifshowsolutions\par\noindent\textbf{答案：} 无右孩子；非完全二叉树。若结点只有左孩子，它必须位于最后一个非叶位置；若它连左孩子也没有，则它本身已是叶结点。两种情况下后续层序结点都必须是叶结点。\fi

\item \textbf{（简答题）} 简述用非递归中序遍历寻找二叉排序树中第 $k$ 小元素的基本思路，并说明时间复杂度。

\ifshowsolutions\par\noindent\textbf{答案：} 使用栈模拟中序遍历：沿左链依次入栈，弹出一个结点时将计数加1；计数首次等于 $k$ 时，该结点就是第 $k$ 小元素，然后转向其右子树继续同样过程。若在访问完 $k$ 个结点前栈已空，则 $k$ 越界。时间复杂度为 $O(h+k)$，最坏为 $O(n)$；辅助空间为 $O(h)$，其中 $h$ 为树高。\fi

\item \textbf{（简答题）} 说明为什么 Boyer-Moore 多数投票算法需要「验证阶段」（即第二次遍历确认候选元素的出现次数）。如果不验证，是否会出错？请给出反例。

\ifshowsolutions\par\noindent\textbf{答案：} 抵消过程只保证“若主元素存在，最终候选一定是它”，并不保证任意输入都存在主元素。例如数组 $[1,2,3]$ 最终可能留下候选3，但任何元素的出现次数都没有超过 $3/2$。因此必须第二次遍历统计候选出现次数，只有大于 $\lfloor n/2\rfloor$ 才能返回。\fi
...
\end{lstlisting}
每个结点只处理一次，时间复杂度 $O(n)$；只使用常数个指针，辅助空间 $O(1)$。尾插保证奇、偶两组内部均稳定。\fi
...
\ifshowsolutions\par\noindent\textbf{解析：} 完全二叉树有 $n_0=\lceil n/2\rceil$ 个叶结点，故 $n_0=2024/2=1012$。\boxed{\textbf{答案：}(B)}\fi
...
\ifshowsolutions\par\noindent\textbf{解析：} 序列严格递减时，每一对下标 $i<j$ 都构成逆序对，共有 $\binom n2=n(n-1)/2$ 对。\boxed{\textbf{答案：}(C)}\fi
...
\end{lstlisting}
每个结点访问一次，时间复杂度 $O(n)$；递归栈为 $O(h)$，其中 $h$ 为树高。\fi
...
\end{lstlisting}
每轮至少排除当前区间的一半，故时间复杂度为 $O(\log n)$，辅助空间为 $O(1)$。题干要求元素互异且确为严格递增数组的旋转，正是上述二分判据成立的条件。\fi
...
\end{lstlisting}
扫描边和遍历图的总时间为 $O(|V|+|E|)$；入度、访问标记和递归栈/队列占 $O(|V|)$ 辅助空间。\fi
...
\end{lstlisting}
直接在线性中序区间内找根，最坏时间复杂度为 $O(n^2)$；递归栈为 $O(h)$，另需 $O(n)$ 新结点空间。若预先用散列表建立“关键字$\to$中序下标”的映射，构造时间可降为平均 $O(n)$，额外映射空间为 $O(n)$。\fi
...
\end{lstlisting}
随机输入或随机选择枢轴时，期望工作量满足 $n+n/2+n/4+\cdots=O(n)$；若每次枢轴都极端不平衡，最坏为 $O(n^2)$。迭代实现的辅助空间为 $O(1)$，但会原地改变数组元素次序。\fi
...
\end{lstlisting}
找中点、逆置、比较和恢复均为线性过程，总时间 $O(n)$，辅助空间 $O(1)$。\fi
