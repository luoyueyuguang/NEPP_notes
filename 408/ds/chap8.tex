...
本章是数据结构课程的综合篇，将前面各章（线性表、树、图、查找、排序）中的算法思想提炼为可复用的设计模式。数据结构综合应用题常以算法阅读、补全或设计为载体，考查的不是死记硬背，而是将基本数据结构和算法思想灵活组合的能力。
...
\node[draw,rectangle,minimum width=0.9cm,minimum height=0.55cm,fill=SoftGray] (hd) at (0,0) {head};
\node[draw,rectangle,minimum width=0.6cm,minimum height=0.55cm] (a) at (1.5,0) {1};
\node[draw,rectangle,minimum width=0.6cm,minimum height=0.55cm] (b) at (2.7,0) {2};
\node[draw,rectangle,minimum width=0.6cm,minimum height=0.55cm,fill=Amber!20] (c) at (3.9,0) {3};
\node[draw,rectangle,minimum width=0.6cm,minimum height=0.55cm] (d) at (5.1,0) {4};
\node[draw,rectangle,minimum width=0.6cm,minimum height=0.55cm,fill=Teal!20] (e) at (6.3,0) {5};
\node[draw,rectangle,minimum width=0.6cm,minimum height=0.55cm] (f) at (7.5,0) {6};
\draw[->] (hd) -- (a);
\draw[->] (a) -- (b);
\draw[->] (b) -- (c);
\draw[->] (c) -- (d);
...
\draw[->] (e) -- (f);
% 环：6 -> 3
\draw[->,Burgundy,thick] (f.north) .. controls (8.2,1.6) and (4.6,1.6) .. (c.north);
% 标注
\node[align=center,font=\scriptsize,black!70] at (3.9,-1.0){(环入口)};
\node[align=center,font=\scriptsize,black!70] at (6.3,-1.0){(slow=fast\\相遇处)};
\node[align=center,font=\scriptsize,black!70] at (3.9,-2.3){快慢指针从 1 同时出发：\texttt{fast} 每步走 2、\texttt{slow} 每步走 1，二者在 5 处相遇\\再将一指针重置回 1，两指针各走 1 步，再次相遇于 3（环入口）};
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}[>=stealth, font=\scriptsize]
% 逆置前
\node[draw,rectangle,minimum width=0.9cm,minimum height=0.55cm,fill=SoftGray] (h1) at (0,0) {head};
\node[draw,rectangle,minimum width=0.55cm,minimum height=0.55cm] (x1) at (1.5,0) {1};
\node[draw,rectangle,minimum width=0.55cm,minimum height=0.55cm] (x2) at (2.7,0) {2};
\node[draw,rectangle,minimum width=0.55cm,minimum height=0.55cm] (x3) at (3.9,0) {3};
\node[draw,rectangle,minimum width=0.4cm,minimum height=0.55cm,fill=SoftGray] (nl) at (5.1,0) {$\wedge$};
\draw[->] (h1) -- (x1);
\draw[->] (x1) -- (x2);
\draw[->] (x2) -- (x3);
\draw[->] (x3) -- (nl);
\node[align=center] at (2.5,-0.9){逆置前：\texttt{head} $\to$ $1\to2\to3\to\wedge$};
% 逆置后
\node[draw,rectangle,minimum width=0.9cm,minimum height=0.55cm,fill=SoftGray] (h2) at (0,-2.2) {head};
\node[draw,rectangle,minimum width=0.55cm,minimum height=0.55cm] (y3) at (1.5,-2.2) {3};
\node[draw,rectangle,minimum width=0.55cm,minimum height=0.55cm] (y2) at (2.7,-2.2) {2};
\node[draw,rectangle,minimum width=0.55cm,minimum height=0.55cm] (y1) at (3.9,-2.2) {1};
\node[draw,rectangle,minimum width=0.4cm,minimum height=0.55cm,fill=SoftGray] (nl2) at (5.1,-2.2) {$\wedge$};
\draw[->] (h2) -- (y3);
\draw[->] (y3) -- (y2);
\draw[->] (y2) -- (y1);
\draw[->] (y1) -- (nl2);
\node[align=center] at (2.5,-3.1){逆置后：\texttt{head} $\to$ $3\to2\to1\to\wedge$};
\end{tikzpicture}
\end{center}
...
\begin{itemize}
    ...
    \item 每 $k$ 个结点一组逆置（分段处理，余下不足 $k$ 个时是否逆置由题设决定）。
\end{itemize}
\end{attention}
...
\begin{center}
\begin{tikzpicture}[>=stealth, font=\scriptsize]
\node[draw,circle,minimum size=0.7cm,fill=Teal!15] (r) at (0,0) {8};
\node[draw,circle,minimum size=0.7cm,fill=Teal!15] (l) at (-2.0,-1.6) {3};
\node[draw,circle,minimum size=0.7cm,fill=Teal!15] (rt) at (2.0,-1.6) {10};
\node[draw,circle,minimum size=0.7cm,fill=Teal!15] (ll) at (-3.0,-3.2) {1};
\node[draw,circle,minimum size=0.7cm,fill=Teal!15] (lr) at (-1.0,-3.2) {6};
\draw (r) -- (l);
\draw (r) -- (rt);
\draw (l) -- (ll);
\draw (l) -- (lr);
% 各子树高度（右上角标注）
\node[font=\scriptsize,black!60] at (0.55,0.35) {h=3};
\node[font=\scriptsize,black!60] at (-1.45,-1.25) {h=2};
\node[font=\scriptsize,black!60] at (2.55,-1.25) {h=1};
\node[font=\scriptsize,black!60] at (-2.45,-2.85) {h=1};
\node[font=\scriptsize,black!60] at (-0.45,-2.85) {h=1};
\node[align=center,font=\scriptsize,black!70] at (0,-4.3){后序递归求高度：$\mathrm{depth}(T)=\max(\mathrm{depth}(L),\mathrm{depth}(R))+1$，空树返回 0\\先求左、右子树高度，再返回二者较大值 $+1$（如根 8 的高度 $=\max(2,1)+1=3$）};
\end{tikzpicture}
\end{center}
...
\textbf{复杂度：}优化方法时间为 $O(n)$，辅助空间为 $O(h)$（递归栈深度，$h$ 为树高）；平衡树时 $h=O(\log n)$，最坏退化树时为 $O(n)$。
...
\begin{enumerate}
    ...
    \item \textbf{常见的二叉树算法训练：}求深度/宽度、判断完全二叉树、判断平衡二叉树、求双分支结点数、交换左右子树。
\end{enumerate}
\end{attention}
...
\textbf{复杂度：}时间 $O(n)$，辅助栈空间为 $O(h)$，其中 $h$ 为树高；退化为链时最坏 $O(n)$，树高为 $O(\log n)$ 时为 $O(\log n)$。
\end{conclusion}
...
\begin{lstlisting}[language=C]
// 通过返回值表示是否存在主元素，*result 输出主元素
bool MajorityElement(int A[], int n, int *result) {
...
\subsection{算法设计题训练覆盖}

\begin{attention}
原按年份、题号和分值排列的索引尚未逐题对照权威原卷，本版不把它作为正式内容发布。下列内容只用于说明算法训练覆盖，不声明某一题对应具体考试年份。
\end{attention}

\begin{conclusion}{训练规律总结}{exam-patterns}
数据结构算法设计题应重点覆盖以下能力：
\begin{enumerate}
    \item \textbf{线性表（数组/链表）：}原地修改、双指针、循环移动、去重、拆分与归并；
    \item \textbf{链表题集中在三种核心操作：}逆置、双指针、归并/拆分；
    \item \textbf{二叉树题集中在：}递归遍历应用（WPL、深度判定、表达式树）、层序遍历；
    \item \textbf{分治、贪心与空间换时间：}中位数、主元素、未出现的最小正整数、逆序对等典型问题；
    \item \textbf{图算法：}基于 DFS/BFS 的可达性、路径约束、连通性与环检测；
    \item \textbf{作答要求：}必须给出核心伪代码，说明边界条件，并分别分析时间复杂度和辅助空间复杂度。
\end{enumerate}
\end{conclusion}
...
\item \textbf{（单项选择题）} 在单链表中，用快慢指针法查找中间结点。慢、快指针均从首结点出发，并执行 \texttt{while (fast \&\& fast->next)}：慢指针每次走一步，快指针每次走两步。当链表长度为偶数时，循环结束后慢指针指向的是
\begin{center}
\begin{tabular}{ll}
(A) 前半段的最后一个结点 & (B) 后半段的第一个结点 \\[4pt]
(C) 链表的最后一个结点 & (D) 仅由链表长度的奇偶性无法确定
\end{tabular}
\end{center}

\ifshowsolutions\par\noindent\textbf{解析：} 长度为 $2k$ 时，快指针走完 $2k$ 个结点后变为 \texttt{NULL}，慢指针恰好前进 $k$ 步，指向第 $k+1$ 个结点，即后半段的第一个结点。\boxed{\textbf{答案：}(B)}\else \answerarea \fi
...
(C) 若检测阶段采用快指针每次走三步，找到相遇点后仍可直接用经典“头指针+相遇点各走一步”定位环入口 \\[4pt]
(D) 算法的时间复杂度为 $O(n)$，空间复杂度为 $O(1)$
\end{tabularx}
\end{center}

\ifshowsolutions\par\noindent\textbf{解析：} 经典 Floyd 算法的步长为慢指针1、快指针2，环内相对步长为1，故必能相遇。即使快指针改为每次3步，环内相对步长为2：设入环前的路径长度为 $a$，慢指针进入环时快指针已多走 $3a-a=2a$ 步，即初始相位差 $d\equiv 2a\pmod\lambda$，相遇条件 $2t\equiv -d\pmod\lambda$ 等价于 $2(t+a)\equiv0\pmod\lambda$，对任意环长 $\lambda$ 恒有解（$\lambda$ 为奇数时取 $t\equiv-a\pmod\lambda$，为偶数时取 $t\equiv-a\pmod{\lambda/2}$），因此检测阶段必相遇。不过，经典入口定位依赖相遇点满足 $a=k\lambda-b$ 的 1:2 关系；改成1:3后，直接重置并各走一步不一定落在入口。因此错误的是 (C)。\boxed{\textbf{答案：}(C)}\else \answerarea \fi
...
\ifshowsolutions\par\noindent\textbf{解析：} 三色 DFS 中遇到指向 GRAY 结点的边说明存在回边；拓扑排序无法处理完全部顶点也说明有环。普通并查集适合无向图判环，不能直接判断一般有向图中的环。\boxed{\textbf{答案：}(C)}\else \answerarea \fi
...
\ifshowsolutions\par\noindent\textbf{答案：} 逆序对；$O(n\log n)$。左右两段已经有序，右段当前元素一旦小于左段当前元素，就同时小于左段全部剩余元素。\else \answerarea \fi
...
\ifshowsolutions\par\noindent\textbf{答案：} 无右孩子；非完全二叉树。若结点只有左孩子，它必须位于最后一个非叶位置；若它连左孩子也没有，则它本身已是叶结点。两种情况下后续层序结点都必须是叶结点。\else \answerarea \fi

\item \textbf{（简答题）} 简述用非递归中序遍历寻找二叉排序树中第 $k$ 小元素的基本思路，并说明时间复杂度。

\ifshowsolutions\par\noindent\textbf{答案：} 使用栈模拟中序遍历：沿左链依次入栈，弹出一个结点时将计数加1；计数首次等于 $k$ 时，该结点就是第 $k$ 小元素，然后转向其右子树继续同样过程。若在访问完 $k$ 个结点前栈已空，则 $k$ 越界。时间复杂度为 $O(h+k)$，最坏为 $O(n)$；辅助空间为 $O(h)$，其中 $h$ 为树高。\else \answerarea \fi

\item \textbf{（简答题）} 说明为什么 Boyer-Moore 多数投票算法需要「验证阶段」（即第二次遍历确认候选元素的出现次数）。如果不验证，是否会出错？请给出反例。

\ifshowsolutions\par\noindent\textbf{答案：} 抵消过程只保证“若主元素存在，最终候选一定是它”，并不保证任意输入都存在主元素。例如数组 $[1,2,3]$ 最终可能留下候选3，但任何元素的出现次数都没有超过 $3/2$。因此必须第二次遍历统计候选出现次数，只有大于 $\lfloor n/2\rfloor$ 才能返回。\else \answerarea \fi

\item \textbf{（算法设计）} 编写算法，将带头结点的单链表 $L$ 中数据域为奇数的结点移到数据域为偶数的结点之前，且保持奇数和偶数各自内部的相对顺序不变。时间复杂度要求 $O(n)$，空间复杂度 $O(1)$。

\ifshowsolutions\par\noindent\textbf{解析：} 将原链表拆成奇数链和偶数链，每次只摘下并尾插原结点，最后连接两条链。
\begin{lstlisting}[language=C]
void OddBeforeEven(LinkList L) {
    LNode oddHead, evenHead;
    LNode *ot = &oddHead, *et = &evenHead;
    oddHead.next = evenHead.next = NULL;
    for (LNode *p = L->next, *next; p; p = next) {
        next = p->next;
        p->next = NULL;
        if (p->data % 2 != 0) { ot->next = p; ot = p; }
        else                  { et->next = p; et = p; }
    }
    ot->next = evenHead.next;
    L->next = oddHead.next ? oddHead.next : evenHead.next;
}
\end{lstlisting}
每个结点只处理一次，时间复杂度 $O(n)$；只使用常数个指针，辅助空间 $O(1)$。尾插保证奇、偶两组内部均稳定。\else \answerarea \fi
...
\ifshowsolutions\par\noindent\textbf{解析：} 完全二叉树有 $n_0=\lceil n/2\rceil$ 个叶结点，故 $n_0=2024/2=1012$。\boxed{\textbf{答案：}(B)}\else \answerarea \fi
...
\ifshowsolutions\par\noindent\textbf{解析：} 序列严格递减时，每一对下标 $i<j$ 都构成逆序对，共有 $\binom n2=n(n-1)/2$ 对。\boxed{\textbf{答案：}(C)}\else \answerarea \fi

\item \textbf{（真题风格，综合题）} 设一棵关键字互异的二叉树以二叉链表存储，结点结构为 \texttt{(lchild, data, rchild)}。按“左子树关键字严格小于根、右子树关键字严格大于根”的定义，设计算法判断该树是否为二叉排序树（BST）。要求：
\begin{enumerate}
    \item[(1)] 描述算法基本思想；
    \item[(2)] 用 C 语言实现算法；
...
\ifshowsolutions\par\noindent\textbf{解析：} BST 的中序遍历结果必须严格递增。用 \texttt{prev} 记录中序序列中上一个已访问结点，一旦出现 \texttt{prev->data >= T->data} 即可判假。
\begin{lstlisting}[language=C]
bool CheckBST(BiTree T, BiTree *prev) {
    if (!T) return true;
    if (!CheckBST(T->lchild, prev)) return false;
    if (*prev && (*prev)->data >= T->data) return false;
    *prev = T;
    return CheckBST(T->rchild, prev);
}

bool IsBST(BiTree T) {
    BiTree prev = NULL;
    return CheckBST(T, &prev);
}
\end{lstlisting}
每个结点访问一次，时间复杂度 $O(n)$；递归栈为 $O(h)$，其中 $h$ 为树高。\else \answerarea \fi

\item \textbf{（真题风格，综合题）} 一个长度为 $n$（$n>1$）且元素互异的严格递增整数数组经过非零次循环右移后得到数组 $A$。等价地，存在某个下标 $p$（$0\leqslant p < n-1$），使 $A[0\ldots p]$、$A[p+1\ldots n-1]$ 各自严格递增，且后一段的所有元素均小于前一段的所有元素。设计 $O(\log n)$ 时间复杂度的算法找出最小值。要求：
\begin{enumerate}
    \item[(1)] 描述算法思想；
    \item[(2)] 用 C 语言实现算法；
...
\ifshowsolutions\par\noindent\textbf{解析：} 将中点元素与当前区间末元素比较。若 $A[mid]>A[high]$，最小值一定在 $mid$ 右侧；否则最小值位于 $[low,mid]$，且不能排除 $mid$ 本身。
\begin{lstlisting}[language=C]
...
\end{lstlisting}
每轮至少排除当前区间的一半，故时间复杂度为 $O(\log n)$，辅助空间为 $O(1)$。题干要求元素互异且确为严格递增数组的旋转，正是上述二分判据成立的条件。\else \answerarea \fi

\item \textbf{（真题风格，综合题）} 设非空有向图 $G=(V,E)$ 以邻接表存储。设计算法，判断 $G$ 是否为一棵以某个顶点为根、边由父结点指向孩子结点的有向根树（out-arborescence）。也就是说，应恰有一个入度为0的根，其余顶点入度均为1，并且从根可到达所有顶点。要求：
\begin{enumerate}
    \item[(1)] 描述算法思想；
    \item[(2)] 用 C 语言实现算法；
    \item[(3)] 分析算法时空复杂度。
\end{enumerate}
\textbf{提示：}满足上述入度条件并且从根可达所有顶点时，边数自动为 $n-1$，且不可能存在有向环。

\ifshowsolutions\par\noindent\textbf{解析：} 扫描邻接表统计每个顶点的入度和边数：必须恰有一个入度为0的根，其余顶点入度均为1，因而边数为 $n-1$。再从根做一次 DFS/BFS，若能访问全部顶点，则图为有向根树。
\begin{lstlisting}[language=C]
static int DFSCount(const ALGraph *G, int v, bool visited[]) {
    int count = 1;
    visited[v] = true;
    for (ArcNode *p = G->adjlist[v].first; p; p = p->next) {
        if (!visited[p->adjvex])
            count += DFSCount(G, p->adjvex, visited);
    }
    return count;
}

bool IsArborescence(const ALGraph *G) {
    int indeg[MaxV] = {0}, edges = 0;
    for (int u = 0; u < G->n; ++u)
        for (ArcNode *p = G->adjlist[u].first; p; p = p->next) {
            if (++indeg[p->adjvex] > 1) return false;
            ++edges;
        }

    int root = -1, roots = 0;
    for (int v = 0; v < G->n; ++v) {
        if (indeg[v] == 0) { root = v; ++roots; }
        else if (indeg[v] != 1) return false;
    }
    if (roots != 1 || edges != G->n - 1) return false;

    bool visited[MaxV] = {false};
    int count = DFSCount(G, root, visited); // 只计首次访问的顶点
    return count == G->n;
}
\end{lstlisting}
扫描边和遍历图的总时间为 $O(|V|+|E|)$；入度、访问标记和递归栈/队列占 $O(|V|)$ 辅助空间。\else \answerarea \fi
...
\ifshowsolutions\par\noindent\textbf{解析：} 先序区间首元素是根；在中序区间中找到根后，中序序列被分为左右子树，据左子树长度切分先序区间，再递归构造。
\begin{lstlisting}[language=C]
BiTree Build(int pre[], int pl, int pr,
             int in[],  int il, int ir) {
    if (pl > pr) return NULL;
    int k = il;
    while (k <= ir && in[k] != pre[pl]) ++k;
    if (k > ir) return NULL;           // 输入序列不一致

    BiTree root = malloc(sizeof(*root));
    root->data = pre[pl];
    int left = k - il;
    root->lchild = Build(pre, pl+1, pl+left, in, il, k-1);
    root->rchild = Build(pre, pl+left+1, pr, in, k+1, ir);
    return root;
}
\end{lstlisting}
直接在线性中序区间内找根，最坏时间复杂度为 $O(n^2)$；递归栈为 $O(h)$，另需 $O(n)$ 新结点空间。若预先用散列表建立“关键字$\to$中序下标”的映射，构造时间可降为平均 $O(n)$，额外映射空间为 $O(n)$。\else \answerarea \fi
...
\ifshowsolutions\par\noindent\textbf{解析：} 使用快速选择。一次划分把枢轴放到最终位置 $p$；若 $p=k-1$ 即返回，否则只在包含第 $k$ 小元素的一侧继续划分。
\begin{lstlisting}[language=C]
...
\end{lstlisting}
随机输入或随机选择枢轴时，期望工作量满足 $n+n/2+n/4+\cdots=O(n)$；若每次枢轴都极端不平衡，最坏为 $O(n^2)$。迭代实现的辅助空间为 $O(1)$，但会原地改变数组元素次序。\else \answerarea \fi
...
\ifshowsolutions\par\noindent\textbf{解析：} 先用快慢指针找到前半段末结点，原地逆置后半段并与前半段逐结点比较；比较后再逆置一次以恢复原链表。
\begin{lstlisting}[language=C]
LNode *Reverse(LNode *p) {
    LNode *pre = NULL;
    while (p) {
        LNode *next = p->next;
        p->next = pre; pre = p; p = next;
    }
    return pre;
}
...
bool IsPalindrome(LinkList L) {
    ...
    LNode *second = Reverse(slow->next);
    bool ok = true;
    for (LNode *p = L->next, *q = second; q; p = p->next, q = q->next)
        if (p->data != q->data) { ok = false; break; }
    slow->next = Reverse(second);       // 恢复原链表
    return ok;
}
\end{lstlisting}
找中点、逆置、比较和恢复均为线性过程，总时间 $O(n)$，辅助空间 $O(1)$。\else \answerarea \fi
