...
\ifshowsolutions\par\noindent\textbf{解析：} 经典 Floyd 算法的步长为慢指针1、快指针2，环内相对步长为1，故必能相遇；入口定位结论也建立在这一经典步长关系上。若快指针改为每次3步，相对步长为2，当环长为偶数时可能因同余类限制而永不相遇，不能无条件保证。\boxed{\textbf{答案：}(B)}\fi
...
bool IsArborescence(ALGraph G) {
    ...
    if (roots != 1 || edges != G.vexnum - 1) return false;
