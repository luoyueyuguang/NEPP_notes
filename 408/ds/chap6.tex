...
\item \textbf{（单项选择题）} 对一个长度为 $n$ 的有序顺序表进行折半查找，查找成功时的 ASL 约等于
\begin{center}
\begin{tabular}{ll}
(A) $\log_2 n$ & (B) $\log_2(n+1)-1$ \\[6pt]
(C) $(n+1)/2$ & (D) $\sqrt{n}$
\end{tabular}
\end{center}

\item \textbf{（单项选择题）} 在含有 $n$ 个结点的二叉排序树中查找一个关键字，以下说法正确的是
\begin{center}
\begin{tabular}{ll}
(A) 时间复杂度恒为 $O(\log n)$ \\[4pt]
(B) 等概率下 ASL$_{\text{成功}} = \frac{n+1}{2}$ \\[4pt]
(C) 最坏情况下时间复杂度为 $O(n)$ \\[4pt]
(D) 查找过程一定沿着一条从根到叶子的路径进行
\end{tabular}
\end{center}

\item \textbf{（单项选择题）} 在下列平衡二叉树中插入一个结点后造成了不平衡，其中需要进行先左旋、后右旋（LR 型）操作的是
\begin{center}
\begin{tabular}{ll}
(A) 在 A 的左子树的左子树上插入 \\[4pt]
(B) 在 A 的右子树的右子树上插入 \\[4pt]
(C) 在 A 的左子树的右子树上插入 \\[4pt]
(D) 在 A 的右子树的左子树上插入
\end{tabular}
\end{center}

\item \textbf{（单项选择题）} 一棵 5 阶 B-树中，除根以外的每个非叶结点至少包含的关键字个数为
\begin{center}
\begin{tabular}{ll}
(A) $1$ & (B) $2$ \\[6pt]
(C) $3$ & (D) $4$
...
\item \textbf{（单项选择题）} 设散列表长 $m=14$，散列函数 $H(key)=key \bmod 13$。表中已有关键字 $15, 28, 41, 54$，用线性探测法处理冲突，则关键字 $67$ 的散列地址是
\begin{center}
\begin{tabular}{ll}
(A) $2$ & (B) $3$ \\[6pt]
(C) $4$ & (D) $5$
...
\item \textbf{（填空题）} 在一个长度为 $n$ 的顺序表中进行顺序查找，等概率下查找成功时的平均比较次数为 \underline{\qquad\qquad}，查找不成功时的比较次数为 \underline{\qquad\qquad}。

\item \textbf{（填空题）} 具有 $n$ 个关键字的 $m$ 阶 B-树，其\textbf{最大高度}（不含失败结点层）的表达式为 \underline{\qquad\qquad}。

\item \textbf{（简答题）} 简述 B+树与 B-树的主要区别，并说明为什么数据库索引通常使用 B+树而非 B-树。

\item \textbf{（计算题）} 设有关键字序列 $\{5, 16, 20, 27, 30, 36, 44, 55, 60, 67, 71\}$，画出对其进行折半查找的判定树，并计算 ASL$_{\text{成功}}$。

\item \textbf{（算法设计）} 编写非递归算法，在二叉排序树中插入一个关键字 $k$，返回插入后树的根结点指针。若关键字已存在，则不插入。
...
\item \textbf{（真题风格，单项选择题）} 设有序顺序表中有 $1000$ 个元素，用折半查找方法进行查找，查找成功时关键字比较次数的最大值为
\begin{center}
\begin{tabular}{ll}
(A) $9$ & (B) $10$ \\[6pt]
(C) $11$ & (D) $1000$
...
\item \textbf{（真题风格，单项选择题）} 下列关于散列表查找的说法中，正确的是
\begin{center}
\begin{tabular}{ll}
(A) 散列表的装填因子越小，发生冲突的可能性越小，查找效率越高 \\[4pt]
(B) 开放定址法中删除一个元素可以直接将其位置清空 \\[4pt]
(C) 链地址法中装填因子不能大于 1 \\[4pt]
(D) 二次探测法不会产生聚集现象
\end{tabular}
\end{center}

\item \textbf{（真题风格，综合题）} 设散列表长 $m=11$，散列函数 $H(key)=key \bmod 11$，采用线性探测法处理冲突。依次插入关键字序列 $\{13, 24, 35, 46, 57, 68, 79\}$。
\begin{enumerate}
    \item[(1)] 构造该散列表；
    \item[(2)] 计算等概率下的 ASL$_{\text{成功}}$；
    \item[(3)] 计算等概率下的 ASL$_{\text{不成功}}$。
\end{enumerate}

\item \textbf{（真题风格，综合题）} 依次将关键字 $10, 20, 30, 25, 5, 35, 15, 28$ 插入一棵初始为空的 AVL 树。
\begin{enumerate}
    \item[(1)] 画出每次插入并调整后的 AVL 树，标注旋转类型及旋转结点；
    \item[(2)] 计算该 AVL 树查找成功的 ASL（等概率）。
\end{enumerate}

\item \textbf{（真题风格，综合题）} 设有一棵 3 阶 B-树，初始为空，依次插入关键字序列 $\{45, 24, 53, 90, 12, 30, 26, 37, 61\}$。
\begin{enumerate}
    \item[(1)] 画出最终 B-树的结构；
    \item[(2)] 在(1)的结果上删除关键字 $24$，画出删除后的 B-树；
    \item[(3)] 继续删除关键字 $45$，画出最终 B-树。
\end{enumerate}

\item \textbf{（真题风格，综合题）} 已知二叉排序树中结点的关键字值均不相同。设计算法：
\begin{enumerate}
    \item[(1)] 判断一棵二叉树是否为二叉排序树；
    \item[(2)] 分析算法的时间复杂度。
\end{enumerate}
...
\noindent\textbf{拓展阅读：}
\begin{itemize}
    \item 邓俊辉《数据结构（C++语言版）》第 7 章「二叉搜索树」—— BST 的完整 ADT 实现，包括前驱后继的查找、带旋转的插入删除统一框架（3+4 重构）。
    \item 邓俊辉《数据结构（C++语言版）》第 8 章「高级搜索树」—— AVL 树的详细旋转分析、伸展树（Splay Tree）自调整策略、B-树的完整实现与复杂度证明。
    \item Sedgewick《算法》第 3.2 节「二叉查找树」—— BST 的递归实现、基于 Hibbard 删除法的完整讨论、性能分析。
    \item Sedgewick《算法》第 3.3 节「平衡查找树」—— 2-3 树到红黑树的自然过渡、红黑树的插入删除操作与代码实现，以及 B-树在文件系统中的实际应用。
\end{itemize}
