...
\draw (f) -- (h);
\draw (g) -- (i);
\node[align=center] at (4.6,-1.6){用中序\textbf{前驱} 40 替代 50，\\再删除原 40（退化为\\仅有一棵子树的情况）};
\end{tikzpicture}
\end{center}
...
\begin{center}
\begin{tikzpicture}[>=stealth, level distance=1.9cm, level 1/.style={sibling distance=3.0cm}, level 2/.style={sibling distance=2.4cm}, font=\scriptsize]
\node[circle,fill=black,text=white] (r) {10}
  child { node[circle,fill=Burgundy,text=white] {5}
    child { node[circle,fill=black,text=white] {3} }
    child { node[circle,fill=black,text=white] {7} } }
  child { node[circle,fill=Burgundy,text=white] {15}
    child { node[circle,fill=black,text=white] {12} }
    child { node[circle,fill=black,text=white] {18} } };
...
\begin{tikzpicture}[>=stealth, font=\scriptsize]
  % 根 / 内部结点（仅索引关键字）
  \node[draw,rectangle,minimum width=3.2cm,minimum height=0.7cm,fill=Navy!12] (root) at (0,0) {索引：$7\mid13$};
  % 叶结点（含全部关键字 + 记录指针）
