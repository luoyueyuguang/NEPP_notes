...
\ifshowsolutions\par\noindent\textbf{解析：} 3 阶 B-树的非根数据结点至少含 $\lceil3/2\rceil-1=1$ 个关键字。删除 10 后左孩子为 $[20]$，未下溢；再删除 20 后左孩子为空。右兄弟 $[50,60]$ 有两个关键字，可以借出：父结点中的 40 下移到左孩子，右兄弟的最小关键字 50 上移替代 40。因此根变为 $[50,70]$，三个孩子为 $[40]$、$[60]$、$[80,90]$。\boxed{\textbf{答案：}(B)}\fi
...
\ifshowsolutions\par\noindent\textbf{解析：} $H(15)=4$占4，$H(38)=5$占5，$H(61)=6$占6，$H(84)=7$占7。$H(49)=5$，5已被占$\to$探测6已占$\to$探测7已占$\to$探测8空，故49在地址8。\boxed{\textbf{答案：}(A)}\fi
...
\ifshowsolutions\par\noindent\textbf{解析：} B+树的非叶结点只起索引作用，全部关键字（及记录指针）都在叶结点，且叶结点构成有序链表。(D) 错：B-树可能在任一数据结点命中后结束；B+树即使在非叶索引结点匹配，也要继续到叶结点取得记录。\boxed{\textbf{答案：}(D)}\fi
...
\]
故 $\mathrm{ASL}_{\text{不成功}}=91/13=7$。\fi
...
\]
其中记号 $x(L,R)$ 表示结点 $x$ 的左右子树。中序遍历为 $10,20,30,40,50,60,70,80$。\fi
