...
\ifshowsolutions\par\noindent\textbf{解析：}
插入40$\to$插入30（40的左）$\to$插入20：40的平衡因子=2，LL型，右旋40$\to$30为根，20在30左，40在30右$\to$插入60（40右）$\to$插入50（60左）：30平衡因子=-2，先RL（40左旋，再30右旋）$\to$插入80（60右）$\to$插入70（80左）：30平衡因子=-2，先RL$\to$插入10（20左）。中序：10,20,30,40,50,60,70,80。\fi
