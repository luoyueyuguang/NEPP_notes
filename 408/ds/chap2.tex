...
\begin{tikzpicture}[>=stealth, font=\scriptsize]
  \node[draw, rectangle, minimum width=1.0cm, minimum height=0.5cm, fill=SoftBlue] (s1) at (0,0){$a_1$};
  \node[draw, rectangle, minimum width=1.0cm, minimum height=0.5cm, fill=SoftBlue] (s2) at (0,0.62){$a_2$};
  \node[draw, rectangle, minimum width=1.0cm, minimum height=0.5cm, fill=SoftBlue] (s3) at (0,1.24){$a_3$};
  \node[draw, rectangle, minimum width=1.0cm, minimum height=0.5cm, fill=SoftAmber] (s4) at (0,1.86){$a_4$};
  \node[font=\scriptsize, Navy] (toplab) at (1.6,1.86){栈顶 Top};
  \node[font=\scriptsize, Navy] (botlab) at (1.6,0){栈底 Bottom};
  \draw[->, Teal, thick] (toplab.west) -- (s4.east);
  \draw[->, Burgundy, thick] (botlab.west) -- (s1.east);
  \node[draw, rectangle, minimum width=1.0cm, minimum height=0.5cm, fill=SoftAmber] (e) at (0,2.6){新元素 $e$};
  \draw[->, Teal, thick, dashed] (e.south) -- (s4.north);
  \node[font=\scriptsize, black!70] at (0,-0.62){入栈：$e$ 压到栈顶；出栈：弹出栈顶元素（后进先出 LIFO）};
\end{tikzpicture}
\end{center}
\end{mydef}
...
\begin{tikzpicture}[>=stealth, font=\scriptsize]
  \foreach \i in {0,1,2,3,4,5,6,7}{
    \draw[thick] (\i,0) rectangle (\i+1,0.9);
    \node[font=\scriptsize, black!55] at (\i+0.5,-0.28){$\i$};
  }
  \node[font=\scriptsize, TealDark] at (0.5,0.45){$a_1$};
  \node[font=\scriptsize, TealDark] at (1.5,0.45){$a_2$};
  \node[font=\scriptsize, Burgundy] at (6.5,0.45){$b_1$};
  \node[font=\scriptsize, Burgundy] at (7.5,0.45){$b_2$};
  \node[font=\scriptsize, MutedText] at (4.0,0.45){空};
  \draw[->, Teal, thick] (1.5,1.3) -- (1.5,0.95);
  \draw[->, Burgundy, thick] (7.5,1.3) -- (7.5,0.95);
  \node[font=\scriptsize, Navy] at (1.5,1.55){\texttt{top1}};
  \node[font=\scriptsize, Navy] at (7.5,1.55){\texttt{top2}};
  \node[font=\scriptsize, black!70] at (3.5,-1.0){共享栈：栈1 从低地址向高地址增长，栈2 从高地址向低地址增长};
  \node[font=\scriptsize, black!70] at (3.5,-1.5){判满条件：\texttt{top1} $+1=$ \texttt{top2}（两栈顶指针相邻）};
\end{tikzpicture}
\end{center}
\end{conclusion}
...
\begin{tikzpicture}[>=stealth, scale=1.0]
  % 环底
  \draw[RuleGray, thick] (0,0) circle (2.05);
  % 环形单元：Q[0]..Q[7]
  \foreach \i/\x/\y in {0/0/2.05, 1/1.45/1.45, 2/2.05/0, 3/1.45/-1.45, 4/0/-2.05, 5/-1.45/-1.45, 6/-2.05/0, 7/-1.45/1.45}
    \node[draw, rounded corners=1.5pt, fill=SoftBlue!22, minimum width=0.74cm, minimum height=0.52cm,
          font=\scriptsize, inner sep=1.5pt] (q\i) at (\x,\y) {Q[\i]};
  % 队头 front：红色，指向 Q[1]
  \node[red, font=\scriptsize] (f) at (2.75,2.4) {front（队头）};
  \draw[->, red, very thick] (f.south) to[bend right=28] (q1.north east);
  % 队尾 rear：蓝色，指向 Q[5]
  \node[blue, font=\scriptsize] (r) at (-2.75,-2.4) {rear（队尾）};
  \draw[->, blue, very thick] (r.north) to[bend right=28] (q5.south west);
  % 空队/队满说明
  \node[font=\scriptsize, black!70, align=center] at (0,-3.35) {front 指向队头元素；rear 指向队尾的下一个空位\\ 队空：front $=$ rear；队满：front $=$ (rear$+1$) mod MaxSize};
\end{tikzpicture}
\end{center}
\end{mydef}
...
策略 & 判空 & 判满 \\
\hline
\textbf{（1） 牺牲一个单元} & \texttt{front == rear} & \texttt{(rear+1)\%MaxSize == front} \\
\textbf{（2） 增设长度计数} & \texttt{len == 0} & \texttt{len == MaxSize} \\
\textbf{（3） 增设 tag 标记} & \texttt{front==rear \&\& tag==0} & \texttt{front==rear \&\& tag==1} \\
\hline
\end{tabularx}
\end{tablebox}

策略（1）是 408 最常用写法，代价是数组有效容量为 $\text{MaxSize}-1$。策略（2）和（3）可以充分利用所有单元，但需要额外维护计数器或标记位。tag 标记在每次入队操作时置 1，出队操作时置 0。
...
\begin{center}
\begin{tikzpicture}[>=stealth, font=\scriptsize]
  \node[draw, rectangle, minimum width=0.9cm, minimum height=0.55cm, fill=SoftGray] (h) at (0,0){头结点};
  \node[draw, rectangle, minimum width=0.9cm, minimum height=0.55cm, fill=SoftBlue] (a1) at (1.7,0){$a_1$};
  \node[draw, rectangle, minimum width=0.9cm, minimum height=0.55cm, fill=SoftBlue] (a2) at (3.4,0){$a_2$};
  \node[draw, rectangle, minimum width=0.9cm, minimum height=0.55cm, fill=SoftBlue] (a3) at (5.1,0){$a_3$};
  \draw[->, Teal, thick] (h.east) -- (a1.west);
  \draw[->, Teal, thick] (a1.east) -- (a2.west);
  \draw[->, Teal, thick] (a2.east) -- (a3.west);
  \node[font=\scriptsize, MutedText] at (6.7,0){$\cdots$};
  \node[font=\scriptsize, Navy] (flab) at (0.45,-1.0){\texttt{front}};
  \node[font=\scriptsize, Navy] (rlab) at (5.55,1.15){\texttt{rear}};
  \draw[->, Burgundy, thick] (flab.north) -- (h.south);
  \draw[->, Teal, thick] (rlab.south) -- (a3.north);
  \node[font=\scriptsize, black!70] at (3.1,-1.5){入队在队尾（rear 侧）追加，出队从队头（front 侧）删除};
\end{tikzpicture}
\end{center}
\end{formula}
...
\subsection{数组与特殊矩阵的压缩存储}

\begin{conclusion}{数组的地址计算}{array-address}
设二维数组 $A[m][n]$ 的每个元素占 $L$ 字节，首元素地址为 $\operatorname{LOC}(A[0][0])$。
\begin{itemize}
    \item \textbf{按行优先：}$\operatorname{LOC}(A[i][j])=\operatorname{LOC}(A[0][0])+(i\cdot n+j)L$；
    \item \textbf{按列优先：}$\operatorname{LOC}(A[i][j])=\operatorname{LOC}(A[0][0])+(j\cdot m+i)L$。
\end{itemize}
若题目下标从 1 开始，必须先把 $i,j$ 分别减 1；若给出了各维下界，也要先减去对应下界。
\end{conclusion}

\begin{conclusion}{特殊矩阵的压缩存储}{matrix-compression}
\begin{itemize}
    \item \textbf{对称矩阵：}只存上三角或下三角，共 $n(n+1)/2$ 个元素。按行存下三角（含主对角线）、下标从 0 开始时，$i\geqslant j$ 的元素映射为 $k=i(i+1)/2+j$；访问上三角元素 $A[i][j]$ 时改取 $A[j][i]$。
    \item \textbf{三角矩阵：}只存一个三角区和一个公共常量，共 $n(n+1)/2+1$ 个存储单元。
    \item \textbf{三对角矩阵：}只有主对角线及其上下相邻对角线可能非零，共 $3n-2$ 个非零位置。
    \item \textbf{稀疏矩阵：}非零元素很少时可用三元组 $(i,j,v)$ 表或十字链表，存储量与非零元素个数 $t$ 相关，而不是固定为 $n^2$。
\end{itemize}
\end{conclusion}
...
// Factorial(4):
// 调用: fact(4) $\to$ fact(3) $\to$ fact(2) $\to$ fact(1)
// 返回: 1 $\to$ 2 $\to$ 3 $\to$ 4 $\to$ 24
\end{lstlisting}
...
\begin{conclusion}{层次遍历（BFS）}{graph-level-order}
...
\item \textbf{（真题风格·选择题）} 设栈S的初始状态为空，入栈序列为$1,2,3,\ldots,n$，若出栈序列的第一个元素是$k$（$2\leqslant k\leqslant n$），则以下说法正确的是
\begin{center}
\begin{tabular}{ll}
...
\ifshowsolutions\par\noindent\textbf{解析：} 第一个出栈元素为 $k$，说明 $1,2,\ldots,k$ 已依次入栈且 $k$ 随即出栈；此时 $1,\ldots,k-1$ 仍在栈中，1 位于栈底。后续仍有多种合法操作次序。\boxed{\textbf{答案：}(A)}\else \answerarea \fi

\item \textbf{（真题风格·选择题）} 设循环队列的存储空间为$Q[0..m-1]$，\texttt{front}指向队头元素，\texttt{rear}指向队尾元素的下一位置。则元素入队时\texttt{rear}的变化为
\begin{center}
\begin{tabular}{ll}
...
\ifshowsolutions\par\noindent\textbf{解析：} 数组下标范围为 $0\sim m-1$，rear 前进一步后应对 $m$ 取模。\boxed{\textbf{答案：}(C)}\else \answerarea \fi

\item \textbf{（真题风格·选择题）} 中缀表达式$A+B*(C-D)-E/F$对应的后缀表达式是
\begin{center}
\begin{tabular}{ll}
...
\ifshowsolutions\par\noindent\textbf{解析：} 先得到 $C-D\to CD-$，再与 $B$ 相乘得 $BCD-*$，与 $A$ 相加得 $ABCD-*+$；最后计算 $E/F\to EF/$ 并相减，结果为 $ABCD-*+EF/-$。\boxed{\textbf{答案：}(A)}\else \answerarea \fi

\item \textbf{（真题风格·选择题）} 对后缀表达式$ab+c*d-ef+/$进行求值（设$a,b,c,d,e,f$均为操作数），在从左到右扫描求值的过程中，操作数栈中元素个数的最大值为
\begin{center}
\begin{tabular}{ll}
(A) 2 & (B) 3 \\[6pt]
(C) 4 & (D) 5
...
\ifshowsolutions\par\noindent\textbf{解析：} 扫描各符号后的栈深依次为 $1,2,1,2,1,2,1,2,3,2,1$，最大值为3。\boxed{\textbf{答案：}(B)}\else \answerarea \fi

\item \textbf{（真题风格·选择题）} 设用数组$A[0..n-1]$实现两个栈共享存储空间的共享栈。栈1的栈底在$A[0]$处，从低地址向高地址增长；栈2的栈底在$A[n-1]$处，从高地址向低地址增长。\texttt{top1}指向栈1的栈顶元素，\texttt{top2}指向栈2的栈顶元素。则栈满的条件是
\begin{center}
\begin{tabular}{ll}
...
\ifshowsolutions\par\noindent\textbf{解析：} 两个栈顶相邻时中间已经没有空闲单元，故判满条件为 $\texttt{top1}+1=\texttt{top2}$。\boxed{\textbf{答案：}(B)}\else \answerarea \fi

\item \textbf{（真题风格·选择题）} 设循环队列的存储空间为$Q[0..m-1]$，\texttt{front}指向队头元素，\texttt{rear}指向队尾元素的下一位置。在采用「牺牲一个存储单元」判满策略时，队列为空的判定条件是
\begin{center}
\begin{tabular}{ll}
...
\ifshowsolutions\par\noindent\textbf{解析：} 牺牲一个单元时，用 $\texttt{front}=\texttt{rear}$ 表示空，用 $(\texttt{rear}+1)\bmod m=\texttt{front}$ 表示满。\boxed{\textbf{答案：}(A)}\else \answerarea \fi

\item \textbf{（基础题）} 一个 $6\times8$ 的二维数组按行优先存储，每个元素占 4 B，$A[0][0]$ 的地址为 1000，则 $A[3][5]$ 的地址是多少？

\ifshowsolutions\par\noindent\textbf{答案：}$1000+(3\times8+5)\times4=1116$。\else \answerarea \fi

\item \textbf{（基础题）} 一个 $n$ 阶三对角矩阵按行压缩存储，可能的非零元素最多有多少个？

\ifshowsolutions\par\noindent\textbf{答案：}第一行和最后一行各 2 个，中间 $n-2$ 行各 3 个，共 $2+3(n-2)+2=3n-2$。\else \answerarea \fi
...
\item \textbf{（真题风格·选择题）} 以下算法中，需要使用队列作为辅助数据结构的是
\begin{center}
\begin{tabular}{ll}
...
\ifshowsolutions\par\noindent\textbf{解析：} 层次遍历按层保存尚待访问的结点，需要先进先出的队列；先序遍历、DFS 和表达式转换通常使用栈。\boxed{\textbf{答案：}(B)}\else \answerarea \fi
...
(1) 合法序列共有3个：
\[
(3,2,1,4,5),\quad(3,2,4,1,5),\quad(3,4,2,1,5).
\]
(2) 第一个输出3时，1、2仍按“1在下、2在上”留在栈中。若最终输出5，则在5入栈前必须把1、2、4全部弹出，否则5会挡住其下方元素。由状态“栈内1、2，下一输入4”枚举：先弹2后可取1、4或4、1；也可先压入并弹出4，再弹2、1。判断原则是任何时刻只能弹出当前栈顶，未入栈元素不能提前输出。\else \answerarea \fi
...
\ifshowsolutions\par\noindent\textbf{答案：}
(1) 元素个数为 $(\texttt{rear}-\texttt{front}+M)\bmod M$。
(2) $(3-6+10)\bmod10=7$，共有7个元素。
(3) 判空：$\texttt{front}=\texttt{rear}$；判满：$(\texttt{rear}+1)\bmod M=\texttt{front}$。\else \answerarea \fi
