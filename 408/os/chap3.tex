...
\draw[thick] (0,0) rectangle (3.2,6.2);
\fill[SoftGray] (0,0) rectangle (3.2,1.0);
\node[align=center] at (1.6,0.5) {系统区\\（低地址，存放 OS）};
\fill[SoftBlue] (0,1.1) rectangle (3.2,2.0);
\node at (1.6,1.55) {分区 1};
\fill[SoftTeal] (0,2.1) rectangle (3.2,3.0);
\node at (1.6,2.55) {分区 2};
\fill[SoftAmber] (0,3.1) rectangle (3.2,4.0);
\node at (1.6,3.55) {分区 3};
\fill[SoftAmber!45] (0,4.1) rectangle (3.2,5.0);
\node at (1.6,4.55) {分区 4};
\fill[SoftGray!50] (0,5.1) rectangle (3.2,6.2);
\node at (1.6,5.65) {用户区剩余};
\node[font=\scriptsize, black!70] at (3.9,6.0) {高地址};
\node[font=\scriptsize, black!70] at (3.9,0.2) {低地址};
\node[font=\scriptsize, black!60] at (1.6,-0.55) {固定分区：各分区大小固定，每区装入一道程序（存在内部碎片）};
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}[>=stealth, font=\scriptsize]
  % 逻辑地址：页号 + 偏移
  \node[draw,rectangle,minimum width=1.5cm,minimum height=0.7cm,fill=SoftBlue] (pn) at (0,0) {页号 $p_n$};
  \node[draw,rectangle,minimum width=1.5cm,minimum height=0.7cm,fill=SoftBlue] (d) at (1.9,0) {偏移 $d$};
  \node[font=\scriptsize, text=TealDark] at (0.95,1.15) {逻辑地址};
  % 页表
  \node[draw,rectangle,minimum width=3.0cm,minimum height=0.7cm,fill=SoftGray] (pt) at (0,-1.7) {页表：$p_n \to f_n$};
  % 物理地址：页框号 + 偏移（不变）
  \node[draw,rectangle,minimum width=1.5cm,minimum height=0.7cm,fill=SoftAmber] (f) at (0,-3.4) {页框号 $f_n$};
  \node[draw,rectangle,minimum width=1.5cm,minimum height=0.7cm,fill=SoftAmber] (d2) at (1.9,-3.4) {偏移 $d$};
  \node[font=\scriptsize, text=TealDark] at (0.95,-4.5) {物理地址};
  % 箭头
  \draw[->] (pn.south) -- (pt.north);
  \draw[->] (pt.south) -- (f.north);
  \draw[->,dashed] (d.south) -- (d2.north);
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}[>=stealth, font=\scriptsize]
  % 逻辑地址：p1 | p2 | d
  \node[draw,rectangle,minimum width=1.5cm,minimum height=0.7cm,fill=SoftBlue] (p1) at (-4.0,0) {$p_1$};
  \node[draw,rectangle,minimum width=1.5cm,minimum height=0.7cm,fill=SoftBlue] (p2) at (-0.5,0) {$p_2$};
  \node[draw,rectangle,minimum width=1.5cm,minimum height=0.7cm,fill=SoftBlue] (d) at (1.5,0) {偏移 $d$};
  \node[font=\scriptsize, text=TealDark] at (-1.25,1.15) {逻辑地址};
  % 页目录（一级页表） + 二级页表
  \node[draw,rectangle,minimum width=2.5cm,minimum height=1.1cm,fill=SoftGray,align=center] (dir) at (-4.0,-2.0) {页目录（一级页表）\\ 页目录项 $\to$ 二级页表基址};
  \node[draw,rectangle,minimum width=2.5cm,minimum height=1.1cm,fill=SoftGray,align=center] (pt) at (-0.5,-2.0) {二级页表\\ 页表项 $\to$ 页框号 $f$};
  % 物理地址：页框号 f | 偏移 d（不变）
  \node[draw,rectangle,minimum width=1.5cm,minimum height=0.7cm,fill=SoftAmber] (f) at (-0.5,-4.0) {页框号 $f$};
  \node[draw,rectangle,minimum width=1.5cm,minimum height=0.7cm,fill=SoftAmber] (d2) at (1.5,-4.0) {偏移 $d$};
  \node[font=\scriptsize, text=TealDark] at (0.5,-5.1) {物理地址};
  % 箭头
  \draw[->] (p1.south) -- node[above]{$p_1$ 索引} (dir.north);
  \draw[->] (p2.south) -- node[above]{$p_2$ 索引} (pt.north);
  \draw[->,dashed] (d.south) -- node[above]{偏移} (d2.north);
  \draw[->] (dir.east) -- node[above]{基址} (pt.west);
  \draw[->] (pt.south) -- (f.north);
\end{tikzpicture}
\end{center}
...
\node[draw, rectangle, fill=SoftGray, minimum width=3.6cm, minimum height=0.55cm] (a) {访问页面不在内存（有效位 = 0）};
\node[draw, rectangle, fill=SoftAmber, minimum width=3.6cm, minimum height=0.55cm, below of=a] (b) {触发缺页中断，陷入内核，保存现场};
\node[draw, rectangle, fill=SoftBlue, minimum width=3.6cm, minimum height=0.55cm, below of=b] (c) {检查地址合法性：非法则终止进程};
\node[draw, rectangle, fill=SoftBlue, minimum width=3.6cm, minimum height=0.55cm, below of=c] (d) {分配空闲页框；无空闲则按算法置换};
\node[draw, rectangle, fill=SoftTeal, minimum width=3.6cm, minimum height=0.55cm, below of=d] (e) {从磁盘读入页面（被置换脏页先写回）};
\node[draw, rectangle, fill=SoftGray, minimum width=3.6cm, minimum height=0.55cm, below of=e] (f) {更新页表与 TLB，恢复现场};
\node[draw, rectangle, fill=SoftGray, minimum width=3.6cm, minimum height=0.55cm, below of=f] (g) {重新执行导致缺页的指令};
...
考题通常给出内存容量（页框数）和页面访问序列，要求模拟 LRU 置换过程，统计缺页次数。技巧：维护一个 LRU 栈（或访问顺序表），每次访问后将当前页面移到队尾（最近被访问的一端），队头即为最久未使用的页面。
...
\begin{center}
\begin{tikzpicture}[>=stealth, font=\scriptsize]
% 环形页框，每个页框标注其访问位（访问位=1 最近被访问，被指针清 0 后视为“第二次机会”）
\node[draw, rectangle, minimum width=1.05cm, minimum height=0.95cm, rounded corners=1pt, fill=SoftBlue, align=center] (F0) at (30:2.5cm) {\scriptsize $P0$\\[1pt]\scriptsize $b=0$};
\node[draw, rectangle, minimum width=1.05cm, minimum height=0.95cm, rounded corners=1pt, fill=SoftBlue, align=center] (F1) at (90:2.5cm) {\scriptsize $P1$\\[1pt]\scriptsize $b=1$};
\node[draw, rectangle, minimum width=1.05cm, minimum height=0.95cm, rounded corners=1pt, fill=SoftBlue, align=center] (F2) at (150:2.5cm) {\scriptsize $P2$\\[1pt]\scriptsize $b=1$};
\node[draw, rectangle, minimum width=1.05cm, minimum height=0.95cm, rounded corners=1pt, fill=SoftBlue, align=center] (F3) at (210:2.5cm) {\scriptsize $P3$\\[1pt]\scriptsize $b=0$};
\node[draw, rectangle, minimum width=1.05cm, minimum height=0.95cm, rounded corners=1pt, fill=SoftBlue, align=center] (F4) at (270:2.5cm) {\scriptsize $P4$\\[1pt]\scriptsize $b=1$};
\node[draw, rectangle, minimum width=1.05cm, minimum height=0.95cm, rounded corners=1pt, fill=SoftBlue, align=center] (F5) at (330:2.5cm) {\scriptsize $P5$\\[1pt]\scriptsize $b=0$};
% 环形链（顺时针）
\draw[->] (F0) -- (F1);
\draw[->] (F1) -- (F2);
\draw[->] (F2) -- (F3);
...
\draw[->] (F5) -- (F0);
% 时钟指针（指向当前候选页框）
\node[circle, fill=Amber, minimum size=0.3cm, inner sep=0pt] at (0,0) {};
\draw[->, very thick, Amber] (0,0) -- (F0);
\node[text=Amber, font=\scriptsize] at (105:3.7) {时钟指针};
% 顺时针方向标注
\draw[->, gray, dashed, thick] (75:3.35) arc[start angle=75, end angle=25, radius=3.35];
\node[text=gray, font=\scriptsize] at (55:3.95) {顺时针};
\node[font=\scriptsize, MutedText] at (0,-3.4) {指针指向候选页框 $P0$（$b=0$ 则置换；$b=1$ 则清 $0$ 后前移）};
\end{tikzpicture}
\end{center}
...
\begin{conclusion}{页面置换算法对比（同串同框）}{replacement-compare}
以访问串 $1,2,3,4,2,1,5,6,2,1,2,3,7,6,3,2,1,2,3,6$、4 个页框为例，四种算法的缺页次数对比：
\begin{tablebox}{四种页面置换算法缺页次数（4 页框）}
\centering
\begin{tabular}{|c|c|c|l|}
\hline
算法 & 缺页次数 & 缺页率 & 说明 \\
\hline
OPT & 8 & 40\% & 不可实现，理论最优标尺 \\
FIFO & 14 & 70\% & 可能出现 Belady 异常 \\
LRU & 10 & 50\% & 栈算法，近似 OPT \\
CLOCK & 14 & 70\% & 用访问位近似 LRU \\
\hline
\end{tabular}
\end{tablebox}
\textbf{结论：} (1) 缺页数 \textbf{OPT $\leqslant$ LRU $\leqslant$ FIFO、CLOCK}——LRU 明显优于 FIFO/CLOCK；(2) \textbf{OPT 与 LRU 是栈算法}，页框数增多时缺页数只减不增；FIFO 与基本 CLOCK 不是栈算法，理论上可能出现 Belady 异常。
\end{conclusion}
...
\ifshowsolutions\par\noindent\textbf{解析：} 缺页中断处理：确定页在外存位置$\to$分配页框$\to$磁盘I/O读入$\to$修改页表（有效位=1）。\boxed{\textbf{答案：}(D)}\else \answerarea \fi
...
\ifshowsolutions\par\noindent\textbf{解析：} LRU 淘汰最久未使用的页。4 个页框逐步模拟，缺页发生在第 1、2、3、4、7、8、12、13、14、17 次访问，对应缺页页号为
$1,2,3,4,5,6,3,7,6,1$，共 10 次，缺页率为 $10/20=50\%$。\boxed{\textbf{答案：}(B)}\else \answerarea \fi
...
\ifshowsolutions\par\noindent\textbf{解析：} 固定分配时页框数固定，若全局置换意味着可从其他进程"抢"页框——这与固定分配矛盾。\boxed{\textbf{答案：}(C)}\else \answerarea \fi
...
\ifshowsolutions\par\noindent\textbf{解析：} 改进CLOCK：优先淘汰未访问未修改(0,0)，其次未访问已修改(0,1)，再次访问未修改(1,0)，最后访问已修改(1,1)。\boxed{\textbf{答案：}(A)}\else \answerarea \fi
...
\ifshowsolutions\par\noindent\textbf{解析：} 虚拟存储器基于局部性原理（时间/空间局部性）。容量受CPU地址位数限制，而非主存+辅存。可分页/分段/段页式实现。\boxed{\textbf{答案：}(B)}\else \answerarea \fi
...
$15000\div 4096=3$（页号$=3$），偏移$=15000-3\times 4096=2712$。页框号$=7$，物理地址$=7\times 4096+2712=31384$。\else \answerarea \fi
...
(3) 仅给出 3 块时的结果，不能据此判断是否发生 Belady 异常。作为对照，对同一访问串用 FIFO 配置 4 块时缺页 14 次，少于 3 块时的 16 次，因此从 3 块增加到 4 块时\textbf{没有}出现 Belady 异常。FIFO 是出现 Belady 异常的典型算法，但并非每个访问串、每次增加页框都会出现；LRU 和 OPT 属于栈算法，不会出现该异常。\else \answerarea \fi
...
(2) 二级页表：一级$2^{10}=1\text{K}$项，共 $4\text{ KB}$；最多需要 $2^{10}$ 张二级页表，每张$2^{10}$项、大小为 $4\text{ KB}$，合计 $4\text{ MB}$。因此所有二级页表都调入时的最大空间为 $4\text{ MB}+4\text{ KB}$；地址空间稀疏时只调入实际需要的二级页表，实际占用可低于该上限。

(3) 0x12345: 页号$=0x12$。0x1234A: 页号同为$0x12$（同一页），TLB命中$\to$无需访问页表。\else \answerarea \fi
...
(3) TLB缺失：访问内存页表(1次)$\to$获取页框号$\to$访问内存取数据(1次)。共\textbf{2次访存}。\else \answerarea \fi
