...
\begin{center}
\begin{tikzpicture}[>=stealth, font=\scriptsize]
\node[draw, rectangle, fill=SoftGray, minimum width=1.6cm, minimum height=0.7cm] (p1) at (-5.2,2.4) {$p_1$};
\node[draw, rectangle, fill=SoftGray, minimum width=1.6cm, minimum height=0.7cm] (p2) at (-5.2,0.0) {$p_2$};
\node[draw, rectangle, fill=SoftGray, minimum width=1.6cm, minimum height=0.7cm] (d) at (-5.2,-2.4) {$d$};
\node[font=\scriptsize, text=Navy] at (-5.2,3.25) {逻辑地址};
\node[draw, rectangle, fill=SoftBlue, minimum width=5.4cm, minimum height=1.1cm, align=center] (dir) at (1.2,2.4) {页目录（一级页表）\\ 页目录项 $\to$ 二级页表基址};
\node[draw, rectangle, fill=SoftTeal, minimum width=5.4cm, minimum height=1.1cm, align=center] (pt) at (1.2,0.0) {二级页表\\ 页表项 $\to$ 页框号 $f$};
\node[draw, rectangle, fill=SoftAmber, minimum width=5.4cm, minimum height=1.1cm, align=center] (pa) at (1.2,-2.4) {物理地址 $=$ 页框号 $f$ $|$ 偏移 $d$};
\draw[->] (p1.east) -- node[above]{$p_1$ 索引} (dir.west);
\draw[->] (p2.east) -- node[above]{$p_2$ 索引} (pt.west);
\draw[->] (d.east) -- node[above]{偏移} (pa.west);
\draw[->] (dir.south) -- (pt.north);
\draw[->] (pt.south) -- (pa.north);
\end{tikzpicture}
\end{center}
