...
\begin{tikzpicture}[>=stealth, font=\scriptsize]
  % 逻辑地址：p1 | p2 | d
  \node[draw,rectangle,minimum width=1.5cm,minimum height=0.6cm] (p1) at (0,1.8) {$p_1$};
  \node[draw,rectangle,minimum width=1.5cm,minimum height=0.6cm] (p2) at (1.7,1.8) {$p_2$};
  \node[draw,rectangle,minimum width=1.5cm,minimum height=0.6cm] (d) at (3.4,1.8) {$d$};
  \node[font=\scriptsize] at (1.7,2.5) {逻辑地址（两级页号 $p_1,p_2$ + 偏移 $d$）};
  % 页目录
  \node[draw,rectangle,minimum width=2.9cm,minimum height=0.6cm,fill=SoftGray] (dir) at (0.85,0.1) {页目录（$p_1$）};
  % 二级页表
  \node[draw,rectangle,minimum width=2.9cm,minimum height=0.6cm,fill=SoftBlue] (pt) at (0.85,-1.7) {二级页表（$p_2$）};
  % 物理地址
  \node[draw,rectangle,minimum width=1.5cm,minimum height=0.6cm] (f) at (0,-3.4) {页框号 $f$};
  \node[draw,rectangle,minimum width=1.5cm,minimum height=0.6cm] (d2) at (1.7,-3.4) {偏移 $d$};
  \node[font=\scriptsize] at (1.7,-4.1) {物理地址};
  % 箭头
  \draw[->] (p1.south) -- (dir.north);
