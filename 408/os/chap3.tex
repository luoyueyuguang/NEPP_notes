...
\begin{center}
\begin{tikzpicture}[>=stealth, font=\scriptsize, node distance=0.9cm]
  % 逻辑地址：页号 + 偏移
  \node[draw,rectangle,minimum width=1.4cm,minimum height=0.7cm] (pn) {页号 $p_n$};
  \node[draw,rectangle,minimum width=1.4cm,minimum height=0.7cm,right of=pn,node distance=0pt] (d) {偏移 $d$};
  \node[above of=pn,node distance=0.45cm,align=center] {逻辑地址};
  % 页表
  \node[draw,rectangle,minimum width=2.0cm,minimum height=0.7cm,fill=SoftGray,below of=pn,node distance=1.4cm] (pt)
        {页表：$p_n \to f_n$};
  % 物理地址：页框号 + 偏移（不变）
  \node[draw,rectangle,minimum width=1.4cm,minimum height=0.7cm,below of=pt,node distance=1.4cm] (f) {页框号 $f_n$};
  \node[draw,rectangle,minimum width=1.4cm,minimum height=0.7cm,right of=f,node distance=0pt] (d2) {偏移 $d$};
  \node[below of=f,node distance=0.45cm,align=center] {物理地址};
  % 箭头
...
\begin{tikzpicture}[>=stealth, font=\scriptsize]
\foreach \i in {0,...,5} {
  \node[draw, circle, minimum size=0.9cm] (F\i) at ({60*\i+30}:1.9cm) {P\i};
}
\draw[->] (F0) -- (F1);
\draw[->] (F1) -- (F2);
\draw[->] (F2) -- (F3);
...
\draw[->] (F4) -- (F5);
\draw[->] (F5) -- (F0);
\draw[->, very thick, Amber] (0,0) -- (F2.west);
\node[font=\scriptsize, black!60] at (0,-2.7) {环形页框链 + 时钟指针（指向候选页框，访问位清 0 后前移）};
\end{tikzpicture}
\end{center}
