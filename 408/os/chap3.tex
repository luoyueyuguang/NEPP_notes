...
\item \textbf{（2011年·选择题·第29题）} 在缺页处理过程中，操作系统执行的操作可能是
\begin{center}
\begin{tabular}{ll}
(A) 修改页表 & (B) 磁盘I/O \\[6pt]
(C) 分配页框 & (D) 以上都是
...
\item \textbf{（2011年·选择题·第30题）} 在一个请求分页系统中，采用LRU页面置换算法时，假设一个作业的页面走向为$1,2,3,4,2,1,5,6,2,1,2,3,7,6,3,2,1,2,3,6$。当分配给该作业的物理块数为4时，缺页次数是
\begin{center}
\begin{tabular}{ll}
(A) 8 & (B) 10 \\[6pt]
(C) 12 & (D) 14
...
\item \textbf{（2014年·选择题·第30题）} 在请求分页系统中，页面分配策略与页面置换策略不能组合使用的是
\begin{center}
\begin{tabular}{ll}
...
\item \textbf{（2016年·选择题·第28题）} 某系统采用改进型CLOCK置换算法，页表项中A为访问位，M为修改位。将页面分为四类：(0,0) (0,1) (1,0) (1,1)，淘汰页面的优先次序为
\begin{center}
\begin{tabular}{ll}
...
\item \textbf{（2020年·选择题·第30题）} 下列关于虚拟存储器的叙述中，正确的是
\begin{center}
\begin{tabular}{ll}
...
\item \textbf{（2009年·综合题·第46题）} 某请求分页系统，进程P共有5页，访问串为$1,2,3,4,2,1,5,6,2,1,2,3,7,6,3,2,1,2,3,6$。若分配给P的物理块为3块。
\begin{enumerate}
...
\ifshowsolutions\par\noindent\textbf{解析：}
(1) FIFO(3块): 缺页$1,2,3,4,1,5,6,2,3,7,6,1,2,6$ — 14次。缺页率$=14/20=70\%$。

(2) LRU(3块): 缺页$1,2,3,4,1,5,6,2,3,7,6,3,2,1,6$ — 15次。缺页率$=15/20=75\%$。

(3) 本页访问串中3块时FIFO缺页14次，若增加到4块FIFO缺页反而可能更多——这就是Belady异常（FIFO独有，LRU和OPT无此现象）。\fi
