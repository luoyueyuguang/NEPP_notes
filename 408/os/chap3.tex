...
\begin{itemize}
    \item 编译时：编译器知道进程驻留在内存的\textbf{确切位置} → 生成\textbf{绝对代码}（Absolute Code）。
    \item 装入时（静态重定位）：编译器不知道确切位置 → 生成\textbf{可重定位代码}（Relocatable Code），装入时修改。
    \item 执行时（动态重定位）：进程可在执行期间从一内存段移到另一内存段 → 需要 MMU 硬件。
\end{itemize}
...
链接方式 & 时机 & 特点 \\
\hline
静态链接 & 程序装入前（编译后） & 所有库模块链接成完整可执行文件；\\
 &  & 占用磁盘和内存空间大，但运行快 \\
\hline
装入时动态链接 & 程序装入内存时 & 装入时链接所需模块；\\
 &  & 便于模块共享和更新 \\
\hline
运行时动态链接 & 程序执行过程中 & 用到时才链接（如 \texttt{dlopen}）；\\
 &  & 最灵活，节省内存；Linux \texttt{.so} 文件 \\
\hline
\end{tabular}
\end{tablebox}
\end{conclusion}
...
根据进程的实际需要，动态地从空闲内存中分配一块\textbf{大小恰好合适}的连续空间。初始时整个用户区是一个大空闲块；随着进程的装入和退出，内存中出现多个大小不等的空闲块和已分配块。
\end{mydef}
...
\hline
\textbf{最佳适应（BF）} & 选所有 $\ge n$ 的空闲块中 & 产生的外部碎片最小，但每次需遍历全链；\\
...
\textbf{首次适应（FF）通常是最快且综合性能最好的算法}——它不需要遍历整个空闲链。最佳适应（BF）虽然单次分配产生的碎片最小，但每次都找最接近的，剩余部分小到几乎无法再被利用，且需要遍历整条链。在实际 OS（如 Linux 早期版本）中，首次适应或其变种更常见。
\end{attention}
...
\textbf{常考题型：} 给出页面大小和逻辑地址，求页号和页内偏移 → 逻辑地址除以页面大小。
\end{attention}
...
典型考题流程：CPU 发出逻辑地址 → \textcircled{1} TLB 查找（命中/缺失）→ \textcircled{2} （若缺失）访问页表 → \textcircled{3} 得到物理地址 → \textcircled{4} 访问 Cache（命中/缺失）→ \textcircled{5} （若缺失）访问主存。\textbf{关键：理解每一步的先后依赖关系和各自的命中率。}
\end{attention}
...
分页中，页内偏移直接拼接在页框号后（二者位宽固定）；分段中，段内偏移与段基址是\textbf{相加}关系。此外，分段需要越界检查（$d < \text{段长}$），而分页不需要（因为所有页面大小相同，偏移不会越界）。
\end{attention}
...
\begin{itemize}
    \item 循环中的求和变量 → \textbf{时间局部性}（反复访问同一变量）；
    \item 遍历数组 \texttt{a[0..N-1]} → \textbf{空间局部性}（依次访问相邻元素）；
    \item 循环体中的指令 → \textbf{时间局部性}（反复执行同一条指令）\textbf{+ 空间局部性}（指令顺序存放）。
\end{itemize}
\end{attention}
...
\begin{enumerate}
    ...
    \item OS 判断该逻辑地址是否合法（属于该进程的地址空间）。若非法→终止进程；若合法→继续：
    \item 从空闲页框中分配一个（若无空闲→执行页面置换算法，选出换出页面）；
...
\[
\boxed{\text{EAT} = (1-p) \times t_{\text{mem}} + p \times t_{\text{pf}}}
\]

由于 $t_{\text{pf}} \gg t_{\text{mem}}$（数量级差 $10^5 \sim 10^6$），即使缺页率很低，也会显著影响性能。例如：$t_{\text{mem}} = 100\text{ ns}$，$t_{\text{pf}} = 8\text{ ms}$，缺页率 $p = 0.001\%$ 时：
\[
\text{EAT} \approx 0.99999 \times 100\text{ ns} + 0.00001 \times 8\text{ ms} \approx 180\text{ ns}
\]
性能下降近一倍。
\end{formula}
...
仅有\textbf{FIFO}算法可能出现 Belady 异常。OPT 和 LRU 是\textbf{栈类算法（Stack Algorithm）}——$n$ 个页框时的驻留集始终是 $n+1$ 个页框时驻留集的子集，因此不会出现缺页反而增加的情况。CLOCK 算法（LRU 的近似）也不会出现 Belady 异常。
\end{attention}
...
% --- CLOCK ---
\begin{conclusion}{CLOCK 置换算法（NRU）}{clock-algorithm}
\textbf{CLOCK 算法}（又称 NRU，Not Recently Used）是 LRU 的一种\textbf{近似实现}，几乎所有现代 OS 都采用 CLOCK 或其变种。它利用 PTE 中的\textbf{访问位（Reference/Accessed Bit）}，避免了 LRU 每次访问都要更新的开销。
...
\textbf{直观理解：} CLOCK 就像给每个页面「一次免死金牌」。如果最近被访问过（访问位 $=1$），则暂时放过（清 $0$），除非一\
轮下来仍无其他候选。
\end{conclusion}
...
\begin{itemize}
    ...
    \item PFF 策略：若进程缺页率太高 → 增加其页框数；若缺页率太低 → 减少其页框数。
    \item 408 常考判断：「系统频繁换页，CPU 利用率低」→ 系统发生了抖动，应\textbf{减少并发进程数}，而不是增加。
\end{itemize}
\end{attention}
...
\begin{enumerate}
    \item \textbf{地址变换计算}：给定逻辑地址和页面大小 → 求页号、页内偏移；查页表 → 求物理地址。\textbf{必考题}。
...
\item \textbf{（单项选择题）} 在分页系统中，逻辑地址长度为 32 位，页面大小为 4 KB，则页号占多少位？
\begin{center}
\begin{tabular}{ll}
(A) 12 位 & (B) 20 位 \\[4pt]
(C) 22 位 & (D) 32 位
...
\item \textbf{（单项选择题）} 以下关于分页和分段的说法，\textbf{错误}的是
\begin{center}
\begin{tabular}{ll}
(A) 分页对程序员透明，分段对程序员可见 \\[4pt]
(B) 分页按固定大小划分，分段按逻辑单元划分 \\[4pt]
(C) 分页会产生外部碎片，分段会产生内部碎片 \\[4pt]
(D) 分段有利于共享和保护，分页有利于消除外部碎片
\end{tabular}
\end{center}

\item \textbf{（单项选择题）} 在动态分区分配中，以下哪种算法会产生最多的外部碎片？
\begin{center}
\begin{tabular}{ll}
(A) 首次适应（FF） & (B) 最佳适应（BF） \\[4pt]
(C) 最坏适应（WF） & (D) 邻近适应（NF）
\end{tabular}
\end{center}

\item \textbf{（单项选择题）} 关于虚拟内存，以下说法正确的是
\begin{center}
\begin{tabular}{ll}
(A) 虚拟内存的容量由物理内存大小决定 \\[4pt]
(B) 请求分页系统中，进程必须一次性装入全部页面 \\[4pt]
(C) 局部性原理是虚拟内存实现的理论基础 \\[4pt]
(D) 缺页中断属于外中断
\end{tabular}
\end{center}

\item \textbf{（单项选择题）} 在改进型 CLOCK 置换算法中，具有以下哪种（访问位 $A$，修改位 $M$）组合的页面\textbf{最优先}被淘汰？
\begin{center}
\begin{tabular}{ll}
(A) $A=0, M=0$ & (B) $A=0, M=1$ \\[4pt]
(C) $A=1, M=0$ & (D) $A=1, M=1$
\end{tabular}
\end{center}

\item \textbf{（单项选择题）} Belady 异常可能出现在以下哪种页面置换算法中？
\begin{center}
\begin{tabular}{ll}
(A) LRU & (B) OPT \\[4pt]
(C) FIFO & (D) CLOCK
...
\item \textbf{（填空题）} 在分页系统中，每次访存（不考虑 TLB）需要 \underline{\qquad} 次内存访问；引入 TLB 后，若 TLB 命中，则每次访存需要 \underline{\qquad} 次内存访问。

\item \textbf{（填空题）} 动态分区分配中，\underline{\qquad} 算法从上次分配结束的位置开始查找第一个合适的空闲块。当内存中出现许多难以利用的微小空闲块时，称产生了\textbf{\underline{\qquad}}。

\item \textbf{（简答题）} 简述静态重定位和动态重定位的区别，并说明为什么现代 OS 需要动态重定位。

\item \textbf{（简答题）} 简述二级页表的工作原理及其比单级页表节省内存的原因。
...
\item \textbf{（真题风格，单项选择题）} 某计算机系统按字节编址，逻辑地址和物理地址均为 32 位，页面大小为 4 KB。某时刻进程的页表部分内容如下（未列出的页表项有效位均为 0）：

\begin{tablebox}{页表部分内容}
\centering
\begin{tabular}{|c|c|c|}
\hline
页号 & 页框号 & 有效位 \\
...
若 CPU 依次访问逻辑地址 2049 和 12288（十进制），则两次访存对应的物理地址分别是多少？
\begin{center}
\begin{tabular}{ll}
(A) 10241 和 28672 & (B) 10241 和 缺页 \\[4pt]
(C) 6145 和 28672 & (D) 6145 和 缺页
\end{tabular}
\end{center}

\item \textbf{（真题风格，综合题）} 某请求分页系统，页面大小 4 KB，某进程的页表如下（含访问位 $A$ 和修改位 $M$）：
...
问：(1) 逻辑地址 5000（十进制）对应的物理地址是多少？(2) 若此时发生缺页，采用改进型 CLOCK 算法选择置换页面，当前时钟指针指向 2 号页框（顺时针扫描），扫描过程中 $A$ 和 $M$ 位如何变化？最终淘汰哪个页面？

\item \textbf{（真题风格，综合题）} 某计算机系统的主存按字节编址，逻辑地址和物理地址均为 32 位，页面大小为 4 KB，采用二级页表的分页存储管理方式。逻辑地址格式为：一级页号（10 位）$|$ 二级页号（10 位）$|$ 页内偏移（12 位）。

问：(1) 一级页表（页目录）共有多少个表项？每个表项至少占多少字节（假设表项中只有页框号）？(2) 一个进程的页目录和二级页表共占多少页框（假设该进程只使用了页号为 $0\sim1023$ 的页面，即连续的前 1024 页）？(3) 计算该进程访问逻辑地址 \texttt{0x0040 3000} 时，一级页号和二级页号分别是多少（十六进制或十进制）？

\item \textbf{（真题风格，综合题）} 在一个请求分页系统中，系统为某进程分配了 3 个物理页框，页面访问序列为：
\[
3, 2, 1, 0, 3, 2, 4, 3, 2, 1, 0, 4
\]
初始时 3 个页框均为空。请分别采用 FIFO、LRU 和 OPT 算法，给出缺页中断次数，并说明是否有算法发生了 Belady 异常（若将页框数增加到 4 个，FIFO 的缺页次数是多少？）。

\item \textbf{（真题风格，综合题）} 某计算机系统采用请求分页虚拟存储管理，TLB 命中率为 $98\%$，Cache 命中率为 $95\%$。一次 TLB 访问时间为 $1\text{ ns}$，一次主存访问时间为 $100\text{ ns}$，一次磁盘访问时间为 $8\text{ ms}$（含缺页处理）。已知缺页率为 $0.01\%$。求：
\begin{enumerate}
    \item 若 TLB 命中，获得物理地址后访问 Cache 也命中，共需多少时间？
    \item 若 TLB 缺失但缺页不发生，Cache 也缺失，共需多少时间？
    \item 该系统的有效访存时间（EAT）是多少？
\end{enumerate}

\item \textbf{（真题风格，简答题）} 系统出现「抖动」时有哪些典型症状？导致抖动的根本原因是什么？有哪些方法可以预防或解除抖动？

\item \textbf{（真题风格，综合题）} 在动态分区分配中，假设空闲分区链的初始状态为一个大小为 1024 KB 的空闲块。依次有以下请求序列：
\begin{center}
\texttt{A 请求 200 KB} $\to$ \texttt{B 请求 300 KB} $\to$ \texttt{C 请求 150 KB} $\to$ \texttt{A 释放} $\to$ \texttt{D 请求 80 KB} $\to$ \texttt{E 请求 250 KB} $\to$ \texttt{B 释放}
\end{center}
请分别用首次适应（FF）和最佳适应（BF）算法画出每次分配和释放后的空闲分区链状态，并比较最终的空闲块分布。
...
\noindent\textbf{拓展阅读：}
\begin{itemize}
    \item OSTEP 第 13--22 章「内存虚拟化」——从地址空间、分页、TLB、分段到页面置换的完整讲授；第 21--22 章对交换机制、页面置换策略有深入讨论；
    \item 陈海波《现代操作系统：原理与实现》第 5 章「内存管理」——涵盖虚拟地址空间、物理内存管理、缺页处理与页面置换的现代 OS 视角，结合 ChCore 实验环境实现。
\end{itemize}
