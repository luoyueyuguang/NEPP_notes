...
(3) 页内偏移为 \texttt{0x789}，故物理地址为 $(\texttt{0x1A}\ll12)+\texttt{0x789}=\texttt{0x1A789}$。\fi
...
P2 请求 $(0,1,0,0)$ 后，Available$=(2,1,2,1)$，此时 P0 可先完成，随后其余进程均可完成，系统仍安全。\fi
...
\ifshowsolutions\par\noindent\textbf{答案：}
逐项模拟可得，4 个页框时 FIFO 缺页 14 次、LRU 缺页 10 次、按题设初始条件执行基本 CLOCK 缺页 14 次。FIFO 在 3 个页框时缺页 16 次，增加到 4 个页框后降为 14 次，因此该序列在 3$\to$4 页框的比较中没有出现 Belady 反常。注意 Belady 反常必须比较不同页框数，不能只凭某一个页框数下的结果判断。\fi
...
(3) 每转用时 $60/10000=6$ ms，平均旋转延迟为 3 ms；4 KB 占 8 个扇区，传输时间为 $(8/512)\times6=0.09375$ ms。平均访问时间约为 $4+3+0.09375=7.094$ ms。\fi
...
\end{lstlisting}
必须先等待对应类别计数，再进入互斥区取元素；否则消费者可能进入缓冲区后找不到所需奇偶性的元素。\fi
...
\end{lstlisting}
管理员置位后，新游客即使取得了 \texttt{capacity} 也会检查到关闭并归还容量；已入馆游客不需要取得 \texttt{state} 即可从出口离开。\fi
...
第 (3) 问必须按依赖顺序访问，调度算法不能重排尚未由上级索引解析出的请求。磁头移动量为 $|11-5|+|12-11|+|40-12|=6+1+28=35$ 个磁道。\fi
...
(4) TLB 缺失后硬件/软件开始页表遍历；发现对应一级页表项未指向有效二级页表时触发异常。OS 校验地址合法性，分配或调入二级页表页，建立相应页表项；若目标数据页也不在内存，还需分配页框并从文件/交换区调入或进行按需清零。随后更新 PTE、必要时更新/失效相关 TLB 项并重新执行引发异常的指令。最终页框为 \texttt{0x5678} 时，该页起始物理地址为 \texttt{0x5678000}。\fi
...
两个独立、均匀的 $0\sim999$ 磁道号之间的期望绝对差为 $(1000^2-1)/(3\times1000)\approx333.333$ 个磁道。C 比 A 多经过 2 个索引块，即多 2 段独立寻道，故期望时间差约为 $2\times333.333\times0.1=66.67$ ms。本结论依赖题设的独立均匀模型；有并发请求时 SSTF 的结果还取决于队列，不能仅由文件大小推出。\fi
...
(4) 当管道读端取走数据、释放出满足写入条件的空间时，内核唤醒等待写端；D 进入就绪队列，之后获得 CPU 才能继续写入。原子写入边界、非阻塞标志和信号处理等细节若影响结果，必须由题目另行说明。\fi
