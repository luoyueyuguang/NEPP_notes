...
\foreach \x in {0,2.5,5.0,7.5} \draw ({\x},-0.12) -- ({\x},0.12) node[below=2pt]{\scriptsize \x};
% Buffer0 条带
\node[text=TealDark, align=left] at (-1.6,2.2) {Buffer0};
\draw[fill=SoftBlue, draw=Navy] (0,1.7) rectangle (2.5,2.7) node[midway]{$T$};
\draw[fill=SoftGray, draw=RuleGray] (2.5,1.7) rectangle (3.5,2.7) node[midway]{$M$};
\draw[fill=SoftAmber, draw=Amber] (3.5,1.7) rectangle (6.0,2.7) node[midway]{$C$};
% Buffer1 条带（与 B0 相位错开：设备填 B1 时 CPU 在处理 B0）
\node[text=TealDark, align=left] at (-1.6,0.4) {Buffer1};
\draw[fill=SoftBlue, draw=Navy] (2.5,-0.1) rectangle (5.0,0.9) node[midway]{$T$};
\draw[fill=SoftGray, draw=RuleGray] (5.0,-0.1) rectangle (6.0,0.9) node[midway]{$M$};
\draw[fill=SoftAmber, draw=Amber] (6.0,-0.1) rectangle (8.5,0.9) node[midway]{$C$};
% 标注：B0 的处理与 B1 的输入并行
\draw[<->, MutedText, thick] (3.5,2.95) -- (5.0,2.95);
\node[text=MutedText] at (4.25,3.3) {B0 处理 与 B1 输入 并行};
...
\begin{center}
\begin{tikzpicture}[>=stealth, node distance=0.7cm, font=\scriptsize]
\node[draw, rectangle, fill=SoftTeal, minimum width=2.6cm, minimum height=0.75cm, align=center] (emq) at (-2.8,0) {空缓冲队列\\\scriptsize emq};
\node[draw, rectangle, fill=SoftAmber, minimum width=2.6cm, minimum height=0.75cm, align=center] (inq) at (2.8,1.3) {输入队列\\\scriptsize inq};
\node[draw, rectangle, fill=SoftBlue, minimum width=2.6cm, minimum height=0.75cm, align=center] (outq) at (2.8,-1.3) {输出队列\\\scriptsize outq};
\draw[<->] (emq.north east) -- node[above,font=\scriptsize]{收容输入/提取输入} (inq.west);
\draw[<->] (emq.south east) -- node[below,font=\scriptsize]{收容输出/提取输出} (outq.west);
\node[font=\scriptsize, black!70, align=center] at (5.9,1.3) {设备填入数据\\CPU 取出使用};
\node[font=\scriptsize, black!70, align=center] at (5.9,-1.3) {进程填入输出\\设备取出输出};
\node[font=\scriptsize, black!60] at (0,-2.55) {缓冲池：全体设备共享，动态分配};
\end{tikzpicture}
\end{center}
