...
\begin{center}
\begin{tikzpicture}[>=stealth, font=\scriptsize, x=1.0cm]
\draw[->] (0,0) -- (10.5,0) node[right] {$t$};
\foreach \x in {0,2.5,5.0,7.5} \draw ({\x},-0.12) -- ({\x},0.12) node[below=3pt]{\scriptsize \x};
\node[text=TealDark, align=left] at (-1.6,1.2) {Buffer1};
\draw[fill=SoftBlue, draw=Navy] (2.5,0.8) rectangle (5.0,1.6) node[midway]{$T$};
\draw[fill=SoftGray, draw=RuleGray] (5.0,0.8) rectangle (6.0,1.6) node[midway]{$M$};
\draw[fill=SoftAmber, draw=Amber] (6.0,0.8) rectangle (8.5,1.6) node[midway]{$C$};
\node[text=TealDark, align=left] at (-1.6,2.6) {Buffer0};
\draw[fill=SoftBlue, draw=Navy] (0,2.2) rectangle (2.5,3.0) node[midway]{$T$};
\draw[fill=SoftGray, draw=RuleGray] (2.5,2.2) rectangle (3.5,3.0) node[midway]{$M$};
\draw[fill=SoftAmber, draw=Amber] (3.5,2.2) rectangle (6.0,3.0) node[midway]{$C$};
\draw[<->, MutedText, thick] (3.5,3.25) -- (5.0,3.25);
\node[text=MutedText] at (4.25,3.6) {B0 的 $C$ 与 B1 的 $T$ 并行};
\node[text=Amber, font=\scriptsize] at (4.75,-1.1) {$T$：设备填入；$M$：搬移；$C$：CPU 处理};
\end{tikzpicture}
\end{center}
...
\begin{center}
\begin{tikzpicture}[>=stealth, font=\scriptsize]
\node[draw, rectangle, fill=SoftGray, minimum width=2.4cm, minimum height=0.9cm, align=center] (min) at (-6.0,2.6) {设备\\（输入）};
\node[draw, rectangle, fill=SoftAmber, minimum width=3.2cm, minimum height=0.9cm, align=center] (inq) at (0.0,2.6) {输入队列\\ inq};
\node[draw, rectangle, fill=SoftGray, minimum width=2.4cm, minimum height=0.9cm, align=center] (prd) at (6.0,2.6) {进程\\（读取）};
\node[draw, rectangle, fill=SoftTeal, minimum width=3.2cm, minimum height=0.9cm, align=center] (emq) at (0.0,0.0) {空缓冲队列\\ emq};
\node[draw, rectangle, fill=SoftGray, minimum width=2.4cm, minimum height=0.9cm, align=center] (pwr) at (-6.0,-2.6) {进程\\（写入）};
\node[draw, rectangle, fill=SoftBlue, minimum width=3.2cm, minimum height=0.9cm, align=center] (outq) at (0.0,-2.6) {输出队列\\ outq};
\node[draw, rectangle, fill=SoftGray, minimum width=2.4cm, minimum height=0.9cm, align=center] (mout) at (6.0,-2.6) {设备\\（输出）};
...
\draw[<->] (emq.north) -- node[right,font=\scriptsize]{收容/提取输入} (inq.south);
\draw[<->] (emq.south) -- node[right,font=\scriptsize]{收容/提取输出} (outq.north);
\node[font=\scriptsize, MutedText] at (0,-3.9) {缓冲池：全体设备共享，动态分配};
\end{tikzpicture}
\end{center}
