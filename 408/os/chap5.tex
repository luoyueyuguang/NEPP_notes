...
\begin{center}
\begin{tikzpicture}[>=stealth, node distance=0.7cm, font=\scriptsize]
\node[draw, rectangle, fill=SoftGray, minimum width=2.2cm, minimum height=0.6cm, align=center] (a) at (0,0) {程序直接控制\\（轮询）};
\node[draw, rectangle, fill=SoftBlue, minimum width=2.2cm, minimum height=0.6cm, align=center] (b) at (2.7,0) {中断驱动};
\node[draw, rectangle, fill=SoftTeal, minimum width=2.2cm, minimum height=0.6cm, align=center] (c) at (5.4,0) {DMA};
\node[draw, rectangle, fill=SoftAmber, minimum width=2.2cm, minimum height=0.6cm, align=center] (d) at (8.1,0) {I/O 通道};
\draw[->] (a) -- (b); \draw[->] (b) -- (c); \draw[->] (c) -- (d);
\node[font=\scriptsize, black!60] at (4.05,-1.0) {CPU 参与 I/O 的程度 $\searrow$，每次中断的数据粒度 $\nearrow$};
\end{tikzpicture}
\end{center}
...
\begin{itemize}
    ...
    \item 中断：CPU 可在设备工作期间执行其他指令，但每次中断有处理与现场切换开销。408 的简化模型常按一次中断传送一个字/字节分析；实际控制器也可能通过 FIFO 在一次服务中搬运多个数据单元。
\end{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{内存地址：}DMA 描述符通常使用物理地址或设备可见地址；传统简化模型常要求一段物理连续缓冲区，但现代 DMA 可用 scatter-gather 描述符或 IOMMU 映射非连续物理页。
\end{enumerate}
\end{attention}

\begin{formula}{DMA周期窃取时的CPU停顿分析}{dma-cycle-stealing}
DMA 对 CPU 的影响取决于 DMA 请求频率、每次传送占用的总线周期数以及 CPU 的总线请求频率。仅给出“CPU 每执行 $k$ 条指令访存一次”不足以唯一确定降速比。若额外假设每 $k$ 条指令恰有一次 CPU 访存、每次 DMA 恰挪用一个周期且二者严格交替，才可在该简化模型下计算比例：

\[
\text{CPU有效时间比例} = \frac{k}{k+1} \quad \text{（上述严格交替模型）}
\]
...
\node[draw, rectangle, fill=SoftGray, minimum width=9.5cm, minimum height=0.95cm, rounded corners=2pt] (U) at (0,4.5) {\textbf{用户层 I/O 软件}（\texttt{printf, scanf} 等库函数 + SPOOLing 系统）};
\node[draw, rectangle, fill=SoftBlue, minimum width=9.5cm, minimum height=0.95cm, rounded corners=2pt] (I) at (0,3.1) {\textbf{设备独立性软件}（命名、保护、阻塞、缓冲、分配）};
\node[draw, rectangle, fill=SoftTeal, minimum width=9.5cm, minimum height=0.95cm, rounded corners=2pt] (D) at (0,1.7) {\textbf{设备驱动程序}（设备寄存器操作，与硬件直接通信）};
\node[draw, rectangle, fill=SoftAmber, minimum width=9.5cm, minimum height=0.95cm, rounded corners=2pt] (H) at (0,0.3) {\textbf{中断处理程序}（保存现场、处理中断、恢复现场）};
\node[draw, rectangle, fill=SoftGray, minimum width=9.5cm, minimum height=0.8cm, rounded corners=2pt, very thick] (HW) at (0,-1.2) {硬件（设备控制器 + 设备本身）};
% 用户态/内核态分界
\draw[dashed, RuleGray] (-4.75,3.8) -- (4.75,3.8);
\node[fill=white, font=\scriptsize, text=MutedText, inner sep=1pt, rounded corners=1pt] at (-3.7,3.8) {用户态 $\to$ 内核态};
\node[font=\scriptsize, text=MutedText] at (-5.4,4.5) {用户态};
\node[font=\scriptsize, text=MutedText] at (-5.4,1.7) {内核态};
% 自顶向下的调用/请求流向
\draw[->] (U.south) -- (I.north);
\draw[->] (I.south) -- (D.north);
\draw[->] (D.south) -- (H.north);
\draw[->] (H.south) -- (HW.north);
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}[>=stealth, font=\scriptsize]
% 磁盘井区（输入井 + 输出井）
\draw[dashed, rounded corners=2pt, thick, draw=RuleGray] (4.95,-2.5) rectangle (7.95,2.5);
\node[font=\scriptsize, text=TealDark] at (6.45,2.15) {磁盘（SPOOLing 井）};
\node[draw, rectangle, minimum width=1.8cm, minimum height=0.7cm, fill=SoftBlue] (inw) at (6.45,1.35) {输入井};
\node[draw, rectangle, minimum width=1.8cm, minimum height=0.7cm, fill=SoftAmber] (outw) at (6.45,-1.35) {输出井};
% 设备（左）
\node[draw, rectangle, minimum width=1.8cm, minimum height=0.7cm, fill=SoftGray] (indev) at (0,1.35) {输入设备};
\node[draw, rectangle, minimum width=1.8cm, minimum height=0.7cm, fill=SoftGray] (outdev) at (0,-1.35) {输出设备};
% 守护进程（中）
\node[draw, rectangle, minimum width=2.0cm, minimum height=0.7cm, fill=SoftTeal] (spi) at (3.2,1.35) {输入进程 SAPi};
\node[draw, rectangle, minimum width=2.0cm, minimum height=0.7cm, fill=SoftTeal] (spo) at (3.2,-1.35) {输出进程 SAPo};
% 用户进程（右）
\node[draw, rectangle, minimum width=2.3cm, minimum height=2.0cm, fill=SoftBlue!45, align=center] (proc) at (10.3,0) {用户进程\\P1…Pn};
% 输入路径（左→右）
\draw[->] (indev) -- node[above]{预输入} (spi);
\draw[->] (spi) -- node[above]{写入} (inw);
\draw[->] (inw.east) -- node[above, pos=0.45]{进程读} (proc.west);
% 输出路径（右→左）
\draw[->] (proc.west) -- node[below, pos=0.45]{进程写} (outw.east);
\draw[->] (outw) -- node[below]{取出} (spo);
\draw[->] (spo) -- node[below]{缓输出} (outdev);
\end{tikzpicture}
\end{center}
\end{mydef}
...
\foreach \x in {0,2.5,5.0,7.5} \draw ({\x},-0.12) -- ({\x},0.12) node[below=3pt]{\scriptsize \x};
% Buffer0（上带）
\node[text=TealDark, anchor=east] at (-0.15,3.6) {Buffer0};
\draw[fill=SoftBlue, draw=Navy] (0,3.15) rectangle (2.5,4.05) node[midway]{$T$};
\draw[fill=SoftGray, draw=RuleGray] (2.5,3.15) rectangle (3.5,4.05) node[midway]{$M$};
\draw[fill=SoftAmber, draw=Amber] (3.5,3.15) rectangle (6.0,4.05) node[midway]{$C$};
% Buffer1（下带）
\node[text=TealDark, anchor=east] at (-0.15,1.0) {Buffer1};
\draw[fill=SoftBlue, draw=Navy] (2.5,0.55) rectangle (5.0,1.45) node[midway]{$T$};
\draw[fill=SoftGray, draw=RuleGray] (5.0,0.55) rectangle (6.0,1.45) node[midway]{$M$};
\draw[fill=SoftAmber, draw=Amber] (6.0,0.55) rectangle (8.5,1.45) node[midway]{$C$};
% 并行标注
\draw[<->, MutedText, thick] (3.5,2.4) -- (5.0,2.4);
\node[fill=white, font=\scriptsize, text=MutedText, inner sep=1pt, rounded corners=1pt] at (4.25,2.4) {B0 的 $C$ 与 B1 的 $T$ 并行};
% 图例
\node[text=Amber, font=\scriptsize] at (4.25,-1.0) {$T$：设备填入；$M$：搬移；$C$：CPU 处理};
\end{tikzpicture}
\end{center}
...
% 环形缓冲区
\foreach \i in {0,...,7} {
  \node[draw, circle, minimum size=0.85cm, fill=SoftBlue] (B\i) at ({45*\i+22.5}:2.5cm) {\scriptsize B\i};
}
% 环形链（顺时针方向）
\foreach \i in {0,...,7} {
...
% 中心指针
\node[circle, fill=RuleGray, minimum size=0.32cm, inner sep=0pt] at (0,0) {};
\draw[->, very thick, Amber] (0,0) -- (B2);
\node[text=Amber, font=\scriptsize] at (-1.1,3.0) {NextIn};
\draw[->, very thick, TealDark] (0,0) -- (B6);
\node[text=TealDark, font=\scriptsize] at (1.2,-3.0) {NextOut};
\node[font=\scriptsize, MutedText] at (0,-3.6) {NextIn：设备下一个填入的空缓冲区；NextOut：CPU 下一个取出的满缓冲区};
\end{tikzpicture}
\end{center}
...
\begin{center}
\begin{tikzpicture}[>=stealth, font=\scriptsize]
\node[draw, rectangle, fill=SoftGray, minimum width=2.2cm, minimum height=0.9cm, align=center] (min) at (-6.8,3.0) {设备\\（输入）};
\node[draw, rectangle, fill=SoftAmber, minimum width=3.6cm, minimum height=1.0cm, align=center] (inq) at (0,3.0) {输入队列\\ inq};
\node[draw, rectangle, fill=SoftGray, minimum width=2.2cm, minimum height=0.9cm, align=center] (prd) at (6.8,3.0) {进程\\（读取）};
\node[draw, rectangle, fill=SoftTeal, minimum width=3.6cm, minimum height=1.0cm, align=center] (emq) at (0,0.0) {空缓冲队列\\ emq};
\node[draw, rectangle, fill=SoftGray, minimum width=2.2cm, minimum height=0.9cm, align=center] (pwr) at (-6.8,-3.0) {进程\\（写入）};
\node[draw, rectangle, fill=SoftBlue, minimum width=3.6cm, minimum height=1.0cm, align=center] (outq) at (0,-3.0) {输出队列\\ outq};
\node[draw, rectangle, fill=SoftGray, minimum width=2.2cm, minimum height=0.9cm, align=center] (mout) at (6.8,-3.0) {设备\\（输出）};
% 输入/输出主路径
\draw[->] (min.east) -- node[above,font=\scriptsize]{填入输入} (inq.west);
\draw[->] (inq.east) -- node[above,font=\scriptsize]{进程取用} (prd.west);
\draw[->] (pwr.east) -- node[above,font=\scriptsize]{填入输出} (outq.west);
\draw[->] (outq.east) -- node[above,font=\scriptsize]{设备输出} (mout.west);
% emq 与 inq / outq 的收容与提取
\draw[<->] (emq.north) -- (inq.south);
\draw[<->] (emq.south) -- (outq.north);
\node[font=\scriptsize, text=MutedText, fill=white, inner sep=1pt, rounded corners=1pt] at (2.1,1.5) {收容/提取输入};
\node[font=\scriptsize, text=MutedText, fill=white, inner sep=1pt, rounded corners=1pt] at (2.1,-1.5) {收容/提取输出};
\node[font=\scriptsize, text=MutedText] at (0,-4.1) {缓冲池：全体设备共享，动态分配};
\end{tikzpicture}
\end{center}
...
\begin{center}
\begin{tikzpicture}[>=stealth, font=\scriptsize]
\node[font=\scriptsize, text=Navy, font=\bfseries] at (0,3.5) {I/O 管理常考点速查};
\node[draw, rectangle, fill=SoftBlue, minimum width=7.2cm, minimum height=2.0cm, align=center, rounded corners=2pt] (A) at (-4.1,1.4) {\textbf{四种 I/O 控制方式}\\ 轮询 $\to$ 中断 $\to$ DMA $\to$ 通道\\ {\scriptsize CPU 参与度、中断频率递减}};
\node[draw, rectangle, fill=SoftTeal, minimum width=7.2cm, minimum height=2.0cm, align=center, rounded corners=2pt] (B) at (4.1,1.4) {\textbf{I/O 软件四层}\\ 用户层 $\to$ 设备独立 $\to$ 驱动 $\to$ 中断\\ {\scriptsize SPOOLing 在用户层；驱动与硬件通信}};
\node[draw, rectangle, fill=SoftAmber, minimum width=7.2cm, minimum height=2.0cm, align=center, rounded corners=2pt] (C) at (-4.1,-1.6) {\textbf{缓冲区处理时间}\\ 单缓冲 $\max(C,T)+M$；双缓冲 $\max(T,M+C)$\\ {\scriptsize 缓冲池：全体设备共享}};
\node[draw, rectangle, fill=SoftGray, minimum width=7.2cm, minimum height=2.0cm, align=center, rounded corners=2pt] (D) at (4.1,-1.6) {\textbf{块设备 vs 字符设备}\\ 块设备可寻址（按块）；字符设备不可寻址\\ {\scriptsize 磁盘为块设备，打印机为字符设备}};
\end{tikzpicture}
\end{center}
\end{conclusion}
...
\ifshowsolutions\par\noindent\textbf{解析：} CPU/驱动先初始化 DMA 控制器并启动设备；DMA 控制器完成磁盘到主存的数据传送；完成后发出中断请求，CPU 再执行 DMA 完成中断服务程序。\boxed{\textbf{答案：}(B)}\else \answerarea \fi
...
\ifshowsolutions\par\noindent\textbf{解析：} 内存中的磁盘缓冲/缓存可暂存近期数据、合并或延后部分访问，从而减少实际设备 I/O，并缓和设备与 CPU 的速度差异。它不能直接缩短磁头的一次物理寻道。\boxed{\textbf{答案：}(A)}\else \answerarea \fi
...
\ifshowsolutions\par\noindent\textbf{解析：} 驱动程序必须了解控制器寄存器和程序查询、中断、DMA 等具体 I/O 控制方式，不能说与控制方式无关。其余叙述符合驱动负责设备初始化和提交具体 I/O 命令的职责。\boxed{\textbf{答案：}(A)}\else \answerarea \fi
...
\ifshowsolutions\par\noindent\textbf{答案：} 中断处理程序、设备驱动程序、设备独立性软件、用户层 I/O 软件；用户层 I/O 软件；逻辑；设备驱动程序。驱动向设备独立层接受规范化请求，再把它转换为控制器能够执行的设备特定命令。\else \answerarea \fi

\item \textbf{（简答题）} 简述 DMA 控制方式与中断驱动方式的核心区别（至少三点），并说明为什么 DMA 方式更适合高速块设备。

\ifshowsolutions\par\noindent\textbf{答案：} 中断驱动方式通常由 CPU 在每个字/小批数据就绪时进入中断处理并搬运或协调传送；DMA 由 CPU 一次设置主存地址、传送方向和计数后，由 DMA 控制器在设备与主存之间完成整块传送。DMA 占用总线/存储周期但不让数据逐字经过 CPU 通用寄存器，通常每块完成后才中断一次；因此 CPU 中断和指令开销更少、可与设备工作更充分重叠，适合磁盘等高速块设备。实际控制器也可能使用 FIFO 或突发传送，不能把中断方式绝对化为任何硬件都一次只传一个字节。\else \answerarea \fi
...
\ifshowsolutions\par\noindent\textbf{解析：} SPOOLing 使用磁盘输入/输出井和系统进程模拟共享独占设备；井与实际设备间的传送由系统控制，而不是由普通用户作业直接控制。\boxed{\textbf{答案：}(D)}\else \answerarea \fi
...
\ifshowsolutions\par\noindent\textbf{解析：} 用户通过系统调用进入内核，设备独立层/系统调用处理再调用驱动提交设备命令；设备完成后才异步进入中断处理程序。四个选项中 (A) 表示这一主流程。\boxed{\textbf{答案：}(A)}\else \answerarea \fi
...
\ifshowsolutions\par\noindent\textbf{解析：} 单缓冲总时间为
\[
T+M+C+(N-1)[\max(T,C)+M]
=100+5+90+(2-1)(100+5)=300.
\]
...
\ifshowsolutions\par\noindent\textbf{解析：}
\[
T_{\rm single}=100+50+50+9[\max(100,50)+50]=1550\ \mu\text{s},
\]
\[
T_{\rm double}=100+50+50+9\max(100,50+50)=1100\ \mu\text{s}.
\]
...
\begin{enumerate}
    ...
    \item[(2)] 若每个缓冲区扩大为 2 KB（每批容纳 2 个磁盘块），在双缓冲下，读入并处理完整个文件需要多长时间？假定各阶段耗时均与本批包含的块数成正比，因此每批 $T'=0.4$ ms、$M'=0.02$ ms、$C'=0.6$ ms，共100批。
    \item[(3)] 在上述“各阶段时间严格按数据量线性增长、没有固定启动开销”的模型下，比较两种缓冲大小的稳态吞吐率，并解释完整文件总时间的细微差别。
\end{enumerate}

\ifshowsolutions\par\noindent\textbf{答案：}
(1) 单缓冲：
\[
0.2+0.01+0.3+199[\max(0.2,0.3)+0.01]=62.20\text{ ms}.
\]
双缓冲：
\[
0.2+0.01+0.3+199\max(0.2,0.01+0.3)=62.20\text{ ms}.
\]
本组参数中 CPU 处理加内存搬移的 $0.31$ ms 正好成为两种流水线的瓶颈，因此结果相同。

(2) 每批2块、共100批：
\[
0.4+0.02+0.6+99\max(0.4,0.02+0.6)=62.40\text{ ms}.
\]

(3) 1 KB 情况稳态每块需要 $0.31$ ms；2 KB 情况每批需要 $0.62$ ms，即折合每块仍为 $0.31$ ms，所以稳态吞吐率相同。2 KB 配置的完整文件时间略长0.20 ms，只来自流水线启动/排空边界，不能据此声称大缓冲在这个纯线性模型下提高了稳态吞吐率。若要体现大块 I/O 减少固定寻道、命令或中断开销，题目还必须给出非零的每批固定开销。\else \answerarea \fi
...
\begin{itemize}
    ...
    \item 随后的综合题中，1 KB 单缓冲的稳态项为 $\max(C,T)+M=0.31$ ms，双缓冲为 $\max(T,M+C)=0.31$ ms，所以两者恰好同为 62.20 ms；2 KB 批量的稳态项为 $\max(0.4,0.62)=0.62$ ms/批。
\end{itemize}
