...
\hline
\multirow{2}{*}{按传输速率} & 低速设备 & 键盘、鼠标（每秒几个~几百字节）\\
                            & 中速设备 & 打印机（每秒数千~数万字节）\\
                            & 高速设备 & 磁盘（每秒数十万~数百万字节）\\
\hline
\multirow{2}{*}{按共享属性} & \textbf{独占设备} & 打印机（临界资源，需互斥使用）\\
...
\begin{itemize}
    ...
    \item \textbf{字符设备}：以字符流为单位传输，不可寻址，通常无缓存或仅有行缓冲。如键盘输入、串口通信。
\end{itemize}
\end{mydef}

\begin{attention}
\textbf{408 辨析：} 块设备 ≠ 独占设备。磁盘是块设备同时又是共享设备；打印机是字符设备同时又是独占设备。设备分类维度不同，不可混淆。\textbf{块设备的典型特征是可寻址（可以 lseek），字符设备不可寻址。}
\end{attention}
...
\begin{mydef}{程序直接控制}{programmed-io}
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item 中断处理程序：保存寄存器上下文 → 处理中断（读取设备状态，唤醒等待进程）→ 恢复上下文 → 中断返回。
...
\textbf{关键题眼：} ``向上提供统一接口，向下操作设备寄存器'' → 这是在描述 \textbf{设备驱动程序}。
\end{attention}
...
\begin{enumerate}
    ...
    \item \textbf{408 真题中常见的 SPOOLing 问题：}哪些设备不能通过 SPOOLing 实现共享？——只有独占设备（打印机、读卡机）才有必要用 SPOOLing；本身就可以共享的设备（磁盘）不需要 SPOOLing。
\end{enumerate}
\end{attention}
...
\textbf{原因分析：}
\begin{itemize}
    \item 若 $C \geqslant T$：设备读下一块数据到缓冲区时，CPU 正在处理上一块数据，两者可以并行。但因为只有一个缓冲区，设备写完后需等 CPU 处理完 + $M$ 搬移后才能释放缓冲区 → 瓶颈在 CPU。
    \item 若 $C < T$：CPU 处理完上一块后需要等设备读完下一块数据 → 瓶颈在设备。
    \item $M$ 与 $T$ 不能并行（因为 $M$ 需要访问缓冲区，和设备写缓冲区冲突），$M$ 与 $C$ 也不能并行（都要访问内存中的用户工作区）。
\end{itemize}

\textbf{处理 $N$ 块数据的总时间：}
\[
T_{\text{total}} = T + N \times [\max(C, T) + M]
\]
（第一块的 $T$ 没有前置处理可并行）
\end{conclusion}
...
\textbf{原因分析：}
\begin{itemize}
    \item \textbf{若 $C > T$（CPU 瓶颈）}：设备速率跟不上 CPU，CPU 偶尔需要等设备读完下一块。每块处理时间 $\approx C$。
    \item \textbf{若 $C < T$（设备瓶颈）}：设备速率跟不上 CPU 处理，CPU 处理完一块后马上可以处理下一块（另一缓冲区已就绪）。每块处理时间 $\approx T$。
    \item $M$可以在 $T$ 完成后与下一个$T$重叠（因为缓冲区已交换）。
\end{itemize}
...
T_{\text{total}} = T + N \times \max(C, T) + M
\]
（第一个 $T$ 是冷启动，最后的 $M$ 是最后一块的搬移，中间每一块的时间为 $\max(C, T)$）
\end{conclusion}
...
\textbf{单缓冲：} $T_{\text{total}} = N \times [\max(C, T) + M] + T$（首块多一个 $T$，每块瓶颈为 $\max(C,T)+M$）

\textbf{双缓冲：} $T_{\text{total}} = N \times \max(C, T) + T + M$（稳态每块 $\max(C,T)$，首尾各有开销）

\textbf{陷阱：} 如果题目没有给出 $M$（系统搬移时间），默认 $M=0$ 或 $M$ 已包含在 $C$ 或 $T$ 中。此时：
\begin{itemize}
    \item 单缓冲每块时间 = $\max(C, T)$（$M=0$时两者公式一致...）——等等，此时单双缓冲的区别在于并行度。
    \item 实际上当 $M$ 忽略不计时，处理 $N$ 块：
    \begin{itemize}
        \item 单缓冲总时间 $= N \times \max(C, T) + T$
        \item 双缓冲总时间 $= N \times \max(C, T) + T$
    \end{itemize}
    这在 $C$ 和 $T$ 两者中一个独占主导时确实相同；但当 $C$ 和 $T$ 比较接近时，双缓冲的并行优势才显现，此时双缓冲 $\approx N \times \max(C, T)$，而单缓冲中 $M$ 不可并行。
\end{itemize}
\end{minipage}}
\end{attention}
...
\textbf{缓冲池的操作原语：}
\begin{itemize}
    \item \textbf{收容输入（GetBuf\_Input）}：从 emq 取一个空缓冲区 → 挂入 inq。
    \item \textbf{提取输入（ReleaseBuf\_Input）}：从 inq 取一个满缓冲区 → 供进程使用 → 回收后挂入 emq。
    \item \textbf{收容输出（GetBuf\_Output）}：从 emq 取一个空缓冲区 → 挂入 outq。
    \item \textbf{提取输出（ReleaseBuf\_Output）}：从 outq 取一个满缓冲区 → 供设备输出 → 回收后挂入 emq。
\end{itemize}
...
\begin{itemize}
    \item \textbf{单缓冲}：一个缓冲区，设备和 CPU 串行使用。适合简单场景，处理每块时间 $= \max(C, T) + M$。
    \item \textbf{双缓冲}：两个缓冲区，设备和 CPU 可交替使用。处理每块时间 $= \max(C, T)$。适合连续 I/O（如磁盘顺序读）。
...
\begin{enumerate}
    \item \textbf{四种 I/O 控制方式}：轮询→中断→DMA→通道，每次演进减少了 CPU 参与 I/O 的程度和中断频率。DMA 与中断的最关键区别：\textbf{DMA 的数据传输不由 CPU 完成，且每块只中断一次}。

    \item \textbf{I/O 软件层次}：四层自底向上依次是中断处理程序→设备驱动程序→设备独立性软件→用户层 I/O 软件。SPOOLing 属于用户层（守护进程形式）。

    \item \textbf{SPOOLing 系统}：独占设备→共享设备的虚拟化技术。输入/输出井\textbf{在磁盘上}。原理：截获输出请求写入磁盘井，后台守护进程逐个送往物理设备。

    \item \textbf{缓冲区处理时间计算}：
    \begin{itemize}
        \item 单缓冲每块时间 $= \max(C, T) + M$；$N$ 块总时间 $= T + N \times [\max(C, T) + M]$
        \item 双缓冲每块时间 $= \max(C, T)$；$N$ 块总时间 $= T + N \times \max(C, T) + M$
    \end{itemize}
...
\item \textbf{（单项选择题）} 以下设备中，属于块设备的是
\begin{center}
\begin{tabular}{ll}
(A) 键盘 & (B) 鼠标 \\[6pt]
(C) 磁盘 & (D) 打印机
...
\item \textbf{（单项选择题）} 在 I/O 控制方式中，由 DMA 控制器直接完成数据传输，CPU 仅在传输开始和结束时参与。DMA 方式每传送完一整块数据后，通过以下哪种方式通知 CPU？
\begin{center}
\begin{tabular}{ll}
(A) 由 CPU 不断轮询 DMA 控制器的状态寄存器 & (B) DMA 控制器向 CPU 发出一次中断 \\[6pt]
(C) 由驱动程序定期检查 DREQ 信号 & (D) 通过修改内存中的标志位
\end{tabular}
\end{center}

\item \textbf{（单项选择题）} SPOOLing 系统中，输入井和输出井位于
\begin{center}
\begin{tabular}{ll}
(A) 内存 & (B) 磁盘 \\[6pt]
(C) 设备控制器的缓冲区 & (D) CPU 的 Cache
\end{tabular}
\end{center}

\item \textbf{（填空题）} I/O 软件的四个层次自底向上依次为：\underline{\qquad}、\underline{\qquad}、\underline{\qquad} 和 \underline{\qquad}。其中，SPOOLing 系统属于 \underline{\qquad} 层。

\item \textbf{（简答题）} 简述 DMA 控制方式与中断驱动方式的核心区别（至少三点）。

\item \textbf{（计算题）} 设磁盘块大小为 2 KB，缓冲区大小等于磁盘块大小。磁盘 I/O 传输一块数据的时间 $T = 0.1$ ms，系统将缓冲区内数据传送到用户工作区的时间 $M = 0.02$ ms，CPU 处理一块数据的时间 $C = 0.05$ ms。现有 100 块数据需要读入并处理，分别使用单缓冲和双缓冲方式，计算总处理时间。
...
\item \textbf{（真题风格，单项选择题）} 下列关于 SPOOLing 技术的叙述中，\textbf{错误}的是
\begin{center}
\begin{tabular}{ll}
(A) SPOOLing 技术可以将独占设备改造为共享设备 \\[4pt]
(B) SPOOLing 输出并位于磁盘上，用于暂存用户进程的输出数据 \\[4pt]
(C) SPOOLing 系统中的输出进程负责将数据从磁盘输出井送至物理设备 \\[4pt]
(D) SPOOLing 技术可以用于加速磁盘 I/O 的性能
\end{tabular}
\end{center}

\item \textbf{（真题风格，单项选择题）} 某系统采用双缓冲技术实现磁盘 I/O。设磁盘将一块数据读入缓冲区的时间为 $T$，操作系统将缓冲区数据传送到用户区的时间为 $M$，CPU 处理一块数据的时间为 $C$。若 $T = 80 \; \mu\text{s}$，$C = 100 \; \mu\text{s}$，$M = 10 \; \mu\text{s}$，则系统处理完一块数据的平均时间约为
\begin{center}
\begin{tabular}{ll}
(A) $100 \; \mu\text{s}$ & (B) $110 \; \mu\text{s}$ \\[6pt]
(C) $130 \; \mu\text{s}$ & (D) $180 \; \mu\text{s}$
\end{tabular}
\end{center}
...
\item \textbf{（真题风格，综合题）} 一个 SPOOLing 打印系统，由三个用户进程 P1、P2、P3 同时请求打印。各进程打印数据量分别为 100 KB、50 KB 和 200 KB，打印机输出速率为 100 KB/s。不考虑磁盘读写时间。

\begin{enumerate}
    \item[(1)] 若三个进程同时发出打印请求，在没有 SPOOLing 的情况下（即进程独占打印机），从第一个进程开始打印到所有进程打印完成需要多长时间？
    \item[(2)] 在 SPOOLing 系统下，三个用户进程从发出打印请求到各自返回（即 ``感觉'' 打印完成）分别需要多长时间？（设输出井写入速率为无限大——不阻塞）
    \item[(3)] 打印守护进程从开始打印到所有文件打印完成需要多少时间？
    \item[(4)] 对比 (1) 和 (2) 的结果，说明 SPOOLing 如何改善了用户体验。
\end{enumerate}
