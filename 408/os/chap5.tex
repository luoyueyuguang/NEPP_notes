...
\item \textbf{（2017年·选择题·第32题）} 系统将数据从磁盘读到内存的过程包括以下操作：
\begin{center}
\begin{tabular}{l}
① DMA 控制器发出中断请求 \\
② 初始化 DMA 控制器并启动磁盘 \\
...
\item \textbf{（2015年·选择题·第28题）} 在系统内存中设置磁盘缓冲区的主要目的是
\begin{center}
\begin{tabular}{ll}
...
\item \textbf{（2022年·选择题·第32题）} 下列关于驱动程序的叙述中，\textbf{不正确}的是
\begin{center}
\begin{tabular}{ll}
...
\item \textbf{（2016年·选择题·第31题）} 下列关于 SPOOLing 技术的叙述中，\textbf{错误}的是
\begin{center}
\begin{tabular}{ll}
...
\item \textbf{（2011年·选择题·第26题）} 用户程序发出磁盘 I/O 请求后，系统的正确处理流程是
\begin{center}
\begin{tabular}{ll}
(A) 用户程序→系统调用处理程序→设备驱动程序→中断处理程序 \\[4pt]
(B) 用户程序→系统调用处理程序→中断处理程序→设备驱动程序 \\[4pt]
(C) 用户程序→设备驱动程序→系统调用处理程序→中断处理程序 \\[4pt]
(D) 用户程序→设备驱动程序→中断处理程序→系统调用处理程序
\end{tabular}
\end{center}

\item \textbf{（2013年·选择题·第27题）} 设系统缓冲区和用户工作区均采用单缓冲，从外设读入 1 个数据块到系统缓冲区的时间为 100，从系统缓冲区读入 1 个数据块到用户工作区的时间为 5，对用户工作区中的 1 个数据块进行分析的时间为 90。进程从外设读入并分析 2 个数据块的最短时间是
\begin{center}
\begin{tabular}{ll}
(A) 200 & (B) 295 \\[6pt]
(C) 300 & (D) 390
...
\item \textbf{（2011年·选择题·第31题）} 某文件占 10 个磁盘块，现要把该文件磁盘块逐个读入主存缓冲区，并送用户区进行分析。假设一个缓冲区与一个磁盘块大小相同，把一个磁盘块读入缓冲区的时间为 100 $\mu\text{s}$，将缓冲区的数据传送到用户区的时间是 50 $\mu\text{s}$，CPU 对一块数据进行分析的时间为 50 $\mu\text{s}$。在单缓冲区和双缓冲区结构下，读入并分析完该文件的时间分别是
\begin{center}
\begin{tabular}{ll}
...
\noindent\textbf{解题提示：}
\begin{itemize}
    \item 第 8 题考查单/双缓冲时间计算的核心公式——单缓冲每块时间 $= \max(C,T) + M$，双缓冲每块时间 $= \max(C, T)$。注意首尾边界（第一块需先填满缓冲区，最后一块需处理完 $M$ 和 $C$）。
    \item 第 9 题双缓冲下 $\max(T, C) = \max(0.2, 0.3) = 0.3$ ms，总时间 $= T + 199 \times \max(T,C) + M + C$（注意第一块 $T$ 和最后一块 $M+C$ 的边界处理）。
\end{itemize}
