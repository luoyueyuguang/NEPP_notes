...
\ifshowsolutions\par\noindent\textbf{解析：} CPU/驱动先初始化 DMA 控制器并启动设备；DMA 控制器完成磁盘到主存的数据传送；完成后发出中断请求，CPU 再执行 DMA 完成中断服务程序。\boxed{\textbf{答案：}(B)}\fi
...
\ifshowsolutions\par\noindent\textbf{解析：} 内存中的磁盘缓冲/缓存可暂存近期数据、合并或延后部分访问，从而减少实际设备 I/O，并缓和设备与 CPU 的速度差异。它不能直接缩短磁头的一次物理寻道。\boxed{\textbf{答案：}(A)}\fi
...
\ifshowsolutions\par\noindent\textbf{解析：} 驱动程序必须了解控制器寄存器和程序查询、中断、DMA 等具体 I/O 控制方式，不能说与控制方式无关。其余叙述符合驱动负责设备初始化和提交具体 I/O 命令的职责。\boxed{\textbf{答案：}(A)}\fi
...
\ifshowsolutions\par\noindent\textbf{答案：} 中断处理程序、设备驱动程序、设备独立性软件、用户层 I/O 软件；用户层 I/O 软件；逻辑；设备驱动程序。驱动向设备独立层接受规范化请求，再把它转换为控制器能够执行的设备特定命令。\fi

\item \textbf{（简答题）} 简述 DMA 控制方式与中断驱动方式的核心区别（至少三点），并说明为什么 DMA 方式更适合高速块设备。

\ifshowsolutions\par\noindent\textbf{答案：} 中断驱动方式通常由 CPU 在每个字/小批数据就绪时进入中断处理并搬运或协调传送；DMA 由 CPU 一次设置主存地址、传送方向和计数后，由 DMA 控制器在设备与主存之间完成整块传送。DMA 占用总线/存储周期但不让数据逐字经过 CPU 通用寄存器，通常每块完成后才中断一次；因此 CPU 中断和指令开销更少、可与设备工作更充分重叠，适合磁盘等高速块设备。实际控制器也可能使用 FIFO 或突发传送，不能把中断方式绝对化为任何硬件都一次只传一个字节。\fi
...
\ifshowsolutions\par\noindent\textbf{解析：} SPOOLing 使用磁盘输入/输出井和系统进程模拟共享独占设备；井与实际设备间的传送由系统控制，而不是由普通用户作业直接控制。\boxed{\textbf{答案：}(D)}\fi
...
\ifshowsolutions\par\noindent\textbf{解析：} 用户通过系统调用进入内核，设备独立层/系统调用处理再调用驱动提交设备命令；设备完成后才异步进入中断处理程序。四个选项中 (A) 表示这一主流程。\boxed{\textbf{答案：}(A)}\fi
...
(3) 1 KB 情况稳态每块需要 $0.31$ ms；2 KB 情况每批需要 $0.62$ ms，即折合每块仍为 $0.31$ ms，所以稳态吞吐率相同。2 KB 配置的完整文件时间略长0.20 ms，只来自流水线启动/排空边界，不能据此声称大缓冲在这个纯线性模型下提高了稳态吞吐率。若要体现大块 I/O 减少固定寻道、命令或中断开销，题目还必须给出非零的每批固定开销。\fi
