...
\item \textbf{（真题风格·选择题）} 下列选项中，导致创建新进程的操作是
\begin{center}
\begin{tabular}{ll}
(A) 用户登录成功 & (B) 设备分配 \\[6pt]
(C) 启动程序执行 & (D) 以上都是
...
\ifshowsolutions\par\noindent\textbf{解析：} 资源分配图：$P_1$持有$R_1$等$R_2$，$P_2$持有$R_2$等$R_3$，$P_3$持有$R_3$等$R_1$——形成环$P_1\to P_2\to P_3\to P_1$，涉及3个进程。$P_4$只需$R_2$，可先分配不参与死锁。\boxed{\textbf{答案：}(C)}\fi
...
\ifshowsolutions\par\noindent\textbf{解析：} SJF可能使长作业饥饿，优先级调度可能使低优先级进程饥饿。普通多级反馈队列若没有周期性优先级提升等额外机制，CPU密集型进程也可能长期得不到运行；题干没有给出这类防饥饿机制。RR按就绪队列循环分配时间片，只要时间片有限且调度持续进行，每个就绪进程都会获得CPU。\boxed{\textbf{答案：}(C)}\fi
...
\end{tabular}
\end{center}
\ifshowsolutions\par\noindent\textbf{解析：}线程各自拥有 PC、寄存器和栈，但共享进程的地址空间。\boxed{\textbf{答案：}(C)}\fi
...
\end{tabular}
\end{center}
\ifshowsolutions\par\noindent\textbf{解析：}内核只看到一个执行实体，阻塞系统调用会使承载这些用户线程的整个进程阻塞。\boxed{\textbf{答案：}(B)}\fi
...
// P3类似P2，将odd改为even
\end{lstlisting}
四个信号量：\texttt{mutex}实现缓冲区互斥访问；\texttt{empty}表示空位数（初值$N$）；\texttt{odd/even}分别表示缓冲区中奇数/偶数个数。\fi
...
安全序列：P1$\to$P3$\to$P0$\to$P2$\to$P4。系统安全。\fi
