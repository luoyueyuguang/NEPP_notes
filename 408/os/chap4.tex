...
% ===== 索引分配 =====
\node[anchor=west, text=Burgundy] at (0,-1.4) {\textbf{索引分配}};
\node[draw, rectangle, minimum width=1.4cm, minimum height=0.65cm, fill=SoftAmber] (idx) at (2.6,-1.4) {索引块};
\foreach \i/\x in {0/4.5,1/5.6,2/6.7} {
  \node[draw, rectangle, minimum width=0.95cm, minimum height=0.55cm, fill=SoftAmber!55] (d\i) at (\x,-1.4) {块\i};
}
\draw[->] (idx) -- (d0);
\draw[->] (idx) -- (d1);
\draw[->] (idx) -- (d2);
\node[anchor=west, align=left] at (8.0,-1.4) {\scriptsize 索引块集中存数据\\[1pt]块号，支持直接访问};
\end{tikzpicture}
\end{center}
