...
\node[draw, rectangle, fill=SoftGray, minimum width=2.0cm, minimum height=2.6cm, align=center] (ino) at (0,0) {\textbf{i-node}};
\node[draw, rectangle, fill=SoftBlue, minimum width=3.4cm, minimum height=0.5cm, align=center] (dA) at (3.8,1.1) {直接块（$N$ 个）$\to$ 数据块};
\node[draw, rectangle, fill=SoftTeal, minimum width=3.4cm, minimum height=0.5cm, align=center] (sA) at (3.8,0.2) {一级间接 $\to$ 索引块 $\to$ 数据块};
\node[draw, rectangle, fill=SoftAmber, minimum width=3.4cm, minimum height=0.5cm, align=center] (d0) at (3.8,-0.7) {二级间接 $\to$ 索引块 $\to$ 一级间接块 $\to$ 数据块};
\node[draw, rectangle, fill=SoftAmber!45, minimum width=3.4cm, minimum height=0.5cm, align=center] (tA) at (3.8,-1.6) {三级间接 $\to$ 索引块 $\to$ 二级/一级 $\to$ 数据块};
\draw[->] (ino.east) -- (dA.west);
\draw[->] (ino.east) -- (sA.west);
\draw[->] (ino.east) -- (d0.west);
\draw[->] (ino.east) -- (tA.west);
\node[font=\scriptsize, black!60] at (2.4,-2.4) {混合索引：小文件用直接块，大文件逐层加间接索引};
\end{tikzpicture}
\end{center}
