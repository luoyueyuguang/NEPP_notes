...
\begin{tikzpicture}[>=stealth, font=\scriptsize]
  \node[draw,rectangle,fill=SoftGray,minimum width=1.8cm,minimum height=3.0cm,align=center] (cpu) at (-5.0,0){CPU\\\scriptsize 主设备};
  \node[draw,rectangle,fill=SoftGray,minimum width=2.2cm,minimum height=3.0cm,align=center] (mio) at (5.0,0){主存 / I/O 接口\\\scriptsize 从设备};
  \draw[Navy!80,very thick,->] (-4.1,1.0) -- (3.9,1.0) node[midway,above,font=\scriptsize]{地址总线（单向：CPU$\to$主存/I/O）};
  \draw[Teal!80,very thick,<->] (-4.1,0) -- (3.9,0) node[midway,above,font=\scriptsize]{数据总线（双向）};
  \draw[Burgundy!80,thick,->] (-4.1,-0.9) -- (3.9,-0.9);
  \draw[Burgundy!80,thick,<-] (-4.1,-1.15) -- (3.9,-1.15);
  \draw[Burgundy!80,thick,->] (-4.1,-1.4) -- (3.9,-1.4);
  \node[font=\scriptsize,MutedText] at (-0.1,-0.66){控制总线（各线方向固定但不统一）};
  \node[font=\scriptsize,MutedText,align=center] at (0,-1.95){读/写命令（CPU$\to$设备）$\cdot$ 中断请求（设备$\to$CPU）$\cdot$ 时钟（CPU$\to$设备）$\cdot$ \ldots};
\end{tikzpicture}
\end{center}
...
\begin{center}
\begin{tikzpicture}[>=stealth, font=\scriptsize]
  \draw[thick,Navy!70] (-5.4,0) -- (5.4,0);
  \node[font=\scriptsize,black!70] at (0,0.3){系统总线};
  \node[draw,rounded corners=2pt,fill=SoftBlue,minimum width=1.7cm,minimum height=0.85cm] (cpu) at (-3.6,1.25){CPU};
  \node[draw,rounded corners=2pt,fill=SoftTeal,minimum width=1.7cm,minimum height=0.85cm] (mem) at (0.0,1.25){主存};
  \node[draw,rounded corners=2pt,fill=SoftGray,minimum width=1.8cm,minimum height=0.85cm] (io) at (3.6,1.25){I/O 设备};
  \draw[Navy!70] (cpu.south) -- (-3.6,0);
  \draw[Navy!70] (mem.south) -- (0.0,0);
  \draw[Navy!70] (io.south) -- (3.6,0);
  \node[font=\scriptsize,black!70,align=center] at (0,-0.75){CPU、主存与所有 I/O 设备共享同一总线 $\to$ 总线成为瓶颈，高低速设备竞争};
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}[>=stealth, font=\scriptsize]
  % 局部总线（高速）
  \draw[thick,Navy!70] (-4.8,2.0) -- (4.8,2.0);
  \node[font=\scriptsize,black!70] at (4.8,2.24){局部总线（最高频）};
  \node[draw,rounded corners=2pt,fill=SoftBlue,minimum width=1.7cm,minimum height=0.72cm] (cpu) at (-3.2,2.0){CPU};
  \node[draw,rounded corners=2pt,fill=SoftTeal,minimum width=1.7cm,minimum height=0.72cm] (cache) at (-0.6,2.0){Cache};
  \draw[Navy!70] (cpu.south) -- (-3.2,2.0);
  % 存储总线
  \draw[thick,Navy!70] (-4.8,0.6) -- (4.8,0.6);
  \node[font=\scriptsize,black!70] at (4.8,0.84){存储总线};
  \node[draw,rounded corners=2pt,fill=SoftTeal,minimum width=1.9cm,minimum height=0.72cm] (ch2) at (0.4,0.6){主存 \& Cache 通路};
  % 扩展总线
  \draw[thick,Navy!70] (-4.8,-0.8) -- (4.8,-0.8);
  \node[font=\scriptsize,black!70] at (4.8,-0.56){扩展总线（I/O）};
  \node[draw,rounded corners=2pt,fill=SoftGray,minimum width=1.9cm,minimum height=0.72cm] (io) at (0.4,-0.8){各类 I/O 设备};
  \draw[dashed,Teal!70] (cache.south) -- (0.4,0.6);
  \draw[dashed,Burgundy!70] (0.4,0.6) -- (0.4,-0.8);
  \node[font=\scriptsize,black!70,align=center] at (0,-1.7){三级总线分层，速度由快到慢：局部总线 $\to$ 存储总线 $\to$ 扩展总线};
\end{tikzpicture}
\end{center}
...
\begin{center}
\begin{tikzpicture}[>=stealth, font=\scriptsize]
  \node[draw,rounded corners=2pt,fill=SoftAmber,minimum width=2.0cm,minimum height=0.8cm] (arb) at (-3.8,1.7){仲裁器};
  \node[draw,rounded corners=2pt,fill=SoftGray,minimum width=1.6cm,minimum height=0.7cm] (d0) at (-1.1,1.7){设备 0};
  \node[draw,rounded corners=2pt,fill=SoftGray,minimum width=1.6cm,minimum height=0.7cm] (d1) at (1.5,1.7){设备 1};
  \node[draw,rounded corners=2pt,fill=SoftGray,minimum width=1.6cm,minimum height=0.7cm] (d2) at (4.1,1.7){设备 2};
  \draw[->,Burgundy!70] (arb.east) -- node[above,font=\scriptsize]{BG 允许} (d0.west);
  \draw[->,Burgundy!70] (d0.east) -- (d1.west);
  \draw[->,Burgundy!70] (d1.east) -- (d2.west);
  \draw[thick,Navy!70] (-3.8,-0.2) -- (4.1,-0.2);
  \node[font=\scriptsize,black!70] at (0.1,0.08){总线请求 BR（所有设备线或共享送到仲裁器）};
  \draw[Navy!70] (arb.south) -- (-3.8,-0.2);
  \draw[Navy!70] (d0.south) -- (-1.1,-0.2);
  \draw[Navy!70] (d1.south) -- (1.5,-0.2);
  \draw[Navy!70] (d2.south) -- (4.1,-0.2);
  \draw[thick,Navy!70] (-3.8,-1.15) -- (4.1,-1.15);
  \node[font=\scriptsize,black!70] at (0.1,-0.87){总线忙 BS};
  \node[font=\scriptsize,black!70,align=center] at (0.1,-1.8){BG 串行传递：离仲裁器最近的设备优先级最高，远端可能饥饿};
\end{tikzpicture}
\end{center}
...
\textbf{典型实现：}具有线与/线或共享仲裁线的自选择式仲裁电路，可由各主设备同时驱动优先级编码并在共享线上比较，最终由最高优先级设备保留总线请求。传统 PCI 不属于这一类，而是采用中央仲裁器和各主设备独立的 REQ\#/GNT\# 信号。
...
\begin{tikzpicture}[>=stealth, font=\scriptsize]
  % ===== 同步通信（统一时钟） =====
  \node[font=\scriptsize\bfseries,text=Navy,anchor=west] at (-5.6,5.15){同步通信（统一时钟，固定节拍）};
  \foreach \x in {-4.5,-2.5,-0.5,1.5,3.5}
    \draw[dotted,RuleGray] (\x,4.6) -- (\x,2.0);
  \node[font=\tiny,MutedText,anchor=west] at (0.3,4.95){$T$（一个固定的总线周期）};
  % 时钟 CLK
  \draw[very thick,Navy] (-4.5,4.6)--(-3.5,4.6)--(-3.5,4.1)--(-2.5,4.1)--(-2.5,4.6)--(-1.5,4.6)--(-1.5,4.1)--(-0.5,4.1)--(-0.5,4.6)--(0.5,4.6)--(0.5,4.1)--(1.5,4.1)--(1.5,4.6)--(2.5,4.6)--(2.5,4.1)--(3.5,4.1)--(3.5,4.6)--(4.5,4.6);
  \node[anchor=east,font=\scriptsize] at (-5.4,4.35){时钟 CLK};
  % 地址 / 读命令（CPU 发出）
  \draw[very thick,Teal!90] (-4.0,3.3)--(4.5,3.3);
  \node[anchor=east,font=\scriptsize] at (-5.4,3.3){地址/读命令};
  \node[font=\tiny,MutedText,anchor=west] at (4.6,3.3){（CPU 发出）};
  % 数据（从设备送回）
  \draw[very thick,Burgundy!90] (-1.0,2.2)--(4.5,2.2);
  \node[anchor=east,font=\scriptsize] at (-5.4,2.2){数据};
  \node[font=\tiny,MutedText,anchor=west] at (4.6,2.2){（从设备送回）};
  \node[font=\scriptsize,black!70,align=center] at (0,1.4){$\Rightarrow$ 地址、命令与数据均按固定时钟节拍完成，各信号在约定的时钟周期内稳定};

  % ===== 异步通信（握手） =====
  \node[font=\scriptsize\bfseries,text=Navy,anchor=west] at (-5.6,-0.5){异步通信（握手应答，全互锁）};
  % 地址 / 数据（主设备发出）
  \draw[very thick,Teal!90] (-4.0,-1.4)--(4.0,-1.4);
  \node[anchor=east,font=\scriptsize] at (-5.4,-1.4){地址/数据};
  \node[font=\tiny,MutedText,anchor=west] at (4.1,-1.4){（主设备发出）};
  % 就绪 READY
  \draw[very thick,Navy] (-3.0,-2.5)--(3.0,-2.5);
  \node[anchor=east,font=\scriptsize] at (-5.4,-2.5){就绪 READY};
  \node[font=\tiny,MutedText,anchor=west] at (3.1,-2.5){（主$\to$从）};
  % 应答 ACK
  \draw[very thick,Amber!85!black] (-2.0,-3.6)--(3.5,-3.6);
  \node[anchor=east,font=\scriptsize] at (-5.4,-3.6){应答 ACK};
  \node[font=\tiny,MutedText,anchor=west] at (3.6,-3.6){（从$\to$主）};
  \node[font=\scriptsize,black!70,align=center] at (0,-4.85){$\Rightarrow$ 主设备放数据并发出就绪 $\to$ 从设备读走并发应答 $\to$ 主设备撤就绪 $\to$ 从设备撤应答，无需统一时钟};
\end{tikzpicture}
\end{center}
...
\textbf{解析：} 系统总线有三类信号线，各自传输不同类型的信息。数据线传输各种「数据字」——指令（取指时从内存读回）、操作数（LOAD/STORE 搬运）、中断类型号（中断响应时由设备送回）都经过数据线。握手/应答信号（ACK/READY）是控制信号，用于通信双方协调时序，走的是\textbf{控制线}而非数据线。区别口诀：数据线传「内容」（指令、操作数、向量号），控制线传「状态」（握手、请求、应答）。\else \answerarea \fi
...
\textbf{解析：} 总线带宽公式 $B = W \times f$（$W$ 为宽度，$f$ 为工作频率）。I 增宽（每周期传更多位）和 II 提频（单位时间更多周期）直接提升传输率。III 突发传输省掉重复发地址的开销，有效数据率显著提升（Cache 行填充、DMA 的典型方式）。IV 地址/数据线复用恰恰\textbf{降低}传输率——同一组线分时传地址和数据，意味着原本一周期能完成的操作现在要拆成两周期。复用的目的是\textbf{减少物理引脚数}而非提速。口诀：「增宽提频突发快，线复用反而慢。」\else \answerarea \fi
...
\textbf{解析：} 异步总线通过握手信号（请求/应答）协调传输，一次完整的握手过程完成的是\textbf{一次完整的数据传输}（通常为一个字/一个总线宽度），而非「一位」。握手粒度是「传输周期」而非「位」——把「一位」套到异步总线是将并行总线与串行通信混淆。(A) 正确——总线定义允许「两个或多个」部件。(B) 正确——DDR 技术使工作频率 $=$ 时钟频率 $\times$ 2，两者不一定相等。(D) 正确——突发传送只需一次起始地址，随后连续传多个数据。\else \answerarea \fi
...
\textbf{解析：} 关键是将\textbf{时钟频率}换算为\textbf{总线周期频率}。时钟频率 10 MHz，但一个总线周期占用 2 个时钟周期，故总线周期频率 $=10/2=5$ MHz。每个总线周期传 4 字节，带宽 $=5\times10^{6}\times4=20$ MB/s。常见错误：(A) 漏了每周期 4 字节只按 2 字节算；(C) 直接用 10 MHz $\times$ 4 = 40 MB/s，忘了除以 2；(D) 把「2 个时钟周期」误解为乘 2。口诀：「先把时钟换成总线周期频率，再乘每周期字节数——两步都不能漏。」\else \answerarea \fi

\item \textbf{（真题风格·选择题）} 某同步总线采用数据线和地址线复用方式，其中地址/数据线有 32 根，总线时钟频率为 66 MHz，每个时钟周期传送两次数据（上升沿和下降沿各传送一次数据）。假设数据相位进行连续突发传输，并忽略初始地址阶段及其他协议开销，则该总线的数据相位理论峰值带宽是
...
\textbf{解析：} 在连续突发的数据相位中，每次数据传送宽度 $=32\ \text{bit}=4\ \text{B}$。时钟频率 66 MHz，每周期传 2 次（DDR），故每秒传送次数 $=66\times10^{6}\times2=132\times10^{6}$。理论峰值带宽 $=4\times132\times10^{6}=528$ MB/s。(A) 漏了 DDR 加倍因子；(B) 只算了单沿（66 MHz $\times$ 4 B = 264 MB/s）；(D) 多乘了一次 2。实际有效带宽还需扣除地址阶段和协议开销。\else \answerarea \fi
...
\textbf{解析：} 时钟周期 $T=1/100\ \text{MHz}=10$ ns。总线宽度 32 位，128 位需分 $128/32=4$ 次传送。突发传输只需\textbf{1 次起始地址 + 4 次连续数据}，共 $1+4=5$ 个时钟周期 $\to 50$ ns。(A) 误把 128 位当一次传完（1 地址 + 1 数据 = 2 周期）；(B) 漏了地址周期，只算了 4 个数据周期 = 40 ns；(D) 误以为每个数据都要单独配地址（$4\times2=8$ 周期），混淆了突发传输与普通传输。\textbf{突发传输的精髓：}一次地址、连续数据——省掉重复寻址开销。\else \answerarea \fi
...
\textbf{解析：} 这是 408 中最具「反直觉」色彩的考点之一。直觉上「并行 = 多根线同时传 = 更快」，但现代高速总线（PCIe、SATA、USB 3.x）几乎全是\textbf{串行}——因为并行总线在高频下受时钟偏移（skew）和线间串扰限制，频率上不去；串行总线通过差分信号可达到数十 GHz，靠极高频率反超并行。例如：传统并行 PCI（32 位 @ 33 MHz）$\approx$ 133 MB/s，而一条 PCIe 4.0 lane（串行 @ 16 GT/s）$\approx$ 2 GB/s。(B) 正确：信号线复用通过分时共享减少物理线数（代价是速度下降）；(C) 正确：突发传送省掉重复寻址开销；(D) 正确：分离事务释放等待期总线给其他设备使用。\else \answerarea \fi
