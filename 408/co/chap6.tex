...
\item \textbf{（2011年·选择题·第20题）} 在系统总线的数据线上，不可能传输的是
...
\item \textbf{（2018年·选择题·第21题）} 下列选项中，可提高同步总线数据传输率的是
...
\item \textbf{（2021年·选择题·第19题）} 下列关于总线的叙述中，\textbf{错误}的是
...
\item \textbf{（2009年·选择题·第20题）} 假设某系统总线在一个总线周期中并行传输 4 字节信息，一个总线周期占用 2 个时钟周期，总线时钟频率为 10 MHz，则总线带宽是
...
\item \textbf{（2014年·选择题·第19题）} 某同步总线采用数据线和地址线复用方式，其中地址/数据线有 32 根，总线时钟频率为 66 MHz，每个时钟周期传送两次数据（上升沿和下降沿各传送一次数据），该总线的最大数据传输率（总线带宽）是
...
\item \textbf{（2012年·选择题·第19题）} 某同步总线的时钟频率为 100 MHz，宽度为 32 位，地址/数据线复用，每传输一个地址或数据占用一个时钟周期。若该总线支持突发（猝发）传输方式，则一次「主存写」总线事务传输 128 位数据所需要的时间至少是
...
\item \textbf{（2016年·选择题·第21题）} 下列关于总线设计的叙述中，\textbf{错误}的是
...
\item \textbf{（2023年·选择题·第20题 改编·综合计算）} 某计算机的主存模块通过同步总线与 CPU 连接。总线宽度为 64 位，时钟频率为 200 MHz，每个时钟周期传送一次数据。CPU 需要从主存读取一个 64 字节的 Cache 行。请回答：
