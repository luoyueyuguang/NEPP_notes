...
\begin{tikzpicture}[>=stealth, font=\scriptsize]
  % 时间轴 + 事件网格线
  \draw[->,thick,Navy] (0,-0.4) -- (9.4,-0.4) node[right,font=\scriptsize]{时间 $t$};
  \foreach \x in {0.5,1.5,3.0,4.5,6.0,7.5}{
    \draw[dotted,RuleGray] (\x,-0.3) -- (\x,3.6);
  }
  % 地址/数据（主设备发出）
  \draw[very thick,Teal!90] (0,2.75) -- (0.5,2.75) -- (0.5,3.25) -- (7.5,3.25) -- (7.5,2.75) -- (9,2.75);
  \node[anchor=east,font=\scriptsize] at (-0.3,3.0){地址/数据（主）};
  % 就绪 READY（主设备发出）
  \draw[very thick,Navy] (0,1.75) -- (1.5,1.75) -- (1.5,2.25) -- (4.5,2.25) -- (4.5,1.75) -- (9,1.75);
  \node[anchor=east,font=\scriptsize] at (-0.3,2.0){就绪 READY（主）};
  % 应答 ACK（从设备发出）
  \draw[very thick,Amber!85!black] (0,0.7) -- (3.0,0.7) -- (3.0,1.3) -- (6.0,1.3) -- (6.0,0.7) -- (9,0.7);
  \node[anchor=east,font=\scriptsize] at (-0.3,1.0){应答 ACK（从）};

  % 全互锁步骤标注
  \node[font=\scriptsize\bfseries,text=TealDark] at (1.5,2.55){(1) 数据就绪};
  \node[font=\scriptsize\bfseries,text=TealDark] at (4.5,2.55){(3) 撤就绪};
  \node[font=\scriptsize\bfseries,text=Amber!90] at (3.0,1.55){(2) 应答};
  \node[font=\scriptsize\bfseries,text=Amber!90] at (6.0,1.55){(4) 撤应答};
  \node[font=\scriptsize,text=Navy] at (6.9,2.55){完成};

  % 底部说明
  \node[font=\scriptsize,text=TealDark,align=center] at (4.5,-1.15){主设备放数据并发出就绪 $\to$ 从设备读走并发出应答 $\to$ 主设备撤就绪 $\to$ 从设备撤应答（全互锁，无需统一时钟）};
\end{tikzpicture}
\end{center}
