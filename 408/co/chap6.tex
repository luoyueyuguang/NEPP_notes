...
\textbf{解析：} 在连续突发的数据相位中，每次数据传送宽度 $=32\ \text{bit}=4\ \text{B}$。时钟频率 66 MHz，每周期传 2 次（DDR），故每秒传送次数 $=66\times10^{6}\times2=132\times10^{6}$。理论峰值带宽 $=4\times132\times10^{6}=528$ MB/s。(A) 漏了 DDR 加倍因子；(B) 只算了单沿（66 MHz $\times$ 4 B = 264 MB/s）；(D) 多乘了一次 2。实际有效带宽还需扣除地址阶段和协议开销。\fi
