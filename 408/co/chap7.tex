...
\begin{tikzpicture}[>=stealth, font=\scriptsize]
  \node[draw,rounded corners=2pt,fill=SoftBlue,minimum width=1.6cm,minimum height=0.9cm,align=center] (cpu) at (-4.5,0){CPU};
  \node[draw,rounded corners=2pt,fill=SoftGray,minimum width=4.4cm,minimum height=2.5cm] (box) at (0,0){};
  \node[draw,rounded corners=2pt,fill=SoftTeal,minimum width=1.8cm,minimum height=0.58cm] (dbr) at (-1.1,0.8){DBR 数据缓冲};
  \node[draw,rounded corners=2pt,fill=SoftAmber,minimum width=1.5cm,minimum height=0.58cm] (cr) at (1.2,0.8){CR 控制};
  \node[draw,rounded corners=2pt,fill=SoftBlue,minimum width=1.5cm,minimum height=0.58cm] (sr) at (0.05,0.05){SR 状态};
  \node[draw,rounded corners=2pt,fill=SoftGray,minimum width=2.3cm,minimum height=0.58cm] (dec) at (0.05,-0.7){地址译码/设备选择};
  \node[font=\scriptsize,black!70] at (0,1.45){I/O 接口（含多个 I/O 端口）};
  \draw[<->,thick,Navy!70] (cpu) -- (box);
  \draw[<->,thick,Teal!70] (box) -- (4.5,0);
  \node[draw,rounded corners=2pt,fill=SoftTeal,minimum width=1.8cm,minimum height=0.9cm,align=center] (dev) at (4.5,0){外部设备};
\end{tikzpicture}
\end{center}
...
\begin{center}
\begin{tikzpicture}[>=stealth, font=\scriptsize]
  \node[font=\scriptsize,black!70] at (2.2,1.75){中断向量表（内存低地址区，实模式）};
  \node[draw,rounded corners=1pt,fill=SoftGray,minimum width=4.4cm,minimum height=0.5cm,align=center] (e0) at (1.7,1.2){类型号 0（IP : CS）};
  \node[draw,rounded corners=1pt,fill=SoftGray,minimum width=4.4cm,minimum height=0.5cm,align=center] (e1) at (1.7,0.65){类型号 1（IP : CS）};
  \node[draw,rounded corners=1pt,fill=SoftGray,minimum width=4.4cm,minimum height=0.5cm,align=center] (e2) at (1.7,0.1){类型号 2（IP : CS）};
  \node[font=\scriptsize,black!70] at (1.7,-0.35){$\vdots$};
  \node[draw,rounded corners=1pt,fill=SoftAmber,minimum width=4.4cm,minimum height=0.5cm,align=center] (en) at (1.7,-0.9){类型号 $n$（IP : CS）};
  \node[font=\scriptsize,black!70] at (-1.4,1.2){地址 $0$};
  \node[font=\scriptsize,black!70] at (-1.4,0.65){地址 $4$};
  \node[font=\scriptsize,black!70] at (-1.4,0.1){地址 $8$};
  \node[font=\scriptsize,black!70] at (-1.4,-0.9){地址 $n\times 4$};
  \node[font=\scriptsize,black!70,align=center] at (0.2,-1.7){每个表项 4 字节 $=$ 中断服务程序入口地址（偏移 IP + 段基址 CS）};
  \node[font=\scriptsize,black!70,align=center] at (0.2,-2.2){中断向量 $=$ 入口地址，不是类型号本身};
\end{tikzpicture}
\end{center}
...
\begin{enumerate}
    ...
    \item \textbf{返回断点}：执行 IRET（中断返回指令），从堆栈中弹出并恢复保存的 PSW 和 PC/IP；IF 按中断前保存的值恢复。若中断前 IF=1，返回后才重新允许可屏蔽中断，不能表述为 IRET 无条件将 IF 置 1。
\end{enumerate}
\end{conclusion}
...
\begin{attention}
\textbf{408 常考辨析——实现可嵌套多重中断的必要条件：}
\begin{enumerate}
    \item 保护必要现场后，\textbf{必须在 ISR 中适时重新允许中断，或由硬件提供等效的分级嵌套机制}，否则更高优先级请求不能打断当前 ISR；不允许嵌套的单级中断系统没有这一要求。
...
\begin{center}
\begin{tikzpicture}[>=stealth, font=\scriptsize]
  \node[draw,rounded corners=2pt,fill=SoftGray,minimum width=2.3cm,minimum height=0.85cm,align=center] (s1) at (0,1.0){(1) 预处理\\（CPU 完成）};
  \node[draw,rounded corners=2pt,fill=SoftTeal,minimum width=2.3cm,minimum height=0.85cm,align=center] (s2) at (2.9,1.0){(2) 数据传送\\（DMAC 完成）};
  \node[draw,rounded corners=2pt,fill=SoftAmber,minimum width=2.3cm,minimum height=0.85cm,align=center] (s3) at (5.8,1.0){(3) 后处理\\（CPU 完成）};
  \draw[->] (s1) -- (s2);
  \draw[->] (s2) -- (s3);
  \node[align=center,font=\scriptsize,black!70] at (0,-0.05){CPU 写 DMAC 寄存器\\初始化地址寄存器、字计数器};
  \node[align=center,font=\scriptsize,black!70] at (2.9,-0.05){DMAC 请求并接管总线\\每传一数据数 AR+1、WC-1};
  \node[align=center,font=\scriptsize,black!70] at (5.8,-0.05){块结束后 DMAC 仅发一次中断\\CPU 检查结果、决定下次传送};
\end{tikzpicture}
\end{center}
...
\begin{center}
\begin{tikzpicture}[>=stealth, font=\scriptsize]
  \node[draw,rounded corners=2pt,fill=SoftBlue,minimum width=2.0cm,minimum height=0.9cm,align=center] (cpu) at (-1.5,2.2){CPU};
  \node[draw,rounded corners=2pt,fill=SoftGray,minimum width=2.8cm,minimum height=0.9cm,align=center] (mem) at (1.5,2.2){主存\\（通道程序 / CCW）};
  \node[draw,rounded corners=2pt,fill=SoftTeal,minimum width=4.8cm,minimum height=1.15cm,align=center] (chan) at (0,0){通道（I/O 处理器，IOP）\\\scriptsize 执行通道程序，独立管理 I/O};
  \draw[<->,thick,Navy!70] (cpu.south) -- (-1.2,0.58);
  \draw[<->,thick,Teal!70] (mem.south) -- (1.2,0.58);
  \node[font=\scriptsize,MutedText,anchor=east] at (-1.75,0.92){命令 / 中断};
  \node[font=\scriptsize,MutedText,anchor=west] at (1.75,0.92){CCW / 数据};
  \draw[->,thick,Teal!70] (-0.6,-0.58) -- (-0.6,-1.1);
  \draw[thick,Teal!70] (-3.4,-1.1) -- (3.4,-1.1);
  \node[font=\scriptsize,MutedText,anchor=west] at (-3.4,-0.82){通道总线};
  \draw[->,thick,Teal!70] (-2.8,-1.1) -- (-2.8,-1.6);
  \draw[->,thick,Teal!70] (0,-1.1) -- (0,-1.6);
  \draw[->,thick,Teal!70] (2.8,-1.1) -- (2.8,-1.6);
  \node[draw,rounded corners=2pt,fill=SoftAmber,minimum width=2.0cm,minimum height=0.8cm,align=center] (dc1) at (-2.8,-2.0){设备控制器 1};
  \node[draw,rounded corners=2pt,fill=SoftAmber,minimum width=2.0cm,minimum height=0.8cm,align=center] (dc2) at (0,-2.0){设备控制器 2};
  \node[draw,rounded corners=2pt,fill=SoftAmber,minimum width=2.0cm,minimum height=0.8cm,align=center] (dc3) at (2.8,-2.0){设备控制器 3};
  \node[draw,rounded corners=2pt,fill=SoftGray,minimum width=1.6cm,minimum height=0.7cm,align=center] (d1) at (-2.8,-3.4){设备 1};
  \node[draw,rounded corners=2pt,fill=SoftGray,minimum width=1.6cm,minimum height=0.7cm,align=center] (d2) at (0,-3.4){设备 2};
  \node[draw,rounded corners=2pt,fill=SoftGray,minimum width=1.6cm,minimum height=0.7cm,align=center] (d3) at (2.8,-3.4){设备 3};
  \draw[->,thick,Burgundy!70] (dc1.south) -- (d1.north);
  \draw[->,thick,Burgundy!70] (dc2.south) -- (d2.north);
  \draw[->,thick,Burgundy!70] (dc3.south) -- (d3.north);
  \node[font=\scriptsize,black!70,align=center] at (0,-4.35){选择通道：一次只为一个高速设备服务；\ 字节多路通道：可轮流为多个低速设备服务};
\end{tikzpicture}
\end{center}
...
\begin{enumerate}
    ...
    \item \textbf{DMA 与中断的区别}：在 408 的典型模型中，DMA 通常在数据块传送结束时才发一次中断（中断驱动 I/O 则常按字中断）；DMA 数据不经过 CPU 通用寄存器，也不由 CPU 指令逐字搬运；CPU 在\textbf{当前总线周期结束}的边界响应 DMA 总线请求，而可屏蔽中断通常在\textbf{当前指令周期结束}后响应。
    \item \textbf{周期挪用}：DMAC 在需要时暂停 CPU 的一次或若干总线周期，取得总线并完成数据传送，造成 CPU 的访存停顿；这不等同于只使用 CPU 空闲总线周期的透明 DMA。
\end{enumerate}
\end{conclusion}
...
\textbf{常见易错点：}
\begin{itemize}
    \item DMA 传送的逻辑端点是「外设 $\leftrightarrow$ 主存」，数据\textbf{不经过 CPU 通用寄存器}，CPU 也不执行逐字搬运指令。DMAC 负责地址、计数和总线控制；具体实现可以设置数据缓冲寄存器暂存一次传送的数据，因此不能绝对断言数据从不经过 DMAC 内部缓冲。
    \item 中断响应周期中，CPU 通常完成当前指令后才检查可屏蔽中断请求，即在\textbf{指令周期结束}时响应；DMA 总线请求则在\textbf{当前总线周期结束}的边界响应，因而可能在一条指令尚未完成时取得后续总线周期。周期挪用还可能使 CPU 的后续访存等待，并不要求总线原本处于空闲状态。
...
\]
(A) 错误——端口和设备之间的数据流是控制器自动处理的，不是 I/O 指令直接驱动。(B) 错误——CPU 不能直接访问 I/O 设备内部硬件状态，必须经端口中转。(C) 错误——端口之间不会直接互相搬数据，需经 CPU 中转（端口$\to$寄存器$\to$另一个端口）。\textbf{口诀：}IN/OUT 指令只负责「寄存器$\leftrightarrow$端口」，端口以下的交互由控制器自治。\else \answerarea \fi
...
\textbf{解析：} 关键区分「单级中断」与「多重中断」。多重中断允许嵌套——必须在 ISR 内\textbf{适当时机开中断}（保护现场之后、恢复现场之前），让更高优先级中断能够打断当前 ISR。所以 ISR 期间 CPU 既有关中断阶段也有开中断阶段，并非「全程关中断」。(B) 把单级中断的行为套到多重中断上是错误的。(A) 正确——中断响应必须在指令边界，以保证断点信息完整保存。(C) 正确——外中断由外部设备触发，与 CPU 当前跑哪条指令无关（这是中断区别于异常的核心特征）。(D) 正确——CPU 在中断响应周期采样中断请求引脚。\textbf{多重中断标准时序：}关中断$\to$保护现场$\to$开中断$\to$处理（可嵌套）$\to$关中断$\to$恢复现场$\to$IRET 返回。\else \answerarea \fi
...
\textbf{解析：} (D) 中有两个绝对化判断都不准确：(1)「中断适用于\textbf{所有}外设」——高速设备若为每个字或很小批次频繁中断，会使 CPU 承担过高的中断与软件搬运开销，工程上通常改用 DMA 等块传送机制。(2)「DMA \textbf{仅}适用于快速外设」——DMA 的本质优势是块传送时减少 CPU 参与；只要设备和系统支持相应 DMA 协议，是否采用还取决于传输粒度和开销，并非只由设备速度决定。(A) 正确——两种方式请求的核心资源不同：中断请求 CPU 处理，DMA 请求总线使用权。(B) 正确——典型 408 模型中，中断在指令边界响应，DMA 在总线周期边界获得传送机会。(C) 正确——中断方式由 CPU 执行 ISR/驱动代码完成数据搬运或协调，DMA 则由控制器硬件完成块传送。\else \answerarea \fi
...
(C) 描述的是「停止 CPU」方式而非题面规定的「周期挪用」。周期挪用中 DMA 每次搬 32 位（4 字节）只占用 1 个总线周期——512 字节共需 128 次挪用，其余大量周期 CPU 均可正常访问主存。(A) 正确：32 位缓冲满即发请求，$512/4=128$ 次。(B) 正确：DMA 优先级高于 CPU——设备数据等不起，丢了就永久丢失；CPU 让一个周期影响微乎其微。(D) 正确：数据块传完，DMAC 发中断触发后处理。\textbf{「挪用」二字的精髓：}借一个周期就还，不是霸占整段时间。\else \answerarea \fi
...
\textbf{解析：} (C) 是经典「矛盾说法」——DMA 方式的核心恰恰是「\textbf{绕过 CPU 让 DMA 控制器自己搬}」。没有「DMA 传送程序」这种东西：传送阶段是 DMA 控制器的硬件电路在自动控总线搬数据，CPU 全程不执行任何与传送相关的指令。(A) 正确：程序查询由 CPU 轮询状态寄存器 + IN/OUT 完成。(B) 正确：中断方式由 CPU 在 ISR 中用 IN/OUT 逐字节搬——传送主体仍是 CPU。(D) 正确：SSD（数百 MB/s～GB/s）和万兆网卡只有 DMA 能胜任。\textbf{三种方式 CPU 参与度对比：}程序查询——全程 CPU 软件；程序中断——每字节中断一次（CPU 执行 ISR）；DMA——仅头尾 CPU 参与，传送阶段硬件自动。\else \answerarea \fi
