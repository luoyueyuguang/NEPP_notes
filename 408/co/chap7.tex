...
\item \textbf{（2014年·选择题·第21题）} 下列有关 I/O 接口的叙述中，\textbf{错误}的是
...
\textbf{解析：} (D) 是\textbf{错误}的——统一编址（Memory-Mapped I/O）的精髓恰恰是将 I/O 端口映射到主存地址空间，CPU 用\textbf{普通访存指令}（LOAD/STORE）访问 I/O 端口，不需要专门的 I/O 指令。这是「统一」二字的含义。(A) 正确：状态字段和控制字段常合用同一寄存器不同位段，节省接口寄存器数，但端口数不因此而减少。(B) 正确：I/O 端口的标准定义就是「I/O 接口中 CPU 可寻址、可读写的寄存器」。(C) 正确：独立编址下 I/O 地址空间与主存地址空间完全独立，即使地址值相同也走不同译码通路，不会冲突。\textbf{对比速记：}统一编址 → 访存指令访问 I/O（ARM/MIPS）；独立编址 → 专用 I/O 指令 IN/OUT（x86）。\fi

\item \textbf{（2017年·选择题·第21题）} I/O 指令实现的数据传送通常发生在
...
\]
(A) 错误——端口和设备之间的数据流是控制器自动处理的，不是 I/O 指令直接驱动。(B) 错误——CPU 不能直接访问 I/O 设备内部硬件状态，必须经端口中转。(C) 错误——端口之间不会直接互相搬数据，需经 CPU 中转（端口→寄存器→另一个端口）。\textbf{口诀：}IN/OUT 指令只负责「寄存器↔端口」，端口以下的交互由控制器自治。\fi

\item \textbf{（2017年·选择题·第22题）} 下列关于多重中断系统的叙述中，\textbf{错误}的是
...
\textbf{解析：} 关键区分「单级中断」与「多重中断」。多重中断允许嵌套——必须在 ISR 内\textbf{适当时机开中断}（保护现场之后、恢复现场之前），让更高优先级中断能够打断当前 ISR。所以 ISR 期间 CPU 既有关中断阶段也有开中断阶段，并非「全程关中断」。(B) 把单级中断的行为套到多重中断上是错误的。(A) 正确——中断响应必须在指令边界，以保证断点信息完整保存。(C) 正确——外中断由外部设备触发，与 CPU 当前跑哪条指令无关（这是中断区别于异常的核心特征）。(D) 正确——CPU 在中断响应周期采样中断请求引脚。\textbf{多重中断标准时序：}关中断→保护现场→开中断→处理（可嵌套）→关中断→恢复现场→IRET 返回。\fi

\item \textbf{（2013年·选择题·第22题）} 下列关于中断 I/O 方式和 DMA 方式比较的叙述中，\textbf{错误}的是
...
\item \textbf{（2019年·选择题·第22题）} 下列关于 DMA 方式的叙述中，正确的是
...
\textbf{解析：} 这是 408 少见的「全选」题，完美覆盖 DMA 三阶段。\textbf{预处理}（I）：CPU 执行设备驱动程序，设置 DMAC 的源/目的地址、字节数、方向等参数——DMA 不是读心术，必须有人告诉它搬什么。\textbf{数据传送}（II + III）：DMA 控制器向总线仲裁器申请使用权，拿到总线后直接在设备↔主存之间搬数据，CPU 全程不参与。\textbf{后处理}（IV）：数据块传送结束，DMAC 向 CPU 发中断请求（仅一次），CPU 跑 ISR 做收尾（关设备、唤醒进程）。\textbf{关键区分：}驱动程序在\textbf{预处理}阶段设置参数，ISR 在\textbf{后处理}阶段处理完成——两者都是 CPU 软件工作，但 DMA 真正搬运数据的传送阶段完全是硬件行为。\fi
...
\item \textbf{（2020年·选择题·第22题）} 若设备采用周期挪用 DMA 方式进行输入输出，每次 DMA 传送的数据块大小为 512 字节，相应的 I/O 接口中有一个 32 位数据缓冲寄存器，对于数据输入过程，下列叙述中\textbf{错误}的是
...
\item \textbf{（2023年·选择题·第22题）} 下列关于 I/O 控制方式的叙述中，\textbf{错误}的是
...
\item \textbf{（2009年·综合题·第43题 改编·综合计算）} 某计算机的 CPU 主频为 500 MHz，CPI 为 1.5。某外设通过中断方式进行数据输入，每次中断传送 4 字节，每次中断处理（包括保护/恢复现场和实际数据传送）共需执行 100 条指令。外设的数据传输率为 2 MB/s。
