...
\begin{itemize}
    \item 最本质的区分标准：\textbf{CPU 使用什么类型的指令来访问 I/O 端口}。用访存指令 → 统一编址；用专用 I/O 指令 → 独立编址。
...
\textbf{中断的分类（按来源）：}
\begin{enumerate}
    \item \textbf{外部中断（硬件中断）}：来自 CPU 外部（如 I/O 设备、时钟、电源故障等）。又称可屏蔽中断和不可屏蔽中断（NMI）。
...
ISR $A$ & & & & ————— & & & & \\
\hline
事件 & $C$ 请求 & CPU 响应 $C$ & $A$ 请求 & CPU 响应 $A$ & $A$ 处理完 & $B$ 请求 & CPU 响应 $B$ & 返回主程序 \\
\hline
\end{tabular}
\end{center}

说明：$C$ 执行过程中 $A$ 请求到来（优先级更高），CPU 暂停 $C$ 转去处理 $A$；$A$ 处理完毕后返回 $C$ 继续执行；$C$ 恢复后 $B$ 请求到来（$B > C$），再次嵌套。
\end{conclusion}
...
程序查询和程序中断方式的共同问题是：数据的传送仍然需要 CPU 执行指令来完成（读端口→写内存，或者读内存→写端口）。当需要高速大批量数据传送时（如磁盘读写），CPU 的指令执行开销成为瓶颈。DMA 方式通过\textbf{专门的硬件控制器}直接实现内存与外设之间的数据传送，CPU 仅在传送开始和结束时进行干预。
...
\textbf{CPU 与外设并行} & 部分并行（CPU 可在数据传送间工作）& 高度并行（CPU 与数据传送完全并行）\\
...
\begin{enumerate}
    \item \textbf{周期挪用（Cycle Stealing）}：DMAC 在 CPU\textbf{不使用总线}的时钟周期内偷偷进行数据传送（即「挪用」一个总线周期）。CPU 和 DMAC 交替使用总线。这是\textbf{最常用}的方式。
...
\begin{enumerate}
    ...
    \item \textbf{中断响应过程（七步曲）}：关中断 → 保护断点（PSW + PC 自动压栈）→ 识别中断源（获取中断类型号）→ 查向量表获取 ISR 入口 → 执行 ISR → 恢复现场 → 中断返回（IRET）。\textbf{保护断点由硬件完成，保护现场由软件（ISR 指令）完成}——这是最高频的辨析考点。
    \item \textbf{中断向量表}：向量表在内存低地址区（0--1023），每个向量占 4 字节（实模式 8086），地址 = 类型号 $\times$ 4。中断向量 = ISR 入口地址（≠ 类型号）。
    \item \textbf{多重中断}：ISR 中开中断 + 优先级裁决 = 可实现嵌套。注意：保护现场前不能开中断。
    \item \textbf{DMA 三阶段}：预处理（CPU 初始化 DMAC 寄存器）→ 数据传送（DMAC 控制，CPU 不参与）→ 后处理（DMAC 发中断，CPU 检查结果）。
...
\item \textbf{（单项选择题）} 以下关于 I/O 端口编址方式的说法中，错误的是
\begin{center}
\begin{tabular}{ll}
(A) 统一编址方式中，CPU 用访存指令访问 I/O 端口 \\[4pt]
(B) 独立编址方式中，CPU 用专用 I/O 指令访问 I/O 端口 \\[4pt]
(C) 统一编址会占用一部分主存地址空间 \\[4pt]
(D) 独立编址方式中，I/O 端口不需要地址，只需要控制信号
\end{tabular}
\end{center}

\item \textbf{（单项选择题）} 在中断响应周期中，由 CPU 硬件自动完成的操作是
\begin{center}
\begin{tabular}{ll}
(A) 保护各通用寄存器的内容到堆栈 & (B) 保护程序计数器（PC）和程序状态字（PSW）到堆栈 \\[4pt]
(C) 执行中断服务程序 & (D) 恢复各通用寄存器的内容
\end{tabular}
\end{center}

\item \textbf{（单项选择题）} 8086 实模式下，中断向量表存放在内存的 0 地址处。若某中断的类型号为 20H，则该中断的中断向量存放在内存地址（十六进制）
\begin{center}
\begin{tabular}{ll}
(A) $20\text{H}$ & (B) $40\text{H}$ \\[6pt]
(C) $80\text{H}$ & (D) $100\text{H}$
\end{tabular}
\end{center}

\item \textbf{（单项选择题）} DMA 方式实现数据传送的路径是
\begin{center}
\begin{tabular}{ll}
(A) 外设 $\to$ CPU $\to$ 内存 & (B) 外设 $\to$ DMAC $\to$ 内存 \\[4pt]
(C) 外设 $\leftrightarrow$ 内存（不经 CPU）& (D) 外设 $\to$ 通道 $\to$ 内存
\end{tabular}
\end{center}

\item \textbf{（单项选择题）} 下列关于 DMA 与中断方式比较的说法，正确的是
\begin{center}
\begin{tabular}{l}
(A) DMA 方式在数据传送过程中不需要 CPU 的任何干预 \\
(B) 中断方式下，CPU 与外设可以完全并行工作 \\
(C) DMA 方式在数据块传送结束时才向 CPU 发一次中断请求 \\
(D) DMA 方式不需要申请总线使用权
\end{tabular}
\end{center}

\item \textbf{（填空题）} I/O 接口中可供 CPU 直接读写的寄存器称为 \underline{\qquad\qquad}。一个 I/O 接口通常包含 \underline{\qquad\qquad} 端口、\underline{\qquad\qquad} 端口和 \underline{\qquad\qquad} 端口。

\item \textbf{（简答题）} 简述中断响应周期中 CPU 硬件自动完成的操作序列，并说明「保护断点」和「保护现场」有什么区别。

\item \textbf{（计算题）} 某计算机系统中，中断方式下一次中断响应和处理共需执行 50 条指令（含保护/恢复现场和实际数据处理），每条指令的 CPI = 2.0，主频为 1.0 GHz。若某外设以 100 KB/s 的速率输入数据，每次中断传送一个字（2 字节）。问：
...
\item \textbf{（真题风格，单项选择题）} 某计算机主频为 500 MHz，某外设以 2 MB/s 的速度与主机进行 DMA 数据传送。若 DMA 预处理和后处理的总时间开销为 1000 个时钟周期（CPU 在整个 DMA 传送过程的开头和结尾各执行一次预处理和后处理），采用周期挪用方式，DMA 传送过程中 CPU 的主频降为原来的 $1/2$（但外设速率和 DMA 传送速率不变）。若传送 64 KB 的数据块，DMA 传送期间 CPU 被 DMA 请求占用的时钟周期为 DMA 数据传送所用周期的 5\%，则整个 DMA 过程中 CPU 用于该外设 I/O 处理的时间开销约为

\begin{center}
\begin{tabular}{ll}
(A) $2\ \mu\text{s}$ & (B) $4\ \mu\text{s}$ \\[6pt]
(C) $8\ \mu\text{s}$ & (D) $16\ \mu\text{s}$
\end{tabular}
\end{center}

\item \textbf{（真题风格，简答题）} 简述程序查询、程序中断、DMA 三种 I/O 控制方式下，CPU 启动一次 I/O 操作后的工作状态有何不同。从 CPU 与 I/O 设备并行性的角度，比较三者的优劣。

\item \textbf{（真题风格，综合题）} 某计算机系统的中断系统支持多重中断，中断优先级从高到低依次为：$A > B > C > D$。CPU 正在执行主程序时，先后发生了以下事件：

\begin{itemize}
    \item $t_1$ 时刻：设备 $C$ 发出中断请求，CPU 响应；
    \item $t_2$ 时刻：CPU 正在执行 $C$ 的中断服务程序（已开中断），设备 $A$ 发出中断请求；
    \item $t_3$ 时刻：CPU 正在执行 $A$ 的中断服务程序（已开中断），设备 $B$ 发出中断请求；
    \item $t_4$ 时刻：CPU 执行完 $A$ 的中断服务程序并返回；
    \item $t_5$ 时刻：CPU 正在继续执行 $C$ 的中断服务程序，设备 $D$ 发出中断请求。
\end{itemize}

\begin{enumerate}
    \item[(1)] 画出从 $t_1$ 到所有中断处理完毕的时间线上 CPU 执行程序的切换过程。
    \item[(2)] 设备 $B$ 的中断请求在哪个时刻被 CPU 响应？为什么不是在 $t_3$ 时刻立即响应？
    \item[(3)] 设备 $D$ 的中断请求能否在 $t_5$ 时刻被响应？为什么？
\end{enumerate}

\item \textbf{（真题风格，综合题）} 某计算机系统的 DMA 控制器结构如下：地址寄存器（AR）为 32 位，字计数器（WC）为 16 位，数据线宽度为 32 位（每次传送 4 字节），总线时钟频率为 100 MHz，每个总线周期传送一个数据。CPU 主频为 2.0 GHz。

\begin{enumerate}
    \item[(1)] 该 DMA 控制器一次最多可传送多少字节的数据？
    \item[(2)] 若 DMA 传送速率为 400 MB/s（实际数据传送速率），DMA 预处理需 CPU 执行 20 条指令（CPI = 1.5），后处理需 30 条指令（CPI = 2.0）。现需从磁盘读入 256 KB 数据到内存，磁盘的数据传输率为 50 MB/s。求完成这次传送，CPU 用于 DMA 处理的总时间为多少？
    \item[(3)] 若将上述 256 KB 数据改用程序中断方式传送，每次中断传送 4 字节，每次中断处理需执行 100 条指令（CPI = 2.0）。比较中断方式和 DMA 方式下 CPU 的时间开销（CPU 主频均为 2.0 GHz），说明 DMA 方式的优势所在。
\end{enumerate}

\item \textbf{（真题风格，综合题）} 在计算机系统的层次化 I/O 架构中，应用程序、操作系统、DMA 控制器和外设各自承担不同的角色。回答以下问题：

\begin{enumerate}
    \item[(1)] 应用程序发起一次磁盘读操作（如 C 语言的 \texttt{fread} 函数调用），描述从应用层到硬件层的完整调用路径。
    \item[(2)] 操作系统中 I/O 软件通常分为几个层次？每个层次的功能是什么？
    \item[(3)] 在 DMA 方式下，磁盘控制器从磁盘读取数据到内存的过程中，CPU 是否完全空闲？请说明理由。
\end{enumerate}
...
\noindent\textbf{拓展阅读：}
\begin{itemize}
    \item Patterson \& Hennessy《计算机组成与设计：硬件/软件接口》（RISC-V / MIPS 版）第 4 章「处理器」4.7--4.9 节——异常与中断的硬件实现；第 5 章「存储器层次结构」中有关 I/O 与总线系统的内容。
    \item Bryant \& O'Hallaron《深入理解计算机系统》（CS:APP）第 8 章「异常控制流」——8.1 节（异常）、8.5 节（信号与非本地跳转的主要机制），从操作系统的视角理解异常控制流，包括中断、陷阱、故障和终止的分类与处理。
    \item Patterson \& Hennessy《计算机组成与设计》第 5 章 5.5--5.6 节——DMA 与 I/O 总线的详细工作流程，包含周期挪用（Cycle Stealing）的时序分析。
    \item 董社勤《计算机组成原理》——第 7 章「输入输出系统」（中断向量表的 x86 实模式实现、8259A 中断控制器原理）可在国内教材中找到更贴合 408 风格的详细讲解。
\end{itemize}
