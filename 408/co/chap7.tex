...
\textbf{解析：} (D) 中有两个绝对化判断都不准确：(1)「中断适用于\textbf{所有}外设」——高速设备若为每个字或很小批次频繁中断，会使 CPU 承担过高的中断与软件搬运开销，工程上通常改用 DMA 等块传送机制。(2)「DMA \textbf{仅}适用于快速外设」——DMA 的本质优势是块传送时减少 CPU 参与；只要设备和系统支持相应 DMA 协议，是否采用还取决于传输粒度和开销，并非只由设备速度决定。(A) 正确——两种方式请求的核心资源不同：中断请求 CPU 处理，DMA 请求总线使用权。(B) 正确——典型 408 模型中，中断在指令边界响应，DMA 在总线周期边界获得传送机会。(C) 正确——中断方式由 CPU 执行 ISR/驱动代码完成数据搬运或协调，DMA 则由控制器硬件完成块传送。\fi
