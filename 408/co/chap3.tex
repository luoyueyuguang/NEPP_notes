...
\node[right of=reg, node distance=4cm] (speed) {\small 速度：$<1$ns};
\node[right of=cache, node distance=4cm] (speed2) {\small $\sim 1$--$10$ns};
\node[right of=mem, node distance=4cm] (speed3) {\small $\sim 50$--$100$ns};
\node[right of=disk, node distance=4cm] (speed4) {\small $\sim 10$ms（磁盘）/$\sim100\mu$s（SSD）};
\node[left of=reg, node distance=5cm] (size) {\small 容量：$\sim$KB};
\node[left of=cache, node distance=5cm] (size2) {\small $\sim$MB};
\node[left of=mem, node distance=5cm] (size3) {\small $\sim$GB};
\node[left of=disk, node distance=5cm] (size4) {\small $\sim$TB};
\end{tikzpicture}
\end{center}

每一层都是下一层的「缓存」——Cache 缓存主存、主存缓存磁盘。上层是下层的子集（包含关系）。
\end{mydef}
...
设 $t_H$ 为上层命中访问时间，$t_M$ 为下层命中（上层缺失后从下层调入）的访问时间：
\begin{itemize}
    \item \textbf{命中率} $H$：访问上层命中的概率。
    \item \textbf{缺失率} $M = 1-H$。
    \item \textbf{平均访问时间}：$T_{\mathrm{avg}} = H \cdot t_H + (1-H)\cdot t_M = t_H + M \cdot t_M$。
\end{itemize}

两级存储（Cache--主存）情况下，$t_H$ 即 Cache 命中时间，$t_M$ 即缺失代价（从主存取一整块到 Cache 的时间）。
\end{conclusion}
...
\begin{itemize}
    \item \textbf{速度快}：读/写延迟 $\sim 1$--$10$ns，不需要刷新。
...
\begin{itemize}
    ...
    \item \textbf{刷新周期}：通常在 64ms 内必须对所有行刷新一次。
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{分散刷新}：将刷新操作分散到系统周期的「空闲时间」中。每经过一定时间刷新一行，CPU 不用专门等待。
    \item \textbf{异步刷新}：将刷新行均匀分散到整个刷新周期内，每隔 $(64\,\mathrm{ms})/(\text{行数})$ 刷新一行。兼有二者的优点有，对 CPU 影响最小，最为常见。
\end{itemize}
\end{mydef}
...
\hline
基本单元 & 六管触发器 & 单管 + 电容 \\
是否需要刷新 & 否 & 是（$\sim 64$ms 刷新周期）\\
读写速度 & 快（$\sim 1$--$10$ns）& 较慢（$\sim 50$--$100$ns）\\
...
\textbf{408 常见辨析：}
\begin{itemize}
    \item 「SRAM 速度快、成本高」→ 用于 Cache。
    \item 「DRAM 需要刷新」→ 这是 DRAM 的「动态」含义。
    \item 「DRAM 破坏性读出」→ 读后必须重写。
    \item 「SRAM 不需要刷新」→ 但不等于非易失——断电数据仍丢失。
\end{itemize}
\end{conclusion}
...
\textbf{特点：}一个连续的程序段通常在单个存储体中，各体只能串行工作。适合多道程序环境（不同程序分布在不同体中），但不能提高单道程序的带宽。
...
LRU 是实际 Cache 中最常用的替换算法，命中率接近最优（Belady 算法）。
\end{conclusion}
...
\begin{itemize}
    ...
    \item $t_m$：主存访问时间（缺失代价）
    \item $h$：命中率（Hit Rate）
    \item $m=1-h$：缺失率（Miss Rate）
\end{itemize}
\[
T_{\mathrm{avg}} = h\cdot t_c + (1-h)\cdot t_m = t_c + m\cdot t_m
\]

\textbf{两级 Cache（L1 + L2）：}
\[
T_{\mathrm{avg}} = h_1\cdot t_{c1} + (1-h_1)\cdot h_2\cdot t_{c2} + (1-h_1)(1-h_2)\cdot t_m
\]
其中 $t_{c1}$ 为 L1 命中时间，$t_{c2}$ 为 L2 命中时间（L1 缺失但 L2 命中时的访问时间），$t_m$ 为主存访问时间。

\textbf{注意（408 易错）：}L1 缺失后从 L2 调入时，$t_{c2}$ 已包含了从 L2 读块并传回 L1 的时间，不是单独读 L2 的时间。
\end{formula}
...
\begin{itemize}
    ...
    \item LRU 计数器（组相联时）：每行 $\lceil\log_2 W\rceil$ 位。
\end{itemize}
...
虚拟存储器将\textbf{主存}用作\textbf{磁盘}的「缓存」：
\begin{itemize}
    \item 每个程序拥有独立的\textbf{虚拟地址空间}（如 32 位系统为 $2^{32}$ 字节 = 4GB），远大于物理主存的实际容量。
    \item 程序使用虚拟地址（Virtual Address，VA），由硬件（MMU，Memory Management Unit）在运行时转换为物理地址（Physical Address，PA）。
    \item 虚拟地址空间中只有当前\textbf{活跃}部分被载入物理主存，其余部分暂存磁盘上的交换区（swap）。
\end{itemize}
...
块大小 & Cache 块（32--128B）& 页（4KB--2MB）\\
缺失 & Cache Miss & Page Fault \\
缺失代价 & $\sim 100$ 个周期 & $\sim 10^6$--$10^7$ 个周期（磁盘）\\
映射方式 & 硬件实现 & 软件（OS）+ 硬件（MMU）\\
替换策略 & 硬件 LRU & OS 页置换算法（如 Clock）\\
写策略 & 写直达/写回 & 始终写回（Write-Back）\\
\hline
\end{tabular}
\end{tablebox}

\textbf{虚拟存储器中始终使用写回策略}——写直达对磁盘而言代价太高。虚拟存储器由 OS 管理，Cache 由硬件管理。
\end{mydef}
...
TLB 命中时，翻译（VA→PA）在一个时钟周期内完成，不再需要访问主存中的页表。TLB 缺失时，硬件（或 OS）从主存中的页表找到对应 PTE 并装入 TLB。
\end{mydef}

\begin{conclusion}{TLB + Cache 的综合访存流程}{tlb-cache-flow}
\textbf{这是 408 高频综合题的主题。}在同时有 TLB 和 Cache 的系统中，CPU 访存的完整流程：
...
\textbf{注意：}若 TLB 和 Cache 都命中，则访存只需一次真正的 Cache 访问（$<1$--$2$ 周期），无需访问主存。这就是存储层次设计的理想目标。
\end{conclusion}

\begin{attention}
\textbf{408 高频辨析——物理寻址 Cache vs 虚拟寻址 Cache：}
\begin{itemize}
...
\begin{enumerate}
    ...
    \item OS 在磁盘（swap 区）中找到对应页，在物理主存中分配一个空闲页框。
    \item 若主存已满，OS 执行\textbf{页面置换算法}（如 LRU、Clock、改进的 Clock 算法）选出一个牺牲页写回磁盘（若脏）。
    \item 将所需页从磁盘读入分配的物理页框。
    \item 更新页表（VPN→PPN，标记 Valid），刷新 TLB 中对应项。
    \item 返回被中断的指令，重新执行。
\end{enumerate}

整个过程中，CPU 可能执行\textbf{数千万}条其他指令（磁盘 IO 极慢），对其他进程完全透明。
\end{conclusion}

\begin{attention}
\textbf{408 常考辨析——Cache 缺失 vs 缺页：}
\begin{itemize}
    \item Cache 缺失\textbf{完全由硬件处理}（硬件自动从主存取块装入 Cache），对 OS 透明。
    \item 缺页\textbf{由 OS 软件处理}（发出异常→OS 缺页处理程序→磁盘 IO→页表更新→返回），代价巨大。
...
\begin{enumerate}
    ...
    \item \textbf{TLB + Cache + 页表的综合访存流程}：虚拟地址→TLB→PA→Cache→数据。
...
\item \textbf{（单项选择题）} 以下存储器中，属于易失性存储器的是
\begin{center}
\begin{tabular}{ll}
(A) Flash 存储器 & (B) EEPROM \\[4pt]
(C) DRAM & (D) 磁盘
\end{tabular}
\end{center}

\item \textbf{（单项选择题）} 某 SRAM 芯片有 14 根地址线、8 根数据线，则该芯片的容量为
\begin{center}
\begin{tabular}{ll}
(A) $16\mathrm{K}\times 8$ 位 & (B) $14\mathrm{K}\times 8$ 位 \\[4pt]
(C) $16\mathrm{K}\times 14$ 位 & (D) $14\times 8$ 位
\end{tabular}
\end{center}

\item \textbf{（单项选择题）} 需要用 $16\mathrm{K}\times 4$ 位的存储芯片构成 $64\mathrm{K}\times 8$ 位的存储器，所需芯片数量为
\begin{center}
\begin{tabular}{ll}
(A) 4 & (B) 6 \\[4pt]
(C) 8 & (D) 16
...
\item \textbf{（单项选择题）} 下列关于 DRAM 的说法中，错误的是
\begin{center}
\begin{tabular}{ll}
(A) DRAM 需要定期刷新以保持数据 \\[4pt]
(B) DRAM 的读写操作是破坏性的 \\[4pt]
(C) DRAM 的集成度高于 SRAM \\[4pt]
(D) DRAM 的存取速度比 SRAM 快
\end{tabular}
\end{center}

\item \textbf{（单项选择题）} 在 Cache 的各种地址映射方式中，冲突缺失率最低的是
\begin{center}
\begin{tabular}{ll}
(A) 直接映射 & (B) 2 路组相联 \\[4pt]
(C) 4 路组相联 & (D) 全相联映射
...
\item \textbf{（填空题）} 某计算机主存按字节编址，Cache 采用 4 路组相联映射，共有 128 行，块大小为 32 字节。则主存地址中，块内偏移占 \underline{\qquad} 位，组号占 \underline{\qquad} 位，Tag 占 \underline{\qquad} 位（设主存地址为 32 位）。

\item \textbf{（填空题）} 一个由 4 个存储体构成的低位交叉编址存储器，每个存储体的存取周期为 $40\,\mathrm{ns}$，总线传输周期为 $10\,\mathrm{ns}$。连续读取 8 个字所需的时间为 \underline{\qquad} ns。

\item \textbf{（简答题）} 简述时间局部性和空间局部性的含义，并各举出一个程序中的典型例子。

\item \textbf{（简答题）} 说明 Cache 的写直达和写回策略的区别，以及它们各自通常搭配的写缺失策略。

\item \textbf{（简答题）} 简述缺页中断的处理过程。为何缺页中断不能由硬件处理而必须由操作系统介入？
...
\item \textbf{（真题风格，单项选择题）} 某计算机主存按字节编址，地址空间为 256MB。Cache 容量为 64KB，块大小为 32 字节，采用直接映射方式。则主存地址中 Tag 所占的位数为
\begin{center}
\begin{tabular}{ll}
(A) 11 & (B) 13 \\[4pt]
(C) 15 & (D) 17
...
\item \textbf{（真题风格，单项选择题）} 某计算机的 Cache 共有 16 行，块大小为 16 字节，采用 2 路组相联映射（每组 2 行），主存按字节编址。以下主存地址中，与 0x0123 映射到同一组的地址是
\begin{center}
\begin{tabular}{ll}
(A) 0x012B & (B) 0x0137 \\[4pt]
(C) 0x0143 & (D) 0x0156
\end{tabular}
\end{center}

\item \textbf{（真题风格，综合题）} 某计算机的主存地址空间为 256MB，按字节编址。指令 Cache 和数据 Cache 分离，其中数据 Cache 容量为 8KB，块大小为 16 字节，采用 4 路组相联映射。
\begin{enumerate}
    \item[(1)] 求主存地址中 Tag、组号（Set Index）和块内偏移各占多少位。
    \item[(2)] 设 CPU 依次访问以下地址（十进制）：$0, 16, 32, 0, 64, 16, 128, 0$。初始 Cache 为空。若采用 LRU 替换策略，求 Cache 命中次数和命中率。
    \item[(3)] 若替换策略改为 FIFO，求命中次数。
    \item[(4)] 比较 (2) 和 (3) 的结果，分析 LRU 和 FIFO 的特点。
\end{enumerate}

\item \textbf{（真题风格，综合题）} 某计算机主存由 8 个存储体构成，采用低位交叉编址。每个存储体的存取周期为 $80\,\mathrm{ns}$，总线传输周期为 $20\,\mathrm{ns}$，每个存储体一次可读写 4 字节。
\begin{enumerate}
    \item[(1)] 求该存储器的最大带宽（MB/s）。
    \item[(2)] 若 CPU 连续从地址 0 开始读取 64 字节数据，需要多少时间？
    \item[(3)] 若改用高位交叉编址，同样读取 64 字节连续数据，需要多少时间？比较结果并说明原因。
\end{enumerate}

\item \textbf{（真题风格，综合题）} 某计算机系统具有 TLB 和两级 Cache。虚拟地址 32 位，物理地址 28 位，页大小 4KB。
\begin{enumerate}
    \item[(1)] 虚拟地址中 VPN 和 VPO 各占多少位？物理地址中 PPN 和 PPO 各占多少位？
    \item[(2)] L1 Cache 为 8 路组相联，容量 32KB，块大小 64 字节。主存地址中 Tag、组号和块内偏移各占多少位？
    \item[(3)] 设 TLB 采用全相联，共 16 项。当 CPU 执行一条加载指令 \texttt{ld R1, [R2]}，R2 中的虚拟地址为 0x12345678 时，若 TLB 命中且 L1 Cache 命中，描述从虚拟地址到数据返回的完整过程。
    \item[(4)] 若 TLB 命中而 L1 Cache 缺失，L2 Cache 命中，设 TLB 命中时间 $1$ 个周期，L1 命中时间 $2$ 个周期，L2 命中时间（含从 L2 调入 L1 的时间）$15$ 个周期，求这次访问的总延迟。
\end{enumerate}

\item \textbf{（真题风格，综合题）} 分析以下两段 C 代码的局部性，并比较二者在 Cache 方面的表现（设 Cache 块大小为 64 字节，\texttt{int} 占 4 字节）：
...
for (int j = 0; j < N; j++)
    for (int i = 0; i < N; i++)
        sum += a[i][j];
\end{lstlisting}
设 $N=1024$，假设 Cache 初始为空。
\begin{enumerate}
    \item[(1)] 分别指出代码 A 和代码 B 的空间局部性和时间局部性表现。
    \item[(2)] 估算代码 A 的缺失率（忽略 \texttt{sum} 和循环变量的访问）。
    \item[(3)] 估算代码 B 的缺失率。
    \item[(4)] 若将循环顺序颠倒（即如代码 B），除了 Cache 缺失率增加，在虚拟存储中有无其他影响？为什么？
\end{enumerate}
...
\noindent\textbf{拓展阅读：}
\begin{itemize}
    \item Patterson \& Hennessy《计算机组成与设计（RISC-V 版）》第 5 章「大而快：层次化存储系统」——完整论述存储层次、Cache 四种映射方式和性能分析，含 RISC-V 处理器的实际 Cache 设计。
    \item CS:APP（深入理解计算机系统）第 6 章「存储器层次结构」——\S6.2 局部性原理的形式化讨论与量化实验；\S6.3--\S6.4 存储层次和 Cache 的设计；\S6.5 编写 Cache 友好的代码；\S6.6 从主存到 Cache 的地址映射详解。
    \item CS:APP 第 9 章「虚拟内存」——\S9.3--\S9.4 页表和 TLB 的详细工作机制；\S9.5 缺页处理和内存映射；\S9.6 地址翻译的综合案例（Core i7 的完整地址翻译过程，含 TLB、多级页表、多级 Cache 的协同，极好的 408 信息源）。
\end{itemize}
