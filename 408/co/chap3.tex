...
\begin{tikzpicture}[>=stealth, font=\scriptsize]
  % ===== 经典存储层次金字塔（上窄下宽 = 容量增大、速度减小）=====
  % 光盘/磁带（最底层）
  \filldraw[draw=Navy!55,fill=gray!44] (-3.584,0.68) -- (3.584,0.68) -- (4.23,0.0) -- (-4.23,0.0) -- cycle;
  \node at (0,0.34) {光盘、磁带（辅存）};
  % 磁盘/SSD
  \filldraw[draw=Navy!55,fill=gray!34] (-2.938,1.36) -- (2.938,1.36) -- (3.584,0.68) -- (-3.584,0.68) -- cycle;
  \node at (0,1.02) {磁盘、SSD（辅存）};
  % 主存
  \filldraw[draw=Navy!55,fill=gray!26] (-2.292,2.04) -- (2.292,2.04) -- (2.938,1.36) -- (-2.938,1.36) -- cycle;
  \node at (0,1.70) {主存（DRAM）};
  % Cache
  \filldraw[draw=Navy!55,fill=gray!18] (-1.646,2.72) -- (1.646,2.72) -- (2.292,2.04) -- (-2.292,2.04) -- cycle;
  \node at (0,2.38) {Cache（SRAM）};
  % 寄存器（最顶层）
  \filldraw[draw=Navy!55,fill=gray!8] (-1.0,3.4) -- (1.0,3.4) -- (1.646,2.72) -- (-1.646,2.72) -- cycle;
  \node at (0,3.06) {寄存器（CPU 内）};

  % ===== 左侧：容量（向底部增大）=====
  \node[anchor=east,font=\scriptsize\bfseries,text=TealDark] at (-5.7,3.62){容量 $\uparrow$};
  \node[anchor=east] at (-5.7,3.06){极小};
  \node[anchor=east] at (-5.7,2.38){小};
  \node[anchor=east] at (-5.7,1.70){中};
  \node[anchor=east] at (-5.7,1.02){大};
  \node[anchor=east] at (-5.7,0.34){极大};

  % ===== 右侧：速度（向底部减小）=====
  \node[anchor=west,font=\scriptsize\bfseries,text=TealDark] at (5.7,3.62){速度 $\downarrow$};
  \node[anchor=west] at (5.7,3.06){最快};
  \node[anchor=west] at (5.7,2.38){快};
  \node[anchor=west] at (5.7,1.70){中};
  \node[anchor=west] at (5.7,1.02){慢};
  \node[anchor=west] at (5.7,0.34){最慢};

  % ===== 底部说明 =====
  \node[font=\scriptsize,text=Navy] at (0,-0.7){每字节成本：寄存器 $>$ Cache $>$ 主存 $>$ 磁盘 $>$ 光盘/磁带};
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}[>=stealth, font=\scriptsize]
  % ===== ① 连续地址流 =====
  \node[font=\scriptsize\bfseries,text=Navy] at (0,4.5){连续地址流（逐字递增）};
  \foreach \a in {0,...,7}{
    \pgfmathsetmacro{\x}{-3.325+0.95*\a}
    \draw[thick,fill=SoftAmber] (\x-0.42,4.02) rectangle (\x+0.42,4.42);
    \node at (\x,4.22){\a};
  }
  \node[font=\scriptsize] at (4.05,4.22){$\dots$};

  % ===== ② 低位地址选体 =====
  \draw[->,thick,Navy] (0,4.02) -- (0,3.38);
  \node[draw,rounded corners=2pt,fill=SoftGray,align=center,minimum width=3.0cm,minimum height=0.62cm] at (0,3.06){体号 $=$ 低位地址\\（地址 $\bmod 4$）};
  \draw[->,thick,Navy] (0,2.74) -- (0,2.4);

  % ===== ③ 分发到 4 个存储体 =====
  \foreach \b in {0,...,3}{
    \pgfmathsetmacro{\bx}{-3.45+2.3*\b}
    \draw[->,thick,Navy] (0,2.4) -- ({\bx},1.82);
  }
  \node[draw,rounded corners=2pt,fill=SoftBlue,minimum width=2.2cm,minimum height=1.4cm,align=center] at (-3.45,1.05){体号 0\\(低位 $00$)\\字 $0,4,8,12$};
  \node[draw,rounded corners=2pt,fill=SoftBlue,minimum width=2.2cm,minimum height=1.4cm,align=center] at (-1.15,1.05){体号 1\\(低位 $01$)\\字 $1,5,9,13$};
  \node[draw,rounded corners=2pt,fill=SoftBlue,minimum width=2.2cm,minimum height=1.4cm,align=center] at (1.15,1.05){体号 2\\(低位 $10$)\\字 $2,6,10,14$};
  \node[draw,rounded corners=2pt,fill=SoftBlue,minimum width=2.2cm,minimum height=1.4cm,align=center] at (3.45,1.05){体号 3\\(低位 $11$)\\字 $3,7,11,15$};

  % ===== ④ 说明 =====
  \node[font=\scriptsize,text=TealDark,align=center] at (0,-0.75){连续字地址依次落入不同存储体 $\Rightarrow$ 可流水访问（每隔 $T/m$ 启动下一体）};
\end{tikzpicture}
\end{center}
...
\begin{center}
\begin{tikzpicture}[>=stealth, font=\scriptsize]
  \node[draw, rounded corners=2pt, fill=SoftBlue!40, minimum width=2cm, minimum height=1.1cm] (cpu) at (0,0) {CPU};
  \node[draw, rounded corners=2pt, fill=SoftTeal!40, minimum width=2.6cm, minimum height=1.1cm] (cache) at (4.8,0) {Cache（SRAM）};
  \node[draw, rounded corners=2pt, fill=SoftGray!50, minimum width=3.2cm, minimum height=1.1cm] (mem) at (10.4,0) {主存（DRAM）};
  \draw[<->, Navy, thick] (cpu) -- node[above, font=\scriptsize] {字传输（Word）} (cache);
  \draw[<->, Burgundy, thick] (cache) -- node[above, font=\scriptsize] {块传输（Block）} (mem);
\end{tikzpicture}
\end{center}
...
\node[draw, rectangle, minimum width=4.6cm, minimum height=0.9cm, fill=gray!10] at (4.95,1.0) {主存地址划分（按字节编址）};
\node[draw, rectangle, minimum width=3.4cm, minimum height=0.8cm, fill=blue!15] at (1.8,0) {标记（Tag）};
\node[draw, rectangle, minimum width=2.9cm, minimum height=0.8cm, fill=green!15] at (4.95,0) {索引（Index）};
\node[draw, rectangle, minimum width=3.4cm, minimum height=0.8cm, fill=yellow!15] at (8.1,0) {块内偏移（Offset）};
\end{tikzpicture}
\end{center}
...
\begin{center}
\begin{tikzpicture}[>=stealth]
\node[draw, rectangle, minimum width=5.2cm, minimum height=0.9cm, fill=gray!10] at (5.0,1.0) {主存地址划分（$W$ 路组相联）};
\node[draw, rectangle, minimum width=4.0cm, minimum height=0.8cm, fill=blue!15] at (2.0,0) {标记（Tag）};
\node[draw, rectangle, minimum width=3.5cm, minimum height=0.8cm, fill=green!15] at (5.75,0) {组号（Set Index）};
\node[draw, rectangle, minimum width=4.5cm, minimum height=0.8cm, fill=yellow!15] at (9.75,0) {块内偏移（Offset）};
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}[>=stealth, font=\scriptsize]
  % (a) 写直达
  \node[draw,rounded corners=2pt,fill=SoftGray,minimum width=1.8cm,minimum height=0.7cm] (cpua) at (0,2.1){CPU};
  \node[draw,rounded corners=2pt,fill=SoftTeal,minimum width=2.2cm,minimum height=0.7cm] (cacha) at (2.9,2.1){Cache 行};
  \node[draw,rounded corners=2pt,fill=SoftGray,minimum width=2.2cm,minimum height=0.7cm] (mema) at (6.2,2.1){主存};
  \draw[->] (cpua) -- node[above,font=\scriptsize]{写} (cacha);
  \draw[->] (cacha) -- node[above,font=\scriptsize]{同时写} (mema);
  \node[font=\scriptsize,Burgundy!80,align=center] at (-1.4,2.1){(a) 写直达\\每次写都更新主存};
  % (b) 写回
  \node[draw,rounded corners=2pt,fill=SoftGray,minimum width=1.8cm,minimum height=0.7cm] (cpub) at (0,0.3){CPU};
  \node[draw,rounded corners=2pt,fill=SoftTeal,minimum width=2.2cm,minimum height=0.7cm] (cachb) at (2.9,0.3){Cache 行（置脏）};
  \node[draw,rounded corners=2pt,fill=SoftGray,minimum width=2.2cm,minimum height=0.7cm] (memb) at (6.2,0.3){主存};
  \draw[->] (cpub) -- node[above,font=\scriptsize]{写} (cachb);
  \draw[->,dashed,Teal!70] (cachb) -- node[above,font=\scriptsize]{替换脏行时才写回} (memb);
  \node[font=\scriptsize,Burgundy!80,align=center] at (-1.4,0.3){(b) 写回\\只写 Cache，置脏位};
\end{tikzpicture}
\end{center}
...
\begin{center}
\begin{tikzpicture}[>=stealth]
\node[draw, rectangle, minimum width=4.0cm, minimum height=0.9cm, fill=gray!10] at (5.0,1.0) {虚拟地址（$N$ 位）};
\node[draw, rectangle, minimum width=5.0cm, minimum height=0.8cm, fill=blue!15] at (2.5,0) {虚拟页号（VPN）};
\node[draw, rectangle, minimum width=5.0cm, minimum height=0.8cm, fill=yellow!15] at (7.5,0) {页内偏移（VPO）};
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}[>=stealth, font=\scriptsize]
  % ===== 主流程（命中：垂直向下）=====
  \node[draw,rounded corners=2pt,fill=SoftGray,minimum width=2.4cm,minimum height=0.85cm,align=center] (VA) at (0,5.5){(1) 虚拟地址 VA\\VPN $|$ VPO};
  \draw[->,thick,Navy] (VA.south) -- (0,4.6);
  \node[draw,rounded corners=2pt,fill=SoftAmber,minimum width=2.6cm,minimum height=0.85cm,align=center] (TLB) at (0,4.15){(2) TLB 查找\\按 VPN 相联比较};
  \draw[->,thick,Navy] (TLB.south) -- (0,3.25);
  \node[font=\scriptsize,text=TealDark] at (-0.62,3.5){命中};
  \node[draw,rounded corners=2pt,fill=SoftGray,minimum width=2.4cm,minimum height=0.85cm,align=center] (PA) at (0,2.8){(3) 物理地址 PA\\PPN $|$ VPO};
  \draw[->,thick,Navy] (PA.south) -- (0,1.9);
  \node[draw,rounded corners=2pt,fill=SoftTeal,minimum width=2.6cm,minimum height=0.85cm,align=center] (CACH) at (0,1.45){(4) Cache 查找\\按 PA 的 Index};
  \draw[->,thick,Navy] (CACH.south) -- (0,0.55);
  \node[font=\scriptsize,text=TealDark] at (-0.62,0.8){命中};
  \node[draw,rounded corners=2pt,fill=SoftBlue,minimum width=2.0cm,minimum height=0.85cm,align=center] (DAT) at (0,0.1){(5) 返回数据};

  % ===== 缺失分支（水平向右）=====
  \draw[->,thick,Amber!85] (TLB.east) -- (2.9,4.15);
  \node[font=\scriptsize,text=Amber!90] at (2.05,4.45){缺失};
  \node[draw,rounded corners=2pt,fill=Amber!15,minimum width=4.5cm,minimum height=0.85cm,align=center] at (5.2,4.15){TLB 缺失：访主存页表取 PTE\\（装入 TLB 后重新执行）};

  \draw[->,thick,Teal!85] (CACH.east) -- (2.9,1.45);
  \node[font=\scriptsize,text=Teal!90] at (2.05,1.75){缺失};
  \node[draw,rounded corners=2pt,fill=Teal!15,minimum width=4.5cm,minimum height=0.85cm,align=center] at (5.2,1.45){Cache 缺失：从主存/下级 Cache 装入块\\（装入后继续本次访问）};

  % ===== 底部说明 =====
  \node[font=\scriptsize,text=Navy,align=center] at (0,-0.95){TLB 与 Cache 都命中时，无需因本次访问再访问主存};
\end{tikzpicture}
\end{center}
...
\begin{enumerate}[leftmargin=*]
    ...
    \ifshowsolutions\par\noindent\textbf{解析：} 主存块号为 $\lfloor\mathtt{1234E8F8H}/8\rfloor$，直接映射行号等于块号模16，也就是取地址的第3--6位。低字节 $\mathtt{F8H}=11111000_2$，相应4位全为1，故行号为15。\boxed{\textbf{答案：第3项}}\else \answerarea \fi
    ...
    \ifshowsolutions\par\noindent\textbf{解析：} 主存块号为 $\lfloor129/32\rfloor=4$；Cache 有 $16/2=8$ 组，故组号为 $4\bmod8=4$。\boxed{\textbf{答案：第3项}}\else \answerarea \fi
    ...
    \ifshowsolutions\par\noindent\textbf{解析：} 主存为 $256\text{ MB}=2^{28}$ B，地址28位；块内偏移6位，直接映射的8行需要3位行号，故 Tag 为 $28-6-3=19$ 位。\boxed{\textbf{答案：第3项}}\else \answerarea \fi
    ...
    \ifshowsolutions\par\noindent\textbf{解析：} 在容量和块大小相同的条件下，提高相联度通常减少或不增加冲突缺失；组相联的冲突缺失率不应被断言为高于直接映射。\boxed{\textbf{答案：第4项}}\else \answerarea \fi
    ...
    \ifshowsolutions\par\noindent\textbf{解析：} 直接映射的硬件简单，但不能保证所谓“利用率最高”；特定地址模式反而可能频繁冲突。全相联仍需比较 Tag；一行一组退化为直接映射，一组包含全部行退化为全相联。\boxed{\textbf{答案：第1项}}\else \answerarea \fi
    ...
    \ifshowsolutions\par\noindent\textbf{解析：} 4个连续字分布到4个存储体。第一个字在40 ns后就绪，随后总线每10 ns传一个字，总时间为 $40+(4-1)\times10=70$ ns。\boxed{\textbf{答案：第2项}}\else \answerarea \fi
    ...
    \ifshowsolutions\par\noindent\textbf{答案：} 6，7，19。Cache 共 $32\text{ KiB}/64\text{ B}=512$ 行，4路时有128组，故组号7位，Tag$=32-7-6=19$位。\else \answerarea \fi
    ...
    \ifshowsolutions\par\noindent\textbf{答案：} 400 MB/s。8个存储体恰好满足 $m\tau=T$，流水稳定后每10 ns传4 B，故带宽为 $4/(10\times10^{-9})=4\times10^8$ B/s。\else \answerarea \fi

    \item \textbf{（简答题）}
    简述时间局部性和空间局部性的含义，并各举出一个程序中的典型例子。说明为什么存储层次结构依赖于局部性原理。

    \ifshowsolutions\par\noindent\textbf{答案：} 时间局部性指刚访问过的数据或指令近期很可能再次访问，例如循环中的计数变量和循环体指令；空间局部性指访问某地址后，邻近地址近期很可能被访问，例如顺序扫描数组。Cache、TLB 和虚拟存储只保存完整地址空间的一小部分，正是利用程序访问集中在近期工作集及邻近块的性质，才可能以较小、较快的上层存储获得较高命中率。\else \answerarea \fi

    \item \textbf{（简答题）}
    简述 Cache 的写直达（Write-Through）和写回（Write-Back）策略的区别，并说明它们各自通常搭配的写缺失（Write Miss）策略是什么，为什么这样搭配。

    \ifshowsolutions\par\noindent\textbf{答案：} 写直达在 Cache 命中写时同时更新下层存储，内存副本较新、控制简单，但写流量较大，常与非写分配搭配，避免仅为一次写入先读入整块。写回只更新 Cache 并置脏位，块被替换时才写回下层，降低写流量，通常与写分配搭配，使后续对同一块的多次写都在 Cache 中完成。这些是常见组合而非体系结构必须遵守的唯一组合，题目若指定其他策略应以题设为准。\else \answerarea \fi
...
\begin{enumerate}[leftmargin=*]
    ...
    (3) 全相联 Cache 可同时容纳4个不同块，前四次装满后其余访问同样全部命中，命中率仍为 $11/15$。提高相联度只减少潜在冲突；本序列在2路映射下本来就没有冲突替换，因此命中率不变。\else \answerarea \fi
    ...
    (3) 没有任何组发生容量超过4路的替换，故 FIFO 与 LRU 都命中3次；替换策略只有在某组需要淘汰块时才会造成差异。\else \answerarea \fi
    ...
    (4) 按题设串行计时，总延迟为 $1+2+15=18$ 个周期。这里15周期已经包含 L2 命中以及回填 L1 的时间，不能重复再加一次回填。\else \answerarea \fi
    ...
    (3) 真 LRU 每次命中都可能更新多行之间的全序关系，比较和更新逻辑随相联度快速增长，影响面积、功耗和关键路径。树形伪 LRU 只用约 $k-1$ 位并沿树更新，成本更低，通常能取得接近 LRU 的效果。\else \answerarea \fi
    ...
    (4) 新启动间隔为 $\max(20,40/4)=20$ ns，最大带宽降为 $4/20\text{ ns}=200$ MB/s。此时只需 $m\geqslant T/\tau=2$ 就能使总线成为瓶颈，继续增加存储体不会再提高这条总线上的连续传输带宽。\else \answerarea \fi
    ...
    (4) 虚拟地址 \texttt{0x00123456} 的 VPN 为 \texttt{0x00123}、页内偏移为 \texttt{0x456}。TLB 命中得到 PPN=\texttt{0x0AB}，拼成物理地址 \texttt{0x0AB456}。对该物理地址划分可得块内偏移 $=\texttt{0x16}$，Cache 组号 $=(\texttt{0x0AB456}\gg5)\mathbin{\&}\texttt{0xFF}=\texttt{0xA2}$，Tag $=\texttt{0x0AB456}\gg13=\texttt{0x55}$。Cache 在组 \texttt{0xA2} 的两路中比较 Tag，命中 \texttt{0x55} 后按块内偏移返回数据。\else \answerarea \fi
...
for (int j = 0; j < N; j++)
    ...
    (4) 4096 B 的步长通常正好跨页，连续内层迭代会触及大量不同虚页，容易造成 TLB 容量抖动和更多页表遍历；若物理内存不能容纳工作集，还会增加缺页和磁盘换入换出。具体是否真正缺页仍取决于驻留集和物理内存，不能仅由访问顺序断言。\else \answerarea \fi
