...
\begin{itemize}
    ...
    \item \textbf{延伸思考：}现代处理器的 Cache 设计远不止 408 范围——多级 Cache 的包含策略（Inclusive/Exclusive/NINE）、硬件预取（Stride/Stream prefetcher）、非阻塞 Cache（MSHR）等。但 408 的核心始终是「给出参数$\to$划分地址位$\to$模拟访问序列$\to$统计算命中率」这一基本功，务必练熟。
\end{itemize}
