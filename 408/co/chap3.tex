...
\begin{tikzpicture}[>=stealth, font=\scriptsize]
  % bank selector on the left
  \node[draw,rounded corners=2pt,fill=SoftGray,minimum width=2.2cm,minimum height=1.5cm,align=center] (dec) at (-3.6,0.6){地址译码\\（低位 2 位选体）};
  \node[draw,rounded corners=2pt,fill=SoftTeal,minimum width=5.6cm,minimum height=0.68cm,align=center] (b0) at (1.6,1.7){Bank 0（低位 $00$）：字地址 $0,4,8,12\ldots$};
  \node[draw,rounded corners=2pt,fill=SoftGray,minimum width=5.6cm,minimum height=0.68cm,align=center] (b1) at (1.6,0.9){Bank 1（低位 $01$）：字地址 $1,5,9,13\ldots$};
  \node[draw,rounded corners=2pt,fill=SoftGray,minimum width=5.6cm,minimum height=0.68cm,align=center] (b2) at (1.6,0.1){Bank 2（低位 $10$）：字地址 $2,6,10,14\ldots$};
  \node[draw,rounded corners=2pt,fill=SoftAmber,minimum width=5.6cm,minimum height=0.68cm,align=center] (b3) at (1.6,-0.7){Bank 3（低位 $11$）：字地址 $3,7,11,15\ldots$};
  \draw[thick,Navy!70] (-2.5,0.6) -- (1.6,1.7) node[midway,above,font=\scriptsize,black!70]{};
  \draw[thick,Navy!70] (-2.5,0.6) -- (1.6,0.9);
  \draw[thick,Navy!70] (-2.5,0.6) -- (1.6,0.1);
  \draw[thick,Navy!70] (-2.5,0.6) -- (1.6,-0.7);
  \node[font=\scriptsize,black!70] at (1.6,-1.55){连续字地址依次落入不同体，可流水访问（每隔 $T/m$ 启动下一体）};
\end{tikzpicture}
\end{center}
...
\begin{center}
\begin{tikzpicture}[>=stealth, font=\scriptsize]
  \node[draw,rounded corners=2pt,fill=SoftGray,minimum width=2.0cm,minimum height=0.85cm,align=center] (va) at (-3.6,0.9){虚拟地址 VA\\VPN | VPO};
  \node[draw,rounded corners=2pt,fill=SoftAmber,minimum width=2.2cm,minimum height=0.85cm,align=center] (tlb) at (-0.4,0.9){TLB 查找\\（按 VPN 相联比较）};
  \node[draw,rounded corners=2pt,fill=SoftGray,minimum width=2.0cm,minimum height=0.85cm,align=center] (pa) at (2.6,0.9){物理地址 PA\\PPN | VPO};
  \node[draw,rounded corners=2pt,fill=SoftTeal,minimum width=2.2cm,minimum height=0.85cm,align=center] (ca) at (5.6,0.9){Cache 查找\\（按 PA 的 Index/Tag）};
  \node[draw,rounded corners=2pt,fill=SoftBlue,minimum width=1.7cm,minimum height=0.85cm,align=center] (dat) at (8.2,0.9){返回数据};
  \draw[->] (va) -- (tlb);
  \draw[->] (tlb) -- node[above,font=\scriptsize]{命中} (pa);
  \draw[->] (pa) -- (ca);
  \draw[->] (ca) -- node[above,font=\scriptsize]{命中} (dat);
  \draw[->,Amber!70] (tlb) to[out=180,in=90] (-3.6,-0.35) node[align=center,font=\scriptsize,black!70]{};
  \node[align=center,font=\scriptsize,black!70] at (-3.6,-0.7){TLB 缺失：访主存页表\\装入 PTE 后重新查找};
  \node[align=center,font=\scriptsize,black!70] at (5.6,-0.7){Cache 缺失：从主存/\\下级 Cache 装入块};
  \draw[->,Teal!70] (ca) to[out=270,in=90] (5.6,-0.2);
  \draw[->,Amber!70] (tlb) to[out=270,in=90] (-0.4,-0.2);
\end{tikzpicture}
\end{center}
