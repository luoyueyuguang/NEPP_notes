...
\begin{enumerate}[leftmargin=*]
    ...
    \item \textbf{（2020年·选择题·第17题）}
    下列关于 Cache 映射方式的叙述中，正确的是\_\_\_\_\_\_。
    \begin{enumerate}
    ...
    \item \textbf{（2023年·选择题·第15题）}
    某计算机主存按字节编址，由 4 个存储体构成，采用低位交叉编址。每个存储体的存取周期为 $40\,\mathrm{ns}$，
    总线传输周期为 $10\,\mathrm{ns}$。CPU 连续从地址 0 开始读取 16 个字节，需要的总时间是\_\_\_\_\_\_。
    \begin{enumerate}
        \item $40\,\mathrm{ns}$
        \item $70\,\mathrm{ns}$
        \item $80\,\mathrm{ns}$
...
\begin{enumerate}[leftmargin=*]
    ...
    \item \textbf{（2012年·综合题·第17题改编）Cache 映射与 LRU 替换。}
    ...
    \item \textbf{（2014年·选择题·第16题改编）Cache 地址结构与命中分析。}
    ...
    \item \textbf{（2013/2017/2021年综合）TLB + Cache + 页表综合。}
    ...
    \begin{enumerate}
        ...
        L1 命中时间 2 个周期，L2 命中时间（含从 L2 调入 L1）15 个周期，求这次访问的总延迟。
    \end{enumerate}

    \item \textbf{（2016年·选择题·第17题改编）LRU 替换算法。}
    ...
    \begin{enumerate}
        ...
        \item[(2)] 采用 LRU 替换策略时，Cache 需要记录哪些信息？需要多少位来记录替换信息（设该组有 $k$ 行）？
        \item[(3)] 为何在实际处理器中，4 路组相联通常使用近似 LRU（如二叉树伪 LRU）而非真正的 LRU？
    \end{enumerate}

    \item \textbf{（2023年·选择题·第15题改编）低位交叉编址综合。}
    某计算机主存按字节编址，由 $m = 4$ 个存储体构成，采用低位交叉编址。每个存储体的存取周期 $T = 40\,\mathrm{ns}$，
    总线传输周期 $\tau = 10\,\mathrm{ns}$，每个存储体一次可读写 4 字节。
    \begin{enumerate}
        \item[(1)] 求该存储器在流水线方式下的最大带宽（MB/s）。
        \item[(2)] 若 CPU 连续从地址 0 开始读取 64 字节数据，需要多少时间？
        \item[(3)] 若改用高位交叉编址，同样读取 64 字节连续数据，需要多少时间？比较 (2) 和 (3) 的结果，说明低位交叉编址的优势。
        \item[(4)] 若总线传输周期变为 $20\,\mathrm{ns}$，而存取周期仍为 $40\,\mathrm{ns}$，最大带宽有何变化？这个变化对 $m$ 的取值有何启示？
    \end{enumerate}

    \item \textbf{（2021年·选择题·第17题改编）虚拟存储器与 Cache 综合。}
    ...
    设 Cache 块大小为 64 字节，\texttt{int} 占 4 字节，$N = 1024$，数组 \texttt{a} 按行优先存储。
...
\begin{itemize}
    \item \textbf{Patterson \& Hennessy《计算机组成与设计（RISC-V 版）》}第 5 章「大而快：层次化存储系统」——完整论述存储层次、Cache 四种映射方式和性能分析，含 RISC-V 处理器的实际 Cache 设计。\textbf{必读：5.4--5.5 节的 Cache 地址映射与性能分析是 408 的直接来源。}
    ...
    \item \textbf{唐朔飞《计算机组成原理》}第 4 章「存储器」——4.1 概述，4.2 主存储器（SRAM/DRAM 对比、刷新方式），4.3 高速缓冲存储器（Cache 地址映射、替换算法、写策略），4.5 辅助存储器。唐教材的符号体系与 408 完全对齐。

    \item \textbf{延伸思考：}现代处理器的 Cache 设计远不止 408 范围——多级 Cache 的包含策略（Inclusive/Exclusive/NINE）、硬件预取（Stride/Stream prefetcher）、非阻塞 Cache（MSHR）等。但 408 的核心始终是「给出参数→划分地址位→模拟访问序列→统计算命中率」这一基本功，务必练熟。
\end{itemize}
