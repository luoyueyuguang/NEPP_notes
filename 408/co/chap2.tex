...
\begin{enumerate}[leftmargin=*]
    ...
    \item \textbf{（2011年·选择题·第14题）} float 型数据通常用 IEEE 754 单精度浮点数格式表示。若编译器将 float 型变量 $x$ 分配在一个 32 位浮点寄存器 FR1 中，且 $x=-8.25$，则 FR1 的内容是（\quad）。
    \begin{enumerate}
        \item C104 0000H
        \item C242 0000H
        \item C184 0000H
...
\vspace{10pt}
    ...
    \item \textbf{（2022年·选择题·第13题）} 32 位补码所能表示的整数范围是（\quad）。
    \begin{enumerate}
...
\vspace{10pt}
    ...
    \item \textbf{（2023年·选择题·第13题）} 若 short 型变量 $x=-8190$，则 $x$ 的机器数为（\quad）。
    \begin{enumerate}
        \item E002H
        \item E001H
        \item 9FFFH
...
\vspace{10pt}
    ...
    \item \textbf{（2018年·选择题·第14题）} IEEE 754 单精度浮点格式表示的数中，最小的规格化正数是（\quad）。
    \begin{enumerate}
        \item $1.0 \times 2^{-126}$
        \item $1.0 \times 2^{-127}$
        \item $1.0 \times 2^{-128}$
...
\vspace{10pt}
    ...
    \item \textbf{（2022年·选择题·第14题）} $-0.4375$ 的 IEEE 754 单精度浮点数表示为（\quad）。
    \begin{enumerate}
        \item BEE0 0000H
        \item BF60 0000H
        \item BF70 0000H
...
\vspace{10pt}
    ...
    \item \textbf{（2012年·选择题·第14题）} float 类型（即 IEEE 754 单精度浮点数格式）能表示的最大正整数是（\quad）。
    \begin{enumerate}
        \item $2^{127} - 2^{104}$
        \item $2^{127} - 2^{103}$
        \item $2^{128} - 2^{104}$
...
\vspace{10pt}
    ...
    \item \textbf{（2014年·选择题·第13题）} 某 IEEE 754 单精度浮点数的机器码为 42E4 8000H，则它的真值为（\quad）。
    \begin{enumerate}
        \item 114.25
        \item 57.125
        \item 228.5
...
\vspace{10pt}
    ...
    \item \textbf{（2009年·选择题·第13题）} 浮点数加减运算过程中，「对阶」操作的原则是（\quad）。
    \begin{enumerate}
        \item 将较小的阶码对齐到较大的阶码
        \item 将较大的阶码对齐到较小的阶码
...
\vspace{10pt}
    ...
    \item \textbf{（2010年·选择题·第18题）} 假定变量 $i$、$f$ 和 $d$ 的数据类型分别为 int、float 和 double（int 用补码表示，float 和 double 分别用 IEEE 754 单精度和双精度浮点数格式表示），已知 $i=785$，$f=1.5678\text{e}3$，$d=1.5\text{e}100$。若在 32 位机器中执行下列关系表达式，则结果为「真」的是（\quad）。
    \begin{center}
...
\vspace{10pt}
    ...
    \item \textbf{（2015年·选择题·第14题）} 下列有关 IEEE 754 浮点数的叙述中，正确的是（\quad）。
    \begin{enumerate}
...
\begin{enumerate}[leftmargin=*]
    ...
    \item \textbf{（2020年·选择题·第14题）} 已知带符号整数用补码表示，float 型数据用 IEEE 754 标准表示，假定变量 $x$ 的类型只能是 int 或 float。当 $x$ 的机器数为 C800 0000H 时，$x$ 的值可能是（\quad）。
    \begin{enumerate}
        \item $-7 \times 2^{27}$
        \item $-2^{126}$
        \item $-2^{27}$
...
\vspace{10pt}
    ...
    \item \textbf{（2025年·选择题·第13题）} 已知 float 型变量用 IEEE 754 单精度浮点数格式表示。若 float 型变量 $x$ 的机器数为 4730 0000H，则 $x$ 的值为（\quad）。
    \begin{enumerate}
    ...
    \textbf{解析：} $\text{47300000H} = 0100\;0111\;0011\;0000\;\ldots\;0000_2$。$S=0$，$E = 1000\;1110_2 = 142$，$e = 142 - 127 = 15$。$M$ 小数部分 = \texttt{011 0000 0000 0000 0000 0000}（即 $.011_2$）。尾数真值 $= 1.011_2 = 1 + 0.25 + 0.125 = 1.375$。故 $x = 1.375 \times 2^{15}$。等等——$e=15$，所以是 $2^{15}$。但仔细算：$E=10001110_2$，让我重新计算。

    $E = 1000\;1110_2 = 128 + 8 + 4 + 2 = 142$？不对，$128+14 = 142$。$e=142-127=15$。所以 $x = 1.375 \times 2^{15}$。选项 (D) 是 $1.375 \times 2^{15}$。

    等一下，让我重新仔细算。$0100\;0111\;0011\;0000\;\ldots = \text{47300000H}$。
    $S = 0$
    $E = 1000\;1110_2 = 8\text{EH} = 8 \times 16 + 14 = 142$
    $e = 142 - 127 = 15$
    尾数 $M = 011\;0000\;\ldots$ 即 $0.011_2 = 0.375$
    数值 $= 1.375 \times 2^{15}$
    答案选 (D)。

    但如果是 $1.375 \times 2^{14}$ (B)，那 $e$ 必须是 14。让我检查 $E$ 是否可能为 $1000\;1101_2 = 141$。$\text{47300000H}$ 中第 2 字节是 $73$ = $0111\;0011_2$，$E$ 的位是 $0\;1000\;1110\;0$（从 bit 30 到 23）。是的 $E = 1000\;1110_2 = 142$。$e=15$。答案选 (D)。

    实际上我再检查一下十六进制：$47300000H = 0100\;0111\;0011\;0000\;0000\;0000\;0000\;0000_2$。IEEE 754 位序：
    bit 31 = 0 (S)
    bits 30-23 = 1000 1110 = 0x8E = 142 (E)
    bits 22-0 = 011 0000 0000 0000 0000 0000
    $e = 142 - 127 = 15$，$1.M = 1.011_2 = 1.375$，$x = 1.375 \times 2^{15}$。答案：(D)。\fi

\vspace{10pt}

    \item \textbf{（2025年·选择题·第14题）} 假设 8 位字长的计算机中，两个带符号整数 $x$ 和 $y$ 的补码表示分别为 $[x]_{\text{补}} = 6\text{AH}$，$[y]_{\text{补}} = 9\text{FH}$。则 $x+y$ 的结果和溢出情况是（\quad）。
    \begin{enumerate}
        \item 结果为 09H，无溢出
        \item 结果为 09H，有溢出
        \item 结果为 F5H，无溢出
    ...
    但用符号位法：正数 $+$ 负数，不可能溢出。故结果 $09\text{H}$，无溢出。答案选 (A)。

    等一下，我再算算：$6\text{AH} = 106$，$9\text{FH} = -97$，$106-97 = 9 = 09\text{H}$。没错，无溢出。答案 (A)。\fi
