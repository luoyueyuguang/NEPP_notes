...
\vspace{10pt}
    ...
    $0110\;1010 + 1001\;1111$：bit 6→7 有进位（$1+1=10$），bit 7→溢出位有进位（$0+1+1=10$）。$C_s=1$，$C_{s-1}=1$，$1 \oplus 1 = 0$，无溢出。
...
\vspace{10pt}
    ...
    \textbf{溢出判断三法：}
    \begin{itemize}
        \item \textbf{进位异或法：}最高位进位 $C_s = 1$，次高位进位 $C_{s-1} = 0$（bit 6→7：$1+0=0+$进位 0 = 0，无进位）。$C_s \oplus C_{s-1} = 1 \oplus 0 = 1$，溢出。
