...
\textbf{摩尔定律（Moore's Law）：} 在经典表述中，集成电路上可经济集成的晶体管数量大约每 18--24 个月增长一倍。它描述的是集成度趋势，并不等价于处理器性能按同一比例自动增长；功耗、存储墙和并行度都会影响实际性能。随着传统缩放放缓，现代处理器更多依靠多核、向量化和专用加速器提升性能。
\end{conclusion}
...
\begin{tikzpicture}[>=stealth, font=\scriptsize]
  \draw[thick,Navy!70] (-6.5,0) -- (6.5,0);
  \node[font=\scriptsize,black!70] at (0,-0.35){系统总线（地址总线 / 数据总线 / 控制总线）};
  \node[draw,rounded corners=2pt,fill=SoftGray,minimum width=1.7cm,minimum height=0.85cm] (in) at (-4.5,1.2){输入设备};
  \node[draw,rounded corners=2pt,fill=SoftBlue,minimum width=3.0cm,minimum height=1.5cm,align=center] (cpu) at (-1.5,1.2){\textbf{CPU}\\运算器 + 控制器};
  \node[draw,rounded corners=2pt,fill=SoftTeal,minimum width=1.7cm,minimum height=0.85cm] (mem) at (2.0,1.2){存储器};
  \node[draw,rounded corners=2pt,fill=SoftGray,minimum width=1.7cm,minimum height=0.85cm] (out) at (4.8,1.2){输出设备};
  \draw[Navy!70] (in.south) -- (-4.5,0);
  \draw[Navy!70] (cpu.south) -- (-1.5,0);
  \draw[Navy!70] (mem.south) -- (2.0,0);
  \draw[Navy!70] (out.south) -- (4.8,0);
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}[>=stealth, font=\scriptsize, node distance=0.3cm]
  \node[draw,rounded corners=2pt,fill=SoftGray,minimum width=1.7cm,minimum height=0.7cm] (f) at (0,0.4) {取指};
  \node[draw,rounded corners=2pt,fill=SoftGray,minimum width=1.7cm,minimum height=0.7cm] (d) at (2.4,0.4) {译码};
  \node[draw,rounded corners=2pt,fill=SoftGray,minimum width=1.7cm,minimum height=0.7cm] (e) at (4.8,0.4) {执行};
  \node[draw,rounded corners=2pt,fill=SoftGray,minimum width=1.7cm,minimum height=0.7cm] (w) at (7.2,0.4) {写回};
  \draw[->] (f) -- (d);
  \draw[->] (d) -- (e);
  \draw[->] (e) -- (w);
  \node[align=center,font=\scriptsize,black!70] at (0,-0.6){按 PC 所指地址\\取指令到 IR，PC 自增};
  \node[align=center,font=\scriptsize,black!70] at (2.4,-0.6){译码器分析操作码\\产生控制信号};
  \node[align=center,font=\scriptsize,black!70] at (4.8,-0.6){按控制信号完成\\数据传送或运算};
  \node[align=center,font=\scriptsize,black!70] at (7.2,-0.6){结果写回寄存器\\或存储器};
\end{tikzpicture}
\end{center}
...
\begin{center}
\begin{tikzpicture}[>=stealth, font=\scriptsize]
  \node[draw,rounded corners=2pt,fill=SoftTeal,minimum width=5.2cm,minimum height=0.62cm,align=center] (L5) at (0,2.6){$L_5$ 应用层（用户层）：API / 库调用};
  \node[draw,rounded corners=2pt,fill=SoftBlue,minimum width=5.2cm,minimum height=0.62cm,align=center] (L4) at (0,1.85){$L_4$ 高级语言级：C/C++/Java};
  \node[draw,rounded corners=2pt,fill=SoftAmber,minimum width=5.2cm,minimum height=0.62cm,align=center] (L3) at (0,1.1){$L_3$ 汇编语言级：助记符};
  \node[draw,rounded corners=2pt,fill=SoftGray,minimum width=5.2cm,minimum height=0.62cm,align=center] (L2) at (0,0.35){$L_2$ 操作系统级：系统调用 / 资源抽象};
  \node[draw,rounded corners=2pt,fill=SoftGray,minimum width=5.2cm,minimum height=0.62cm,align=center] (L1) at (0,-0.4){$L_1$ 传统机器语言级（二进制指令）};
  \node[draw,rounded corners=2pt,fill=Burgundy!12,minimum width=5.2cm,minimum height=0.62cm,align=center] (L0) at (0,-1.15){$L_0$ 微程序级 / 硬联逻辑（物理机）};
  \node[font=\scriptsize,black!70,anchor=west] at (3.0,1.475){翻译：$L_4\!\to\!L_1$（编译器/汇编器）};
  \node[font=\scriptsize,black!70,anchor=west] at (3.0,-0.775){解释：$L_1\!\to\!L_0$（微程序）};
  % 虚框标注：L2--L5 为虚拟机
  \draw[dashed,Burgundy!60,rounded corners=2pt] (-3.1,2.9) rectangle (3.1,-0.05);
  \node[font=\scriptsize,Burgundy!80] at (-3.1,3.05){$L_2$--$L_5$：虚拟机（由下层软件+硬件模拟）};
\end{tikzpicture}
\end{center}
...
\ifshowsolutions\par\noindent\textbf{解析：} 存储程序原理把指令和数据以同等地位放在统一编址的主存中，二者由指令执行语境区分；现代实现可在 Cache 层采用分离结构，但不改变该逻辑模型。\boxed{\textbf{答案：}(B)}\else \answerarea \fi
...
\ifshowsolutions\par\noindent\textbf{解析：} CPU 的核心组成是运算器和控制器，现代 CPU 通常还包含寄存器、Cache 等；主存不属于 CPU 的基本组成。\boxed{\textbf{答案：}(C)}\else \answerarea \fi
...
\ifshowsolutions\par\noindent\textbf{解析：} $\mathrm{MIPS}=f/(\mathrm{CPI}\times10^6)=1.2\times10^9/(2\times10^6)=600$。\boxed{\textbf{答案：}(A)}\else \answerarea \fi
...
\ifshowsolutions\par\noindent\textbf{解析：} 操作系统、编译器和数据库管理系统通常归为系统软件；浏览器属于应用软件。\boxed{\textbf{答案：}(D)}\else \answerarea \fi

\item \textbf{（填空题）} 在计算机系统的层次结构中，能直接被硬件识别和执行的层次是 \underline{\qquad\qquad} 级。汇编语言程序需要通过 \underline{\qquad\qquad} 翻译为机器语言。

\ifshowsolutions\par\noindent\textbf{答案：} 机器语言；汇编程序（汇编器）。\else \answerarea \fi

\item \textbf{（简答题）} 简述冯·诺依曼结构的主要特点，并说明现代计算机结构与其最主要的区别是什么。

\ifshowsolutions\par\noindent\textbf{答案：} 冯·诺依曼结构的要点包括：采用存储程序方式；指令和数据以二进制表示并存放在统一编址的存储器中；按地址访问；通常按指令顺序执行，除非遇到转移；由运算器、控制器、存储器、输入和输出五部分组成。现代通用计算机仍以该逻辑模型为基础，但通过指令/数据分离 Cache、流水线、乱序执行、多核和并行存储层次等手段缓解“冯·诺依曼瓶颈”。不能简单说现代计算机已经完全脱离冯·诺依曼结构。\else \answerarea \fi
...
=\frac{5\times10^9\times1.6}{2.5\times10^9}=3.2\text{ s}.
\]
\[
\mathrm{MIPS}=\frac{2.5\times10^9}{1.6\times10^6}=1562.5.
\]
\else \answerarea \fi
...
\item \textbf{（真题风格，单项选择题）} 某程序在计算机 A 上运行需 10 s，A 的主频为 2 GHz。计算机 B 执行该程序的指令条数与 A 相同，但平均 CPI 为 A 的 1.2 倍。若目标是在 B 上用 6 s 运行完该程序，则 B 所需的主频为
\begin{center}
\begin{tabular}{ll}
...
\ifshowsolutions\par\noindent\textbf{解析：} 指令条数相同，故
\[
\frac{T_B}{T_A}=\frac{\mathrm{CPI}_B}{\mathrm{CPI}_A}
\frac{f_A}{f_B}=\frac{6}{10}.
\]
代入 $\mathrm{CPI}_B/\mathrm{CPI}_A=1.2$、$f_A=2$ GHz，得 $f_B=4$ GHz。\boxed{\textbf{答案：}(C)}\else \answerarea \fi
...
\ifshowsolutions\par\noindent\textbf{解析：} 加权平均 CPI 为 $(2\times1+3\times2+5\times3)/(2+3+5)=23/10=2.3$。\boxed{\textbf{答案：}(C)}\else \answerarea \fi
...
\ifshowsolutions\par\noindent\textbf{答案：}
(1) $\mathrm{CPI}=f/(\mathrm{MIPS}\times10^6)=3.0\times10^9/(1200\times10^6)=2.5$。

(2) $T=3.6\times10^9\times2.5/(3.0\times10^9)=3$ s。

(3) 主频不变时，执行时间比为 $0.8\times1.1=0.88$。优化后为 $3\times0.88=2.64$ s，比原来快12\%。\else \answerarea \fi
...
\ifshowsolutions\par\noindent\textbf{答案：}
(1) 高级语言源程序通常由编译器翻译为汇编代码或目标代码；汇编程序把汇编语言翻译为机器语言，链接器再把目标模块和库连接为可执行程序。某些语言也可先翻译为中间代码，再由解释器或虚拟机逐条解释/即时编译执行。机器语言最终由硬件直接执行。

(2) Java 编译器先把源程序翻译为与具体 ISA 无关的字节码；JVM 对字节码进行解释或 JIT 编译，并向程序提供统一的类加载、内存管理和运行时服务，因此同一字节码可运行在不同物理机器上的 JVM 实现中。

(3) OS 通过系统调用和资源管理把裸机扩展成更方便、安全的抽象机器，例如进程/线程、虚拟地址空间、文件、目录、套接字和逻辑设备；这些对象由软件构造，不是硬件直接提供的物理实体。\else \answerarea \fi
...
(1) 平均 CPI 为
\[
1\times0.45+2\times0.30+4\times0.15+3\times0.10=1.95.
\]
(2) $\mathrm{MIPS}=2000/1.95\approx1025.64$。

(3) 新平均 CPI 为 $1.95-4\times0.15+2\times0.15=1.65$。在指令条数不变时，性能提升倍数为
\[
\frac{T_{\text{old}}}{T_{\text{new}}}
=\frac{1.95/(2.0\text{ GHz})}{1.65/(2.5\text{ GHz})}
\approx1.477.
\]
即性能约提高到原来的1.48倍。\else \answerarea \fi
