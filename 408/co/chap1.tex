...
\ifshowsolutions\par\noindent\textbf{解析：} 存储程序原理把指令和数据以同等地位放在统一编址的主存中，二者由指令执行语境区分；现代实现可在 Cache 层采用分离结构，但不改变该逻辑模型。\boxed{\textbf{答案：}(B)}\fi
...
\ifshowsolutions\par\noindent\textbf{解析：} CPU 的核心组成是运算器和控制器，现代 CPU 通常还包含寄存器、Cache 等；主存不属于 CPU 的基本组成。\boxed{\textbf{答案：}(C)}\fi
...
\ifshowsolutions\par\noindent\textbf{解析：} $\mathrm{MIPS}=f/(\mathrm{CPI}\times10^6)=1.2\times10^9/(2\times10^6)=600$。\boxed{\textbf{答案：}(A)}\fi
...
\ifshowsolutions\par\noindent\textbf{解析：} 操作系统、编译器和数据库管理系统通常归为系统软件；浏览器属于应用软件。\boxed{\textbf{答案：}(D)}\fi

\item \textbf{（填空题）} 在计算机系统的层次结构中，能直接被硬件识别和执行的层次是 \underline{\qquad\qquad} 级。汇编语言程序需要通过 \underline{\qquad\qquad} 翻译为机器语言。

\ifshowsolutions\par\noindent\textbf{答案：} 机器语言；汇编程序（汇编器）。\fi

\item \textbf{（简答题）} 简述冯·诺依曼结构的主要特点，并说明现代计算机结构与其最主要的区别是什么。

\ifshowsolutions\par\noindent\textbf{答案：} 冯·诺依曼结构的要点包括：采用存储程序方式；指令和数据以二进制表示并存放在统一编址的存储器中；按地址访问；通常按指令顺序执行，除非遇到转移；由运算器、控制器、存储器、输入和输出五部分组成。现代通用计算机仍以该逻辑模型为基础，但通过指令/数据分离 Cache、流水线、乱序执行、多核和并行存储层次等手段缓解“冯·诺依曼瓶颈”。不能简单说现代计算机已经完全脱离冯·诺依曼结构。\fi
...
\]
代入 $\mathrm{CPI}_B/\mathrm{CPI}_A=1.2$、$f_A=2$ GHz，得 $f_B=4$ GHz。\boxed{\textbf{答案：}(C)}\fi
...
\ifshowsolutions\par\noindent\textbf{解析：} 加权平均 CPI 为 $(2\times1+3\times2+5\times3)/(2+3+5)=23/10=2.3$。\boxed{\textbf{答案：}(C)}\fi
...
(3) 主频不变时，执行时间比为 $0.8\times1.1=0.88$。优化后为 $3\times0.88=2.64$ s，比原来快12\%。\fi
...
(3) OS 通过系统调用和资源管理把裸机扩展成更方便、安全的抽象机器，例如进程/线程、虚拟地址空间、文件、目录、套接字和逻辑设备；这些对象由软件构造，不是硬件直接提供的物理实体。\fi
