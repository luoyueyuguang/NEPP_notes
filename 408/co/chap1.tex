...
\begin{enumerate}
    \item \textbf{取指（Fetch）}：按 PC 所指地址从存储器取指令→IR，PC 自增指向下一条指令。
...
\begin{enumerate}
    ...
    \item \textbf{主频（Clock Frequency）}：CPU 内部时钟信号的频率，以 Hz 为单位。主频 $f$ 决定了时钟周期 $T = 1/f$。例如主频 3.0 GHz → 时钟周期 $T \approx 0.33\text{ ns}$。
...
主频 $f$ & 时钟信号频率 & GHz 级 \\
$T_{\text{clk}}$ & $1/f$，时钟周期 & 例如 3 GHz → 0.33 ns \\
CPI & 每条指令的平均时钟数 & 越小越好 \\
...
\begin{enumerate}
    ...
    \item \textbf{三个语言级别}：机器语言（二进制）、汇编语言（助记符→汇编器翻译）、高级语言（编译器翻译或解释执行）。注意区分编译与解释。
