...
\begin{axis}[
  ...
  xlabel={$x$}, ylabel={$y$},
  xmin=-1.1, xmax=1.1, ymin=-6, ymax=6,
  xtick={-1,0,1}, ytick={-5,-4,-3,-2,-1,1,2,3,4,5},
  legend pos=north east, font=\footnotesize,
  domain=-1.1:1.1, samples=120,
]
\addplot[blue,thick] {2*exp(-x)};
\addlegendentry{$y=2e^{-x}$}
\addplot[red,thick] {exp(-x)};
\addlegendentry{$y=e^{-x}$}
\addplot[green!55!black,thick] {0.5*exp(-x)};
\addlegendentry{$y=0.5e^{-x}$}
\addplot[blue!45,thick] {-2*exp(-x)};
\addplot[red!55,thick] {-exp(-x)};
\addplot[green!65!black,thick] {-0.5*exp(-x)};
\end{axis}
\end{tikzpicture}
\end{center}

\noindent\textbf{拓展（数值解 ODE）:}当 $y'+p(x)y=q(x)$ 这类方程无法（或难以）写出显式解时，数值方法把连续方程“离散化”：在网格 $x_n=x_0+nh$ 上，Euler 法 $y_{n+1}=y_n+h\,f(x_n,y_n)$ 沿切线步进，Runge--Kutta（如 RK4）则用若干函数值加权获得更高阶精度。步长 $h$ 越小越精确，但过小的 $h$ 会放大舍入误差并引发稳定性问题（刚性方程、A-稳定）。这套“离散化+迭代”正是动力学模拟与神经网络常微分方程 \texttt{neural ODE} 的起点。

\begin{center}
\begin{tikzpicture}
\begin{axis}[width=0.82\linewidth, height=5.4cm,
  xlabel={$x$}, ylabel={$y$},
  xmin=-0.1, xmax=1.3, ymin=0.9, ymax=2.9,
  xtick={0,0.25,0.5,0.75,1}, ytick={1,1.5,2,2.5,3},
  legend pos=north west, font=\footnotesize,
  domain=0:1.2, samples=120]
\addplot[blue,thick] {exp(x)};
\addlegendentry{精确解 $y=e^x$}
\addplot[red,thick,mark=*] coordinates {(0,1) (0.25,1.25) (0.5,1.5625) (0.75,1.953125) (1,2.441406)};
\addlegendentry{Euler 折线 $h=0.25$}
\end{axis}
\end{tikzpicture}
\end{center}
\centerline{\footnotesize 解 $y'=y,\ y(0)=1$：Euler 法（$h=0.25$）的折线点列逐步偏离精确解 $e^x$；$h$ 越小偏离越小，但步数增多、舍入误差累积。}
...
\begin{axis}[
  width=0.82\linewidth, height=5.4cm,
  xlabel={$x$}, ylabel={$y$},
  xmin=0, xmax=6, ymin=-1.6, ymax=1.6,
  xtick={0,1,2,3,4,5,6}, ytick={-1,0,1},
  legend pos=north east, font=\footnotesize,
  domain=0:6, samples=200,
]
\addplot[dashed,gray!70,thick] {exp(-x)};
\addlegendentry{$y=e^{\alpha x}$}
\addplot[dashed,gray!70,thick] {-exp(-x)};
\addlegendentry{$y=-e^{\alpha x}$}
\addplot[blue,thick] {exp(-x)*sin(deg(2*x))};
\addlegendentry{$y=e^{-x}\sin 2x$}
\end{axis}
\end{tikzpicture}
\end{center}
...
\begin{axis}[
  width=0.82\linewidth, height=5.4cm,
  xlabel={$t$}, ylabel={$T$},
  xmin=0, xmax=60, ymin=0, ymax=105,
  xtick={0,10,20,30,40,50,60}, ytick={0,20,40,60,80,100},
  legend pos=north east, font=\footnotesize,
  domain=0:60, samples=160,
]
\addplot[dashed,gray!70,thick] {20};
\addlegendentry{$T=20$ (环境温度)}
\addplot[blue,thick] {20+80*exp(-0.0693*x)};
\addlegendentry{$T(t)=20+80e^{-kt}$}
\addplot[only marks,red,mark size=2pt] coordinates {(10,60) (30,30)};
\node[font=\scriptsize,red] at (axis cs:10,66) {$(10,60)$};
\node[font=\scriptsize,red] at (axis cs:33,26) {$(30,30)$};
\end{axis}
\end{tikzpicture}
\end{center}
...
\[
\begin{aligned}
I''(x) &= C_{1}e^{x} + \frac{\mathrm{d}^{2}}{\mathrm{d}x^{2}}\Bigl[e^{-x/2}\bigl(C_{2}\cos\alpha x + C_{3}\sin\alpha x\bigr)\Bigr] - \frac{1}{2}e^{-x},\quad \alpha=\frac{\sqrt{3}}{2}.
\end{aligned}
\]
...
C_{1} - \frac{1}{2}C_{2} - \frac{\sqrt{3}}{2}C_{3} = \frac{3}{2}.
\end{cases}
\]
(2)+(3) 得 $2C_{1} - C_{2} = 1$。与 (1) 联立：$C_{1}+C_{2}=\frac{1}{2}$，解得 $C_{1}=\frac{1}{2}$，$C_{2}=0$。
(2)-(3) 得 $\sqrt{3}C_{3} = -2$，故 $C_{3}=-\frac{2}{\sqrt{3}}$。

因此
\[
I(x) = \frac{1}{2}e^{x} - \frac{2}{\sqrt{3}}e^{-x/2}\sin\frac{\sqrt{3}}{2}x - \frac{1}{2}e^{-x}.
\]
最终 $y(x)=I'(x)$。求导：
\[
\begin{aligned}
y(x)
&=\frac12e^x+e^{-x/2}
\left(\frac1{\sqrt3}\sin\frac{\sqrt3}{2}x
-\cos\frac{\sqrt3}{2}x\right)
+\frac12e^{-x}.
\end{aligned}
\]
可验证 $y(0)=\frac{1}{2}-1+\frac{1}{2}=0$，$y'(0)=1$，满足条件。
