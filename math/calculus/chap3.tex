...
\begin{axis}[width=0.8\linewidth, height=5.4cm,
  xlabel={$x$}, ylabel={$y$},
  xmin=-0.12, xmax=2.15, ymin=-0.12, ymax=2.4,
  xtick={0,0.5,1,1.5,2}, ytick={0,1,2},
  domain=0:2, samples=160, font=\footnotesize,
  legend pos=north west]
\addplot[ybar interval,fill=orange!28,draw=orange!60,domain=0:2,samples=9] {x^2/2};
\addlegendentry{黎曼和 $\sum f(\xi_i)\Delta x_i$}
\addplot[blue,thick] {x^2/2};
\addlegendentry{$y=f(x)=\frac{x^2}{2}$}
\end{axis}
\end{tikzpicture}
\end{center}
\centerline{\footnotesize 用若干小矩形面积之和逼近曲边梯形面积，当 $\lambda=\max\Delta x_i\to 0$ 时，其极限即为定积分。}
\noindent\textbf{拓展（数值积分）:}定积分 $\int_a^b f$ 本身是黎曼和的极限，计算机无法真正“求极限”，于是截断为有限项：在等分点 $x_i=a+ih$ 上，梯形公式 $\int_a^b f\approx\frac{h}{2}\bigl[f(x_0)+2\sum x_i+f(x_n)\bigr]$，Simpson 公式再用抛物线代替直线、获得更高阶精度，这就是“数值积分”的基础；对高维积分，蒙特卡洛方法用随机采样估计平均值的积分。现实中任何定积分在内存里都只能以这样的离散求和近似。
...
\begin{axis}[width=0.82\linewidth, height=5.4cm,
  xlabel={$x$}, ylabel={$y$},
  xmin=0.9, xmax=8.2, ymin=0, ymax=3,
  xtick={1,2,4,6,8}, ytick={0,1,2,3},
  domain=1:8, samples=200, font=\footnotesize,
  legend pos=north east,
  legend style={draw=black!50, fill=white, font=\scriptsize}]
\draw[dashed,gray!50] (axis cs:1,0) -- (axis cs:1,3);
\addplot[red,thick] {x^(-0.5)};
\addlegendentry{$p=\frac{1}{2}$（发散）}
\addplot[blue,thick] {1/x};
\addlegendentry{$p=1$（发散）}
\addplot[green!55!black,thick] {1/x^2};
\addlegendentry{$p=2$（收敛）}
\node[font=\scriptsize,red!70!black,anchor=west] at (axis cs:2.55,0.95){$p=\frac{1}{2}$};
\node[font=\scriptsize,blue!70!black,anchor=west] at (axis cs:2.75,0.50){$p=1$};
\node[font=\scriptsize,green!55!black,anchor=west] at (axis cs:3.00,0.225){$p=2$};
\end{axis}
\end{tikzpicture}
\end{center}
\centerline{\parbox{\dimexpr\linewidth-2em\relax}{\centering\footnotesize 比较 $\int_1^{+\infty}\frac{1}{x^p}\mathrm{d}x$：三条曲线都以 $(1,1)$ 为起点、随后衰减，$p$ 越大衰减越快。$p>1$ 时尾部面积有限（收敛），$p\le1$ 时尾部面积无限（发散）。}}
...
\begin{axis}[width=0.8\linewidth, height=5.4cm,
  ...
  domain=0:1, samples=160, font=\footnotesize,
  legend pos=north west]
\addplot[draw=none,fill=orange!25] coordinates {(0,0) (0.05,0.224) (0.1,0.316) (0.15,0.387) (0.2,0.447) (0.25,0.5) (0.3,0.548) (0.35,0.592) (0.4,0.632) (0.45,0.671) (0.5,0.707) (0.55,0.742) (0.6,0.775) (0.65,0.806) (0.7,0.837) (0.75,0.866) (0.8,0.894) (0.85,0.922) (0.9,0.949) (0.95,0.975) (1,1) (1,1) (0.95,0.902) (0.9,0.81) (0.85,0.722) (0.8,0.64) (0.75,0.562) (0.7,0.49) (0.65,0.423) (0.6,0.36) (0.55,0.303) (0.5,0.25) (0.45,0.203) (0.4,0.16) (0.35,0.122) (0.3,0.09) (0.25,0.062) (0.2,0.04) (0.15,0.022) (0.1,0.01) (0.05,0.003) (0,0)};
...
\end{tikzpicture}
\end{center}
\centerline{\footnotesize 交点 $(0,0),(1,1)$，在 $[0,1]$ 上 $\sqrt{x}\ge x^2$，面积 $A=\int_0^1(\sqrt{x}-x^2)\,\mathrm{d}x=\frac{1}{3}$。}
...
\begin{axis}[width=0.8\linewidth, height=5.4cm,
  xlabel={$x$}, ylabel={$y$},
  xmin=-0.05, xmax=2.15, ymin=-0.05, ymax=5.4,
  xtick={0,0.5,1,1.5,2}, ytick={1,2,3,4,5},
  domain=0:2, samples=160, font=\footnotesize,
  legend pos=north west]
\addplot[fill=blue!12,draw=blue,thick] {x^2+1} \closedcycle;
\addlegendentry{$y=f(x)=x^2+1$}
\addplot[red!70!black,thick,dashed] {2.3333};
\addlegendentry{$y=\bar f=\frac{1}{b-a}\int_a^b f=\frac{7}{3}$}
\draw[red!70!black,thin] (axis cs:0,0) rectangle (axis cs:2,2.3333);
\addplot[only marks,mark=*,red] coordinates {(1.1547,2.3333)};
\node[font=\scriptsize,red!70!black] at (axis cs:1.4,2.5){$\xi$};
\end{axis}
\end{tikzpicture}
\end{center}
\centerline{\footnotesize 曲线下方面积等于高为 $\bar f$ 的矩形面积，存在 $\xi\in[a,b]$ 使 $f(\xi)=\bar f$（积分中值定理）。}
\subsection{其他技巧}
