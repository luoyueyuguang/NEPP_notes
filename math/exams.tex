...
% 解析开关与 main.tex 保持单一全局状态（\showsolutionstrue/false 只在 main.tex 设定）。
% 模拟卷的题解同样包裹在 \ifshowsolutions 中，刷题版整体隐藏、完整版照常显示。
...
=\frac{x^3}{3}+o(x^3).
\]
故 $\lim\limits_{x \to 0} \dfrac{\sin x - x\cos x}{x^3} = \dfrac{1}{3}$。\boxed{\textbf{答案：}(B)}\else \answerarea \fi
...
\ifshowsolutions\par\noindent\textbf{解析：} $x^2-1=0 \Rightarrow x=\pm 1$ 为垂直渐近线。$\lim\limits_{x\to\infty} \dfrac{x^2+1}{x^2-1}=1$，$y=1$ 为水平渐近线。无斜渐近线。共3条。\boxed{\textbf{答案：}(C)}\else \answerarea \fi
...
\]
故 $2I = \pi \cdot \dfrac{\pi}{2} = \dfrac{\pi^2}{2}$，$I = \dfrac{\pi^2}{4}$。\boxed{\textbf{答案：}(B)}\else \answerarea \fi
...
(C) 将原级数按相邻两项分组不改变收敛性，且 $\sum_{n=1}^{N}(a_{2n-1}+a_{2n})=\sum_{k=1}^{2N}a_k$，故必收敛。(D) $\sum(|a_n|-a_n)$ 的部分和等于对应的 $\sum|a_n|$ 部分和减去 $\sum a_n$ 部分和，因此趋于 $+\infty$。\boxed{\textbf{答案：}(D)}\else \answerarea \fi
...
$f_1' + xy f_{11}'' + 2(x^2+y^2)f_{12}'' + 4xy f_{22}''$。\boxed{\textbf{答案：}(B)}\else \answerarea \fi
...
\ifshowsolutions\par\noindent\textbf{解析：} 特征方程：$r^2-3r+2=0$，$r_1=1$, $r_2=2$。右端 $e^x\cos x$ 对应 $1\pm i$，不是特征根。特解设为 $e^x(A\cos x+B\sin x)$。\boxed{\textbf{答案：}(A)}\else \answerarea \fi
...
(A) 反例：$A=\begin{pmatrix}1&0\\0&0\end{pmatrix}$。(B) 反例同上。(D) 特征值可为 $0$ 或 $1$。\boxed{\textbf{答案：}(C)}\else \answerarea \fi
...
\ifshowsolutions\par\noindent\textbf{解析：} $A = \begin{pmatrix} 1 & 1 & 2 \\ 1 & 2 & 1 \\ 2 & 1 & 3 \end{pmatrix}$。顺序主子式：$\Delta_1=1>0$, $\Delta_2=\begin{vmatrix}1&1\\1&2\end{vmatrix}=1>0$, $\Delta_3=|A|=-2<0$。正惯性指数 $p=2$，负惯性指数 $q=1$。规范形为 $y_1^2+y_2^2-y_3^2$。\boxed{\textbf{答案：}(B)}\else \answerarea \fi
...
\ifshowsolutions\par\noindent\textbf{解析：} 由 $t$ 分布定义：$X\sim N(0,1)$, $Y\sim\chi^2(n)$ 独立，则 $\dfrac{X}{\sqrt{Y/n}}\sim t(n)$。$n=3$，故 $\dfrac{\sqrt{3}X}{\sqrt{Y}}\sim t(3)$。\boxed{\textbf{答案：}(B)}\else \answerarea \fi
...
\ifshowsolutions\par\noindent\textbf{解析：} 由抽样分布基本定理，$\dfrac{\bar{X}-\mu}{S/\sqrt{n}}\sim t(n-1)$。\boxed{\textbf{答案：}(B)}\else \answerarea \fi
...
\boxed{\textbf{答案：}\dfrac{1}{2}\ln 2}\else \answerarea \fi
...
\boxed{\textbf{答案：}\dfrac{1}{4}\displaystyle\int_{0}^{1} e^{-x^2}dx + \dfrac{1}{4e}}\else \answerarea \fi
...
\boxed{\textbf{答案：}dx + e\,dy}\else \answerarea \fi
...
\boxed{\textbf{答案：}\pi}\else \answerarea \fi
...
\boxed{\textbf{答案：}3}\else \answerarea \fi
...
\boxed{\textbf{答案：}18.4}\else \answerarea \fi
...
\]
故 $f'(\eta_1)f'(\eta_2)=\dfrac{1-\xi}{\xi}\cdot\dfrac{\xi}{1-\xi}=1$，且 $\eta_1<\xi<\eta_2$。证毕。\boxed{\textbf{答案：}\text{证明如上}}\else \answerarea[8] \fi
...
\end{align*}
合计：$\dfrac{1}{5}+\dfrac{8}{15}=\dfrac{11}{15}$。\boxed{\textbf{答案：}\dfrac{11}{15}}\else \answerarea[8] \fi
...
\end{align*}
\boxed{\textbf{答案：}\text{收敛域 }(-1,1)\text{，和函数 }S(x)=\dfrac{2x}{(1-x)^3}}\else \answerarea[8] \fi
...
\boxed{\textbf{答案：}\dfrac{12\pi a^5}{5}}\else \answerarea[8] \fi
...
\par\noindent\textbf{答案：}(1) $\dfrac{3}{2}\bar{X}$；(2) $X_{(n)}$；(3) 不是无偏估计，修正为 $\dfrac{2n+1}{2n}X_{(n)}$。\else \answerarea[8] \fi
...
\ifshowsolutions\par\noindent\textbf{解析：} $a_n = \dfrac{1}{n}\displaystyle\sum_{k=1}^{n} \dfrac{1}{1+k/n} \to \displaystyle\int_{0}^{1} \dfrac{1}{1+x}\,dx = \ln 2$。\boxed{\textbf{答案：}(A)}\else \answerarea \fi
...
\ifshowsolutions\par\noindent\textbf{解析：} $|f(x)|\leqslant x^2\to0=f(0)$，连续。$f'(0)=\lim_{x\to0}x\sin(1/x)=0$，可导。$x\neq0$ 时 $f'(x)=2x\sin(1/x)-\cos(1/x)$，$\lim_{x\to0}f'(x)$ 振荡不存在，导函数不连续。\boxed{\textbf{答案：}(C)}\else \answerarea \fi
...
\ifshowsolutions\par\noindent\textbf{解析：} 令 $u=x^2-t^2$，$du=-2t\,dt$。$F(x)=\frac{1}{2}\int_{0}^{x^2}f(u)du$，$F'(x)=\frac{1}{2}f(x^2)\cdot2x=x f(x^2)$。\boxed{\textbf{答案：}(A)}\else \answerarea \fi
...
(A) 反例：$a_n=1/(n\ln n)$（$n\geq2$）。(B) 可令 $a_n=1/n^2$（当 $n\neq2^k$），并令 $a_{2^k}=1/2^k$；此时 $\sum a_n$ 收敛，但 $2^k a_{2^k}=1$，故 $na_n$ 不趋于 $0$。(D) 反例：$a_n=1/n$。\boxed{\textbf{答案：}(C)}\else \answerarea \fi
...
\ifshowsolutions\par\noindent\textbf{解析：} 对 $F(x+y+z)=0$ 分别对 $x,y$ 求偏导：$F'\cdot(1+z_x)=0$, $F'\cdot(1+z_y)=0$。由 $F'\neq0$ 得 $z_x=z_y=-1$。故 $xz_y-yz_x = x(-1)-y(-1)=y-x$。\boxed{\textbf{答案：}(A)}\else \answerarea \fi
...
\ifshowsolutions\par\noindent\textbf{解析：} 特征方程 $r^2+4=0$, $r=\pm2i$。右端 $x\sin2x$ 对应 $\pm2i$，是单重特征根。自由项含一次多项式，特解乘以 $x$：$x[(Ax+B)\sin2x+(Cx+D)\cos2x]$。\boxed{\textbf{答案：}(B)}\else \answerarea \fi
...
\ifshowsolutions\par\noindent\textbf{解析：} $AP = P\begin{pmatrix}0&1&1\\1&0&1\\1&1&0\end{pmatrix}$, $|A|=\begin{vmatrix}0&1&1\\1&0&1\\1&1&0\end{vmatrix}=2$。\boxed{\textbf{答案：}(C)}\else \answerarea \fi
...
\ifshowsolutions\par\noindent\textbf{解析：} (C) 错误：$|A|>0$ 只是正定的必要条件。反例：$A=\begin{pmatrix}-1&0\\0&-1\end{pmatrix}$, $|A|=1>0$ 但负定。\boxed{\textbf{答案：}(C)}\else \answerarea \fi
...
\ifshowsolutions\par\noindent\textbf{解析：} $X+Y\sim N(0,2)$，关于 $0$ 对称，$P\{X+Y>0\}=1/2$。\boxed{\textbf{答案：}(B)}\else \answerarea \fi
...
\ifshowsolutions\par\noindent\textbf{解析：} $\sigma^2$ 已知用 $Z$ 区间：$\bar{X}\pm\frac{\sigma}{\sqrt{n}}z_{\alpha/2}$，长度 $=\frac{2\sigma}{\sqrt{n}}z_{\alpha/2}$。\boxed{\textbf{答案：}(A)}\else \answerarea \fi
...
\ifshowsolutions\par\noindent\textbf{解析：} $\dfrac{e^x-e^{\sin x}}{x-\sin x}=e^{\sin x}\cdot\dfrac{e^{x-\sin x}-1}{x-\sin x}\to 1\cdot1=1$。\boxed{\textbf{答案：}1}\else \answerarea \fi
...
\boxed{\textbf{答案：}\dfrac{\pi}{4} - \dfrac{1}{2}}\else \answerarea \fi

\vspace{8pt}

\textbf{13.} 设 $z = \arctan\dfrac{x}{y}$（$y\neq0$），则 $\dfrac{\partial^2 z}{\partial x^2} + \dfrac{\partial^2 z}{\partial y^2} = \underline{\qquad\qquad}$。

\ifshowsolutions\par\noindent\textbf{解析：} $z_x=\frac{y}{x^2+y^2}$, $z_y=-\frac{x}{x^2+y^2}$。$z_{xx}=-\frac{2xy}{(x^2+y^2)^2}$, $z_{yy}=\frac{2xy}{(x^2+y^2)^2}$。和为 $0$。\boxed{\textbf{答案：}0}\else \answerarea \fi
...
\boxed{\textbf{答案：}\dfrac{3}{4}\ln2}\else \answerarea \fi

\vspace{8pt}

\textbf{15.} 设三阶矩阵 $A$ 的特征值为 $1,2,3$，则 $|A^2 - 3A + 2I| = \underline{\qquad\qquad}$。

\ifshowsolutions\par\noindent\textbf{解析：} $A^2-3A+2I$ 的特征值为 $\lambda^2-3\lambda+2$：$1\to0$, $2\to0$, $3\to2$。行列式 $=0\times0\times2=0$。\boxed{\textbf{答案：}0}\else \answerarea \fi
...
\boxed{\textbf{答案：}\dfrac{1}{4}}\else \answerarea \fi
...
若 $x_0\leqslant1/2$，则 $f''(\xi_1)\geqslant8$；若 $x_0>1/2$，则 $f''(\xi_2)\geqslant8$。证毕。\boxed{\textbf{答案：}\text{证明如上}}\else \answerarea[8] \fi
...
\]
已知 $\int_0^{\pi/2}\ln\cos\theta\,d\theta = -\frac{\pi}{2}\ln2$，故原式 $= \pi\ln2$。\boxed{\textbf{答案：}\pi\ln2}\else \answerarea[8] \fi
...
\boxed{\textbf{答案：}\displaystyle\sum_{n=0}^{\infty}\left(\frac{1}{2^{n+1}}-\frac{1}{3^{n+1}}\right)(x+4)^n\text{，收敛域 }(-6,-2)}\else \answerarea[8] \fi
...
\boxed{\textbf{答案：}2\pi a^3}\else \answerarea[8] \fi
...
\boxed{\textbf{答案：}(1)\ a=3,\ b=1\text{；}(2)\ f=y_2^2+4y_3^2}\else \answerarea[8] \fi
...
\par\noindent\textbf{答案：}(1) $\dfrac{\bar{X}}{1-\bar{X}}$；(2) $-\dfrac{n}{\sum\ln X_i}$；(3) 有偏但一致，$\dfrac{n-1}{n}\hat{\theta}_L$无偏。\else \answerarea[8] \fi
...
由于左右导数分别为 $\lim_{x\to0^+}\frac{f(x)-f(0)}{x}=1$, $\lim_{x\to0^-}\frac{f(x)-f(0)}{x}=-1$，不相等，故 $f'(0)$ 不存在。\boxed{\textbf{答案：}(D)}\else \answerarea \fi
...
因此极限为 $\sqrt{\pi/2}$。\boxed{\textbf{答案：}(B)}\else \answerarea \fi
...
\ifshowsolutions\par\noindent\textbf{解析：} $S(x)$ 以 $2\pi$ 为周期。$S(3\pi)=S(\pi)=f(\pi)=\pi$。由狄利克雷收敛定理，在连续点处和函数等于 $f(x)$。$x=\pi$ 处 $f$ 连续（$f(\pi)=|\pi|=\pi$），故 $S(3\pi)=\pi$。\boxed{\textbf{答案：}(C)}\else \answerarea \fi
...
\ifshowsolutions\par\noindent\textbf{解析：} 由平面散度公式（格林公式），$\oint_C\frac{\partial u}{\partial n}\,ds = \iint_{D_C} (\frac{\partial^2 u}{\partial x^2}+\frac{\partial^2 u}{\partial y^2})\,dxdy = \iint_{D_C} 0\,dxdy = 0$。\boxed{\textbf{答案：}(A)}\else \answerarea \fi
...
特解 $y_p=-x\cos x+\sin x\ln|\sin x|$。\boxed{\textbf{答案：}(C)}\else \answerarea \fi
...
\ifshowsolutions\par\noindent\textbf{解析：} 对于实矩阵，$A^T A x=0 \iff Ax=0$（因为 $x^T A^T A x = \|Ax\|^2=0 \iff Ax=0$），故 $A^T A$ 与 $A$ 有相同的零空间，从而 $r(A^T A)=r(A)$。同理 $r(AA^T)=r(A^T)=r(A)$。故 (A)(B)(C) 全对。\boxed{\textbf{答案：}(D)}\else \answerarea \fi
...
求导：$f_Y(y)=2\varphi(y)=2\cdot\frac{1}{\sqrt{2\pi}}e^{-y^2/2}$（$y>0$）；$y\leqslant0$ 时 $f_Y(y)=0$。\boxed{\textbf{答案：}(B)}\else \answerarea \fi
...
\ifshowsolutions\par\noindent\textbf{解析：} 拒绝域扩大后，在原假设下落入拒绝域的概率不减，故$\alpha$不减；在给定备择下落入接受域的概率不增，故$\beta$不增。只有当新增区域在相应原假设分布和备择分布下均有正概率时，才能写成严格的“$\alpha$增大、$\beta$减小”。\boxed{\textbf{答案：}(B)}\else \answerarea \fi
...
\ifshowsolutions\par\noindent\textbf{解析：} 由洛必达法则：$\lim_{x\to0}\frac{e^{x^2}-1}{3x^2} = \lim_{x\to0}\frac{x^2+o(x^2)}{3x^2} = \frac{1}{3}$。\boxed{\textbf{答案：}\dfrac{1}{3}}\else \answerarea \fi
...
或配方 $x(2-x)=1-(x-1)^2$，原式 $=\int_0^2\frac{dx}{\sqrt{1-(x-1)^2}} = \arcsin(x-1)\big|_0^2 = \arcsin1-\arcsin(-1)=\pi$。\boxed{\textbf{答案：}\pi}\else \answerarea \fi
...
\boxed{\textbf{答案：}\dfrac{1}{a}}\else \answerarea \fi
...
\boxed{\textbf{答案：}-\dfrac{2\pi}{3}}\else \answerarea \fi
...
$\operatorname{tr}(A) = 1+1+0 = 2$。$\operatorname{tr}(A^2) = \operatorname{tr}(2A) = 2\cdot2 = 4$。\boxed{\textbf{答案：}4}\else \answerarea \fi
...
\boxed{\textbf{答案：}\left\lceil\dfrac{4\sigma^2 z_{0.025}^2}{L^2}\right\rceil}\else \answerarea \fi
...
(3) 需要证明 $\lim n a_n = 0$。取 $q<r<1$，则当 $n$ 充分大时 $a_n \leqslant C r^n$。$\lim n r^n = 0$（由洛必达法则：$\lim_{x\to\infty} x r^x = \lim\frac{x}{r^{-x}} = \lim\frac{1}{-r^{-x}\ln r} = 0$）。由夹逼定理，$\lim n a_n = 0$。证毕。\boxed{\textbf{答案：}\text{证明如上}}\else \answerarea[8] \fi
...
$f''(\xi) = \frac{f'(\xi_2)-f'(\xi_1)}{\xi_2-\xi_1} < 0$（因为分子 $<0$，分母 $>0$）。证毕。\boxed{\textbf{答案：}\text{证明如上}}\else \answerarea[8] \fi
...
\]
从而 $S=\frac{\pi^2}{6}$。
\par\noindent\textbf{答案：}(1) 奇周期延拓；(2) $S(-1/2)=-1/2$，$S(5/2)=-1/2$；(3) $\displaystyle\sum_{n=1}^{\infty}\frac{1}{n^2}=\frac{\pi^2}{6}$。\else \answerarea[8] \fi
...
\boxed{\textbf{答案：}2\pi}\else \answerarea[8] \fi
...
\boxed{\textbf{答案：}(1)(2)\ \hat{p}=1/\bar{X}\text{；(3)\ }\hat p_L\xrightarrow{P}p}\else \answerarea[8] \fi
