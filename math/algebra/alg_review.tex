...
线性代数诸概念环环相扣，其主线关系如下图所示：
\begin{center}
\begin{tikzpicture}[>=stealth, font=\scriptsize]
\node[draw, rounded corners, fill=blue!8, minimum width=2.0cm, minimum height=0.7cm] (det) at (0,3.3) {行列式};
\node[draw, rounded corners, fill=blue!8, minimum width=2.0cm, minimum height=0.7cm] (mat) at (0,1.7) {矩阵};
\node[draw, rounded corners, fill=blue!8, minimum width=2.0cm, minimum height=0.7cm] (vec) at (-3.6,0) {向量组};
\node[draw, rounded corners, fill=blue!8, minimum width=2.8cm, minimum height=0.7cm] (sys) at (0,0) {线性方程组};
\node[draw, rounded corners, fill=blue!8, minimum width=2.2cm, minimum height=0.7cm] (eig) at (3.7,0) {特征值/二次型};
\node[draw, rounded corners, fill=blue!8, minimum width=3.0cm, minimum height=0.7cm] (rank) at (0,-1.8) {秩与解的结构};
\draw[->] (det) -- node[right,font=\tiny] {可逆} (mat);
\draw[->] (mat) -- node[above,font=\tiny] {线性变换} (eig);
\draw[->] (mat) -- node[left,font=\tiny] {行变换} (sys);
\draw[->] (vec) -- node[above,font=\tiny] {线性表出} (sys);
\draw[->] (sys) -- node[right,font=\tiny] {秩判定} (rank);
\draw[->] (vec) -- node[left,font=\tiny] {秩} (rank);
\end{tikzpicture}
\end{center}

行列式的绝对值反映了二阶（三阶）行列式作为有向面积（体积）的几何意义。以 \(\boldsymbol{a}=(2,1)\)，\(\boldsymbol{b}=(1,3)\) 为例，\(\det=2\cdot3-1\cdot1=5\)：
\begin{center}
\begin{tikzpicture}
\begin{axis}[
  axis lines=center,
  ...
  xmin=-0.6, xmax=3.9, ymin=-0.6, ymax=4.6,
  xtick={1,2,3}, ytick={1,2,3,4},
  font=\footnotesize,
]
\draw[blue,thick,fill=blue!12] (axis cs:0,0) -- (axis cs:2,1) -- (axis cs:3,4) -- (axis cs:1,3) -- cycle;
\draw[->,thick,red] (axis cs:0,0) -- (axis cs:2,1);
\draw[->,thick,green!50!black] (axis cs:0,0) -- (axis cs:1,3);
\node[red,anchor=south east] at (axis cs:2,1) {$\boldsymbol{a}=(2,1)$};
\node[green!50!black,anchor=south east] at (axis cs:1,3) {$\boldsymbol{b}=(1,3)$};
\node[black] at (axis cs:2.55,2.35) {面积 $=\left|\det\right|=5$};
\end{axis}
\end{tikzpicture}
\end{center}
