...
\begin{axis}[
  ...
  xmin=-3.2, xmax=5.4, ymin=-2.2, ymax=6.2,
  xtick={-2,-1,1,2,3,4}, ytick={-1,1,2,3,4,5},
  font=\footnotesize,
]
\draw[blue,thick,dashed] (axis cs:-3,-1.5) -- (axis cs:5,2.5);
\draw[red,thick] (axis cs:-3,1) -- (axis cs:5,5);
\fill[red] (axis cs:1,3) circle (2pt);
\node[red,anchor=south west] at (axis cs:1,3) {$\boldsymbol{\eta}^*$};
\draw[->,thick,blue] (axis cs:1,3) -- (axis cs:3,4);
\node[blue,anchor=south] at (axis cs:3,4) {$\boldsymbol{\xi}$};
\node[blue] at (axis cs:2.7,1.9) {$A\boldsymbol{x}=\boldsymbol{0}$ 解空间};
\node[red] at (axis cs:3.0,4.6) {$A\boldsymbol{x}=\boldsymbol{b}$ 解集};
\end{axis}
\end{tikzpicture}
\end{center}
