...
\noindent\textbf{拓展（极大似然与学习）:} 最大似然估计 $\widehat{\theta}=\arg\max_{\theta}\prod_{i=1}^{n}p_\theta(x_i)$ 在统计上与“最小化负对数似然”等价；对独立同分布样本取负对数后，它正是监督学习的训练目标——分类问题对应交叉熵损失，高斯噪声回归对应均方误差。贝叶斯估计再引入先验，改为最大化后验 $\widehat{\theta}_{\mathrm{MAP}}=\arg\max_{\theta}\,p(\theta)\prod_{i}p_\theta(x_i)$；当先验取高斯、噪声亦高斯时，MAP 即退化为带 $L_2$ 正则的岭回归。
...
\addplot[red!20,fill=red!20,draw=none,domain=-4:-1.96] {0.3989*exp(-x^2/2)} \closedcycle;
\addplot[red!20,fill=red!20,draw=none,domain=1.96:4] {0.3989*exp(-x^2/2)} \closedcycle;
\addplot[blue,thick] {0.3989*exp(-x^2/2)};
\addlegendentry{检验统计量分布}
\draw[dashed,gray!70] (axis cs:-1.96,0) -- (axis cs:-1.96,0.42);
\draw[dashed,gray!70] (axis cs:1.96,0) -- (axis cs:1.96,0.42);
\node[font=\scriptsize] at (axis cs:-1.96,0.02){$-z_{\alpha/2}$};
\node[font=\scriptsize] at (axis cs:1.96,0.02){$z_{\alpha/2}$};
\node[font=\scriptsize,red!70!black] at (axis cs:-3.2,0.10){拒绝 $H_0$};
\node[font=\scriptsize,red!70!black] at (axis cs:3.2,0.10){拒绝 $H_0$};
\node[font=\scriptsize,blue!60!black] at (axis cs:0,0.42){不拒绝 $H_0$};
\end{axis}
\end{tikzpicture}
\end{center}
